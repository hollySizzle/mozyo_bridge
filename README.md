...
- `./tmp/mb-smoke-asana` / `./tmp/mb-smoke-redmine` / `./tmp/mb-smoke-redmine-rails` のような isolated target を使う (`./tmp/` は `.gitignore` 配下の作業領域)。
...
5. install 直後の前提を確認します。Claude 側は plugin marketplace 経路を、`mozyo-bridge doctor` は CLI / rules / Codex skill / (fallback path を使った場合のみ) Claude skill を見ます。
   ...
   このタイミングでは `scaffold` section が `missing` (`-> mozyo-bridge scaffold rules <preset> --target ...`) になりますが、`cli` / `rules` / `codex_skill` の 3 section が ok であることを確認します。`next_action` (`-> ...`) を読み、不足があれば該当 install / set up を再実行してください。
...
6. isolated target に対して Asana / Redmine の repo-local scaffold を smoke:
   ...
   mozyo-bridge scaffold rules redmine-rails --target ./tmp/mb-smoke-redmine-rails
   mozyo-bridge scaffold status --target ./tmp/mb-smoke-redmine-rails
   mozyo-bridge doctor --target ./tmp/mb-smoke-redmine-rails
   ```

   各 target で `scaffold status` が `result: clean` を返し、`mozyo-bridge doctor --target ...` の `scaffold` section が `ok` であれば、user-global 規約・repo-local routers・manifest が整合しています。主要 preset boundary (Asana / Redmine / Redmine Rails) を確認します。片側だけで完了させると preset 間 boundary の検証が落ちます。
...
mozyo-bridge scaffold rules redmine-rails --target /path/to/project
...
- `vibes/docs/logics/scaffold-rules.md`: scaffold rules presets and YAML registry governance.
