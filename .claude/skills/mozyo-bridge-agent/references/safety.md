...
- 送信 rail は 2 本存在する。どちらかを弱めるのではなく、正しい方を選ぶ:
  ...
  - **Herdr backend の queue-enter** は admission 後に本文を高々 1 回だけ type する。first Enter は body injection 後の pinned generation 再確認、working-transition wait の arm、absolute deadline check がすべて成功した場合だけ発行するため zero-or-one である。その後は既定 30 秒 / 2 秒の単一 budget 内で、同一 target / launch generation / composer body / screen / runtime state を fresh に確認した Enter-only fallback を行える。wait error は次の Enter を許可しない。同じ terminal と collision-free generation に結び付く causal turn-start を確認した場合だけ `sent` / `ok` / `submitted_confirmed` とし、それ以外は precise `blocked` + non-zero に閉じる。本文は再入力しない。
