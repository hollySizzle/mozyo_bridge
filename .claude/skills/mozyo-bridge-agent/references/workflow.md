...
- **review head 連言 (Redmine #13974)** — review-gate producer は central preset の Review Generation Marker Contract v2 (`review_request` / `review` gate の structured_marker) を読み、canonical renderer / producer 契約に従って marker を付与する。generation identity を散文から組み立てない。field key / 必須 gate 組合せ / missing・drift disposition / source_sequence authority は preset 正本にあり、本 reference は複製しない。
