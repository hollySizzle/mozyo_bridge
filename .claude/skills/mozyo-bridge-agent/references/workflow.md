...
- `mozyo-bridge handoff cross-workspace-consult` (Redmine #11779) packages this route into one boundary-preserving command: receiver fixed to `codex` (no `--to`; never types into a foreign Claude pane), `--target` + `--target-repo` mandatory so the identity gate always runs, and `--kind` defaulting to `design_consultation`. The target Codex reads the durable anchor and performs the **local same-session Claude handoff** if implementation follows. All gates are delegated to `handoff send` and are not weakened. See the canonical `skills/mozyo-bridge-agent/references/workflow.md` for the full route.
