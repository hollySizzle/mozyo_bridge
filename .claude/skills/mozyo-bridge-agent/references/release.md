...
- `source_ref` は **origin 上の ref literal** で綴る (`refs/heads/<branch>` が canonical、短縮 `<branch>` も可)。git が local で表示する remote-tracking name (`origin/<branch>` / `refs/remotes/origin/<branch>`) は origin 上に存在しないため **渡さない**。helper は dispatch 前に reject し exact な訂正を返す (silent normalize はしない)。policy 正本: `vibes/docs/logics/release-helper-contract.md` の `### source_ref Spelling Policy` (Redmine #13883)。
