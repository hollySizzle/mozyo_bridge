...
- **Redmine** (default for `mozyo_bridge`; preset `redmine-governed`): the durable work log is the Redmine issue and its journals. Gates such as Start / Implementation Done / Review Request / Review / Owner Close Approval / Close land as separate journal entries on the issue. The standard Review Request / Review / owner close approval unit is the UserStory (`### US-Level Audit Model` in the central preset): child Task / Test / Bug issues carry implementation/verification journals and feed one US-level Codex audit instead of per-task reviews, except for the preset-listed task-level exceptions. The central preset (`${MOZYO_BRIDGE_HOME:-~/.mozyo_bridge}/rules/presets/redmine-governed/agent-workflow.md`) defines the required fields per gate; this skill must not duplicate those tables.
  - **Issue authoring granularity and Version operation**: how to choose Epic / Feature / UserStory / leaf granularity when authoring tickets, how to separate the owner utterance digest from normalized intent, and how to size / select Redmine Versions as planning and lane-inventory buckets live in `references/redmine-issue-authoring.md`.
...
The portable rule is *that creators pass an explicit concise subject and correct a malformed subject immediately on the durable record*. An operator's private subject phrasing style, naming templates, or ticket-title conventions are their own runbook (see the adopting repo's public / private boundary rule; in `mozyo_bridge`, `vibes/docs/rules/public-private-boundary.md`), not the distributed skill or preset body.

## Local Documentation

- `AGENTS.md` and `CLAUDE.md` are routers.
- The adopting repo's own docs namespace holds its working rules, structure / specification notes, decision and release logic, and reusable templates; resolve it through the repo's router and docs catalog rather than assuming a fixed layout. In the `mozyo_bridge` repo itself that namespace is `vibes/docs/` (`rules/` working rules, `specs/` structure and specification notes, `logics/` decision and release logic, `temps/` reusable templates).
...
1. **Record the durable anchor first, then submit-complete the send.** Run `mozyo-bridge handoff send --to claude --source redmine --issue <issue_id> --journal <journal_id> --kind <implementation_request|review_request|...>`. The default `queue-enter` rail presses Enter with no `--mode` needed when the Claude pane is the active split of its window, or when an inactive registered Claude pane passes standard_target_admission (step 2).
2. **Inactive-split Claude pane: standard_target_admission activates a registered pane; only unadmitted panes need `--mode standard`.** In the common cockpit-grid case the Codex pane is the active split and the Claude pane is an inactive sibling. Since Redmine #12597 the default `queue-enter` rail runs standard_target_admission: a *registered* inactive Claude pane (live / strong role / `workspace_id` present / unambiguous) is activated via `tmux select-pane` and submit-completed on the default rail with no `--mode` needed — pane selection only, never raw key injection, and the activation is recorded in the durable record. Only when the pane is **not** admitted (e.g. no `workspace_id`), or activation is disabled with `--no-target-activation`, does the rail fail-closed (`blocked` / `invalid_args`) and type nothing; then re-dispatch on the strict-but-submitting rail with `--mode standard --target <claude_%pane> --target-repo auto`, exactly as the block's emitted recovery command instructs. `--mode standard` observes the landing marker and then presses Enter, so it submit-completes against the inactive same-identity pane — the same `--mode standard` choice the callback procedure step 4 makes for the (also typically inactive) coordinator pane.
...
The submit rails themselves are fixed by `references/safety.md` (`queue-enter` default, `standard` strict explicit fallback, `pending` operator-submit path; the deep send-safety contract doc in the `mozyo_bridge` repo is `vibes/docs/logics/tmux-send-safety-contract.md`); this section only fixes *which* rail a same-lane dispatch uses so it reaches submit instead of stalling pending. It does not relax the receiver-binding / per-receiver process gates, and it does not introduce a blind Enter (Redmine #12207 non-goals). The Redmine #12597 standard_target_admission activation of an inactive registered pane uses pane selection (`tmux select-pane`) only — never raw key injection — and leaves the minimal admission contract and the other preflight gates intact.
...
- The portable rule: when the installed CLI may lag the features a dispatch or callback needs, prefer a runtime known to carry them and record the runtime fingerprint, so the send is not silently blocked by a version that predates a needed subcommand. This applies mainly to the repo that develops the CLI itself — in `mozyo_bridge` dogfooding, prefer the repo-local source CLI (`PYTHONPATH=src python3 -m mozyo_bridge ...`) over the plain installed `mozyo-bridge` until release / install catches up.
...
- **Keep operator-specific policy out of OSS defaults.** The concrete lane-count soft profile (target / burst / stop numbers), the private cockpit composition, and the operator's path / branch naming are operator runtime policy that lives in the spine's local profile and the operator's own runbook (see the adopting repo's public / private boundary rule; in `mozyo_bridge`, `vibes/docs/rules/public-private-boundary.md`), never in this distributed body. The portable part is *that the coordinator re-runs the fill loop after every dispatch and every drain, classifies lanes by the minimal coordinator-blocking vocabulary, drains in the Drain Order, and records one durable fill decision whenever ready work is not dispatched* — the concrete numbers are the operator's.

## Integration Disposition And Push Authority

Pushing is a two-layer authority, and the layers have different owners (Redmine #13026, from the #13024 / #13025 main-unit sessions where an implementer read the preset's "push before recording" as license to push `origin/main` directly, and a shared-checkout branch switch put an implementer commit on the wrong branch):

- **The implementer pushes the issue / lane branch, and only that.** The origin-reachability precondition on `implementation_done` / `review_request` (see `### Origin reachability preflight before recording a commit hash in a gate`) is satisfied by branch reachability — it never requires, or permits, the implementer to advance `origin/main`, a release branch, or any other integration branch. An implementer commit that accidentally lands on the integration branch locally is moved to the issue branch and recorded as a correction; it is not pushed.
- **The coordinator owns integration.** After review approval, the coordinator advances the integration branch and records the decision as an **integration disposition** — one of `merge` (fast-forward-only is the default; a non-ff merge commit or rebase integration records its reason), `patch_equivalent` (the change reached the target branch in equivalent form; record how equivalence was checked), or `explicit_deferral` (integration intentionally postponed; record what unblocks it). The disposition journal names the integrated commits, the merge type, and the post-merge verification. This is the durable definition behind the `integration_waiting` coordinator-blocking state in `## Post-Dispatch Fill Loop`: a commit-bearing unit is `integration_waiting` until its disposition journal lands.
- **Main-unit exception implementation still branches.** When a dispatch decision records the exception of implementing in the main unit / primary checkout, the implementer still creates an issue branch and pushes that branch; work is never committed directly on the primary checkout's integration branch. When the coordinator and the implementer share one checkout, branch switches can collide — verify the current branch before committing, prefer a dedicated worktree, and record any collision as a correction rather than silently repairing it.
- **Authority fields stay in the central preset.** The gate-level statement of this rule (implementer push ref restriction, integration disposition record, the `implementer_advances_integration_branch` prohibited transition) lives in the central preset's `### Commit Hash Origin 到達可能性`; this section is the operating procedure and does not restate the preset's field tables.

## Existing-Project Sublane Adoption

The sublane sections above describe how a coordinator runs the flow once a project already carries the governed scaffold. This section is the portable extract of the repo-local runbook `vibes/docs/logics/existing-project-sublane-adoption.md` (Redmine #12423): how to bring an **existing** project — one that already has code, routers, a ticket lifecycle, and its own docs — onto the governed scaffold and sublane flow **without breaking its existing routing**. It is the adopter-facing procedure reachable from the `--with-sublane-flow` opt-in profile (`vibes/docs/profiles/sublane-flow-runtime-profile.md`); the dogfood-specific lane counts, cockpit composition, and absolute paths stay out of this distributed body.

Adoption is the setup path, not a relaxation of any gate. Owner close approval, the Review / Close separation, and the release / credential / destructive-operation gates all stay exactly as the rest of this reference defines them. The one adoption-specific allowance is a **bootstrap exception**: when child gateways or target-lane docs do not yet exist, the coordinator may proceed on a thinner pre-dispatch decision record and record the missing decision as a follow-up correction. The bootstrap exception only repairs a thin decision record; it never bypasses an approval invariant and is never a shortcut for ordinary development.

### When existing-project adoption applies

Use this procedure when the project will treat the durable ticket record (Redmine journal) as its source of truth, agents run a continuing implement / review / close lifecycle that pane chat alone cannot replay, and the project root has no router or an ambiguous router-to-subdir relationship. Do not use it for a one-off sandbox, a short-lived demo, a project with no catalog owner to maintain `catalog.yaml` past day one, or a project that would only work by hard-coding private operating policy as an OSS default.

### Read-only preflight before any adoption edit

Before changing the adoption target, the coordinator records a **read-only preflight** on the durable record. It captures: the target's durable work system and ticket project; whether `AGENTS.md` / `CLAUDE.md` already exist and whether they are hand-edited or scaffold-managed; the presence of `.mozyo-bridge/scaffold.json`, a repo-local or central rules store, and an existing `.mozyo-bridge/docs/catalog.yaml`; the root-to-subdir relationship so a root adoption does not silently overwrite a subdir catalog or subproject router; and `git status --short` with the current branch, naming any unrelated dirty files as out of scope. The preflight states the reason to adopt, the paths it will touch, the paths it will preserve, the intended preset, the verification plan, and known risks. **Preserve existing routing**: root adoption must keep the existing subdir and project-local docs reachable from the root router rather than overwriting them.

### Decompose adoption into an implementation child and a verification child

Split adoption into a parent UserStory (the adoption goal and close conditions), an implementation child (the scaffold / rules / catalog / router changes), and a verification child (dry-run, status, docs validation, handoff smoke). Keeping commit-bearing implementation and dry-run verification in separate issues stops their review anchors from mixing; the parent US audit then reads the child issues, commits, verification, callback outcomes, and integration disposition together.

### Dispatch decision and scaffold / rules / catalog adoption

Implementation-shaped adoption work defaults to a sublane dispatch (`## Post-Dispatch Fill Loop` routing); a coordinator that edits in its own lane records the exception reason. The dispatch decision records the target issue, the target lane / branch identity, `work_shape: implementation`, the expected changed paths, the reason main-lane work is or is not used, and the callback expectation. The implementation lane runs `mozyo-bridge rules install`, `scaffold status --target .`, `scaffold apply <preset> --target . --backup`, then `scaffold status --target .` again (repo-local stores use the `--repo-local` variants; never mix central and repo-local). It reads every `scaffold apply` diff and confirms the root router is a thin router pointing at the preset, that shipped artifacts are tracked in `.mozyo-bridge/scaffold.json`, and that existing subdir routers, business docs, private overlays, and generator outputs are left unedited. `.mozyo-bridge/docs/catalog.yaml` is target-owned data the scaffold never overwrites; a `rules install` hash-only drift is corrected and recorded in a commit and journal, kept distinct from router / artifact body drift. On first adoption the shipped `catalog.yaml.example` is adapted into the target's `catalog.yaml` with coverage roots that exist and file conventions for the root router, governed artifacts, project docs, and implementation paths — never duplicating an existing subdir catalog into a second source of truth, and never recording a private path, credential, or operator-specific cockpit composition.

### Verification, origin-reachable commit, callback recovery, and close order

The minimum verification is `scaffold status --target .`, `doctor --target .`, `docs validate --repo .`, `docs validate --check-file-coverage --repo .`, `docs generate-file-conventions --check --repo .`, and `git diff --check`, extended as needed with `docs resolve`, `docs audit-impact --all-changed --check-generated --repo .`, target-specific tests, and a same-lane handoff / coordinator callback smoke; the verification child records the target's clean status, docs validation, generated sync, and dry-run smoke independently of the commit-bearing implementation. A commit used as a review or close anchor must be origin-reachable; a local-only commit reads as an implementation observation but is never a close or audit anchor (`### Origin reachability preflight before recording a commit hash in a gate`). If the implementation branch carries unrelated local history, build a clean integration branch from the target branch and cherry-pick only the adoption-scope commits, recording the original-to-integration commit mapping. The sublane sends a coordinator callback on every handoff-worthy state (`## Sublane Coordinator Callback`); when a callback is missing or misaddressed the coordinator may recover from a Redmine journal sweep, recording which state had advanced on the durable record, where the callback did or did not arrive, which journal it recovered from, and the route correction — the journal is the source of truth, the callback is a pointer, and a nagger is a secondary signal, never the primary control. Close runs in order: audit the child implementation / verification issues, record the parent US Review Gate, collect the owner close approval as a separate journal, close the children, then record the parent US Close Gate.

### Boundaries existing-project adoption does not relax

- **Adoption never relaxes an approval gate.** A clean `scaffold status` is not workflow adoption — the project must actually run Start / handoff / callback / review / close on the durable record. The bootstrap exception only fills a thin decision record; owner close approval, the Review / Close separation, and the release / credential / destructive-operation gates are unchanged.
- **The durable record stays the source of truth.** The adoption decisions, the preflight, the dispatch, callback recovery, and the integration disposition are recorded on the ticket journals, not inferred from pane scrollback / `status` / `doctor`. `catalog.yaml.example` is the shipped skeleton; `catalog.yaml` is the target-owned source of truth, and generated file conventions are never hand-edited.
- **Keep operator-specific policy out of OSS defaults.** Private absolute paths, the operator's cockpit composition, parallel-lane counts, and session / window naming stay in the operator's own private operating profile (see the adopting repo's public / private boundary rule; in `mozyo_bridge`, `vibes/docs/rules/public-private-boundary.md`), never in this distributed body or the shipped scaffold defaults. The portable part is *the adoption sequence — read-only preflight, child decomposition, a dispatch decision that preserves existing routing, scaffold / rules / catalog adoption, verification, an origin-reachable commit, callback recovery, and the close order* — the concrete numbers and paths are the operator's.
...
- **Keep operator-specific policy out of OSS defaults.** Whether a given operator runs a Claude pane in the main unit at all, which concrete tasks they routinely offload to it, and any private prioritization are operator runtime policy (see the adopting repo's public / private boundary rule; in `mozyo_bridge`, `vibes/docs/rules/public-private-boundary.md`). The portable part is *that a main-unit Claude is an assistant whose output is input-not-evidence, may take the listed safe tasks, and must not take owner-facing or gate decisions or implement outside a gated lane*; the operator's concrete offload list belongs in their own runbook, not in the distributed skill or preset body.

## Evidence Provenance In Durable Records

Every load-bearing claim written into a gate journal — a review finding, the rationale behind a dispatch instruction, the input to a design consultation — declares **whose authority it rests on**, separately from how certain it is (the fact / hypothesis classification is the certainty axis; provenance is the authority axis, and the two are orthogonal). The provenance classes are fixed at four: **owner intent** (valid only with a durable anchor — a journal id or the issue description's utterance digest), **documented rule** (path plus section), the **agent's own judgment** (a contestable claim, not a demoted one), and **hearsay** (an unrecorded owner utterance, which must be labeled as such).

The rule with teeth is not the label but the weight: **an unrecorded owner utterance never justifies a block, a 要修正 finding, or a gate transition by itself.** To use it, record it first — the user-facing window (Codex in the default role split) confirms with the owner and lands the utterance in the durable record, after which it counts as owner intent. An owner-intent claim without an anchor is treated as hearsay regardless of how it is labeled. This generalizes the existing rule that an owner's raw answer observed in a pane is unconfirmed input for close approval: it applies to every gate and every agent, in both directions — a reviewer cannot ground a finding in reported speech the implementer cannot verify, and an implementer cannot ground a decision in a pane remark it never recorded. The normative field requirements live in the central preset's `### 根拠出所分類 (Evidence Provenance)`; this section does not restate them.
...
- When Codex makes a direct edit under an exception, the durable record is **system-specific** and must exist BEFORE the edit lands:
  ...
  - Redmine projects (including the `mozyo_bridge` repo, which uses the `redmine-governed` preset): create a Redmine `codex_direct_edit` gate journal on the active issue before any Codex edit. Its required fields — `role: 実装者`, `direct_edit: true`, `allowed_paths`, `reason`, `follow_up_review` — are defined by the central preset's Gate Schema (`codex_direct_edit`), which is authoritative; this reference does not restate their semantics. A direct edit without the gate journal is itself a violation subject to correction.
...
The `redmine-governed` and `redmine-rails-governed` presets distribute a **Repo-Local Guardrail Autonomous Lane** that carves a narrow set of repo-local paths out of the `codex_direct_edit` gate: by default `vibes/docs/rules/**`, `vibes/docs/logics/**`, `vibes/docs/specs/**`, and `.mozyo-bridge/docs/catalog.yaml`. Inside the lane Codex edits directly without a pre-edit gate journal and records a `codex_autonomous_edit` journal (`lane: autonomous`) instead. The lane is enabled by the preset, not by chat phrasing; project-local additions may extend or restrict it, but never onto distributed / runtime / implementation surfaces.

The central preset's `### Repo-Local Guardrail Autonomous Lane` is authoritative for the journal fields, the pre-commit verification commands, the non-entry conditions, and the correction flow for a missing journal; this reference does not restate them. Two skill-level reminders survive here because they shape how agents react: a lane commit without its `codex_autonomous_edit` journal is a record-keeping correction (the change is not reverted on that basis alone), and any modification to the lane definition itself is a workflow / guardrail change that triggers the standard `Workflow Change Verification` flow.
