...
<!-- mozyo-bridge:activation:always id=canonical-terminology-discipline digest="未定義の project 固有語を定義済み用語として使わない。新概念は `proposed term` と明示し定義・適用範囲・既存語との差を書く。正本: skill `references/workflow.md` の `### 未定義の project 固有語を定義済み用語として使わない` 節。" -->

ticket (Redmine issue / journal、Asana task / comment) と、ユーザー向け narrative の双方で、**正本で定義を確認できない project 固有語を、定義済みの語彙であるかのように使わない**。

これは文体の統制ではなく、durable record の検証可能性を守る規則である。定義のない語を正式名称のように使うと、次が壊れる:

- 観測した事実と推定した原因が同じ名前に畳まれ、両者の境界が読み手から見えなくなる。
- 後続 agent / reviewer / ticket 検索が同じ語を再現できず、記録が再利用できない。
- 未確認の subsystem 名が issue subject や acceptance に入り、誤った scope 分割や実装先の決め打ちを誘発する。
- 通常の説明文と machine-readable token の区別が崩れる。

#### 語の区分

- **定義済み用語**: cataloged docs、central preset / rule / skill 本文、公開 CLI の help / schema、または code 上の closed vocabulary で、意味と適用範囲を確認できる語。そのまま使ってよい。
- **通常の説明文**: 観測事実を一般語で書いた文。新しい project 用語を命名していなければ、この規則の対象ではない。表現の新しさ自体は禁止しない。
- **未定義の project 固有語**: 既存語の組み合わせを含め、上記の正本で意味・範囲を確認できないのに正式名称として扱う語。ticket と narrative では使わない。
- **`proposed term`**: 新概念の検討に必要な仮称。`proposed term: <name>` と明示し、同じ記録に定義、適用範囲、既存語との差、正本候補、採用状態を書く。採用が決まるまで issue subject、gate 名、reason token、acceptance 語彙には使わない。
- **machine-readable literal**: gate 名、transport kind、JSON field と値、CLI command / flag、code 識別子、commit trailer、file path、branch 名。改名も翻訳もしない。code 識別子を source 箇所の引用として literal に書くことは許可されるが、その識別子を確認済みの project 概念へ一般化しない。

#### 使う前の確認

project 固有語を書く前に、作業 domain の cataloged docs を解決して読み、必要なら公開 CLI / code 上の vocabulary を確認する。定義を確認できないときは、**原因を命名せず**、定義済み用語と通常の説明文で「何を観測したか」を書く。

- 許可される形: 「`review_request` の後、same-lane `implementation_gateway` 宛ての callback outcome が durable record に無く、delivery を確認できない」。観測できた事実と確認できていない事実を分けて書き、未確認の原因に名前を付けていない。
- 禁止される形: 正本で定義を確認できない複合語を、issue subject、原因名、gate 名、reason token、acceptance 語彙として使う。

#### 訂正

違反に気づいたら、同一 turn または判明直後の turn で訂正する。

- ユーザー向け応答: その語が未定義であった事実、撤回する対象、置換後の観測事実を明示する。
- ticket: 履歴を黙って上書きせず、同じ ticket に correction record (Redmine では correction journal) を追加し、元の記録への pointer、撤回する対象、定義済み用語による置換、未確認事項を書く。
- 訂正の場でも、識別に必須でない限り未定義語を再掲しない。元の記録を pointer で指す。誤った語を後続の ticket / 記録 / 実装本文へコピーしない。
...
### Agent handoff の output hygiene と review generation recovery

test / review automation の subprocess output は検証 evidence であって、その raw
transcript 自体を agent handoff 本文へ複製する必要はない。negative-path test が意図的に
発生させる stderr は test 内で capture して期待内容を assert し、green suite の親
terminal へ漏らさない。application の実運用 stderr や fail-closed 診断を弱めるのでは
なく、test harness 側だけで expected output を閉じ込める。

- handoff / callback / review request には raw test transcript を貼らず、実行 command、
  exit code、test 件数、`OK` / `FAILED`、必要なら durable log pointer だけを載せる。
  詳細 log が必要な場合も、receiver が明示的な pointer から取得できる場所に保持し、
  pane message を log transport にしない。
- expected negative-path stderr は test 自身が capture した値を assert する。単に捨てて
  silent green にせず、診断契約が変われば test が fail する形を保つ。
- content filter 等によって reviewer の response generation が成立しなかった場合、
  同じ turn / prompt へ blind resend しない。fresh generation を開始し、durable な
  Review Request journal と structured marker の exact `target_head` を再読して review を
  再開する。generation failure は Review Gate の verdict ではないため、approval / finding
  を推測で補わず、成功した新 generation が通常どおり durable Review Gate を記録する。

portable な契約は、*expected stderr を test 内で検証しながら親 terminal を clean に保つ /
handoff は結果 summary と durable pointer に限定する / reviewer generation failure は同一
turn へ再送せず durable Review Request から fresh generation で再開する*ことである。
...
記録は **該当条件を名指しし**、何を確認するために何を読むかを含む。通常の再開判断 journal はこの記録を兼ねない — 条件を名指ししない記録、または調査対象を書かない記録は escalation の許可にならない。宣言した条件と実際に依拠する条件が食い違う記録も同様である。

**詳細調査の解禁は 4 step の順序を解除しない。** escalation 下でも、step 3 (判断の記録) より前に step 4 (高レベル操作) を実行しない。解禁されるのは 4 step の *外側* の調査操作であって、4 step 相互の順序ではない。
...
target-lane の Codex が実装 / review request を **自 lane の** Claude implementer へ route するとき — `## Workspace 横断 handoff` と `lane 横断 handoff は target lane の Codex を経由する` が、作業が lane の Claude に到達する唯一の sanctioned な経路として予約する same-lane の `--to claude` hop — その dispatch は **標準 handoff であり、pending input を意図的に残す経路ではない**。ただし「標準 command を実行した」「本文を type した」「Enter を押した」のいずれも成功の代用にはならない。Herdr の成功は 2 形ある: idle/turn-ended 受信では causal turn-start を確認した `submitted_confirmed`、busy 受信では composer clear を証拠とする非 causal な `sent` / `queue_enter` (queued submission、mozyo_bridge #15537。`injection_stage=uncertain_partial` のままで blind retry 禁止は不変)。どちらの証拠も無ければ fail-closed する。pending input のまま operator / coordinator に手で Enter を押させる形も、確認不能な outcome を成功扱いする形も禁止する (Redmine #12207 j#60741)。

1. **まず durable anchor を記録し、その後 standard send を実行する。** `mozyo-bridge handoff send --to claude --source redmine --issue <issue_id> --journal <journal_id> --kind <implementation_request|review_request|...>` を使う。default の `queue-enter` は backend 別の admission / observation を所有する。tmux compatibility backend は従来の queued-submission outcome を返す。Herdr backend は admission 後の本文を高々 1 回に限定し、first Enter を zero-or-one とする。idle/turn-ended 受信は同じ terminal / generation に結び付く causal turn-start を確認した場合に `submitted_confirmed`、busy 受信は wait 非依存の full effect fence を通した Enter と composer clear を証拠に `sent` / `queue_enter` (非 causal queued submission、#15537) を返す。
2. **tmux inactive-split の Claude pane: standard_target_admission が登録済み pane を activate する。** 一般的な cockpit-grid の場合、Codex pane が active split で、Claude pane は inactive な sibling である。Redmine #12597 以降、tmux の default `queue-enter` rail は standard_target_admission を実行する: *登録済み* の inactive Claude pane (live / strong role / `workspace_id` あり / 非曖昧) は `tmux select-pane` で activate される。pane selection のみで raw key injection は決して行わず、activation は durable record に記録する。pane が admit され **ない** 場合、rail は typing 前に `not_sent` で fail-closed する。その structured outcome が示す high-level recovery command は bounded retry できる。`marker_timeout` 等の `uncertain_partial` に同じ retry 規則を流用しない。Herdr target には tmux pane activation 規則を適用せず、Herdr 自身の identity / generation / readiness gate に従う。
3. **`--no-submit` / `--mode pending` は標準の dispatch path ではない。** これは notification を入力し、意図的に input を pending のまま operator の submit に委ねる明示的な operator / debug fallback であり、same-lane Claude へ作業を route する default route では決してない。成功した pending send は `uncertain_partial` なので、同じ本文を再送しない。標準の dispatch で pending を選ぶことは #12207 が修正した regression である: dispatch は配送済みに見えるのに、implementer は turn を決して受け取らない。
4. **dispatch outcome と `injection_stage` を記録する。** `submitted_confirmed` は causal confirmation 済み、`not_sent` は本文も Enter も未送信で bounded retry 可能、`uncertain_partial` は本文または Enter が到達した可能性があり blind retry 禁止である。`not_sent` の blocked record だけに replay 可能な high-level retry command を添える。`uncertain_partial` は retry command を作らず、durable anchor と receiver state を照合する reconcile plan を記録する。黙って pending のまま残された dispatch は、配送された handoff ではない。

submit rail 自体は `references/safety.md` が固定する (`queue-enter` が default、`standard` が strict な明示 fallback、`pending` が operator-submit path。`mozyo_bridge` repo における深い send-safety 契約 doc は `vibes/docs/logics/tmux-send-safety-contract.md`)。本 section は、same-lane dispatch が pending で stall しない標準経路と、成功 / retry / reconcile の判定を固定する。receiver-binding / process / identity / generation gate を緩めず、blind な Enter や本文再入力を導入しない (Redmine #12207 non-goals)。tmux の standard_target_admission は pane selection (`tmux select-pane`) のみを使い、Herdr は自身の causal gate を使う。
...
4. **短く anchor 付きの通知を送る。** `mozyo-bridge handoff send --to codex --target coordinator --mode standard` を使い (`coordinator` が解決できない場合は明示的な `--target <coordinator_codex_%pane> --target-repo auto` に fall back する)、本文に durable anchor — issue id、gate journal id、到達した state、そして関連する場合は commit hash — を入れる。最小の state + pointer にとどめる。callback outcome (sent / blocked-with-reason / not-attempted) と、送信を試した場合の `injection_stage` を durable record に記録する。`not_sent` だけが replay 可能であり、`uncertain_partial` は無言の re-poke や blind resend をせず durable reconcile に進む。callback 欠落は `progress_without_callback` として検出可能なままにする (`## Stall / no-progress 検出標準`)。
...
box 1–4 が未完の state は、作業がどう位置づけられていようと handoff されていない (`## Handoff ライフサイクル` の "every" rule)。`--target coordinator` が失敗した場合、state を回復可能に保つのは box 4 の typed `blocked` outcome であって沈黙ではない。`not_sent` と確認できない pane を無言で re-poke しない。
...
### callback uptake は durable Redmine event を正本とする

sublane → coordinator の uptake (coordinator が「この lane が前進した」と認識して行動を開始すること) は、**durable な Redmine event を正本**とし、pane ACK や process 終了を gate へ昇格しない。callback pane message、`sent` / `ok` の delivery outcome、worker process の終了、assistant turn の完了は、いずれも uptake の trigger ではあっても gate 遷移の根拠ではない (`## ACK / delivery / completion の分離`)。coordinator は callback を受けたら名指しされた journal を読み、そこに載った gate をもって前進する。

自動 uptake (background watcher / supervisor が durable event を coordinator runtime へ供給する形) を実装する場合、次の fail-closed 不変条件を維持する (これらは同一 issue の重複 ingest / 重複 deliver を防ぎ、pending / dead-letter の増幅を止める):

- **authoritative workspace 一意性** — 各 issue の uptake owner は exactly-one の authoritative workspace / lane に解決する。zero owner または ≥2 owner は fail-closed で omit する。host-global な roster を全 registry workspace へ投影して同一 issue を複数 workspace で重複処理しない。
- **issue generation fence** — uptake は current dispatch anchor (当該 lane + generation) に fence し、superseded な過去 generation の event を live として再取り込みしない。判定 authority の粒度 (per-issue anchor) と適用対象集合の粒度 (claim 対象 row 集合) を一致させる。
- **historical replay suppression** — 過去 (pre-existing / recovered) の pending / backlog row を、不可逆な send edge で fence し、fence を迂回して再送しない。fence は ingest だけでなく send・recovery を含む全 edge に置く。
- **review head 連言 (Redmine #13974)** — review-gate producer は central preset の `### Review Generation Marker Contract v2` を読み、canonical renderer / producer 契約に従って marker を付与し、generation identity を散文から組み立てない。field key / 必須 gate 組合せ / missing・drift disposition / source_sequence authority は同 preset 契約の正本にあり、本 reference は複製しない。

これらは durable state (outbox / lease / publication fence) の contract であり、緩めると同一 issue の重複通知・pending 増幅を招く。深い設計正本は採用 repo 側 (`mozyo_bridge` では #13968 callback supervisor workspace partition と `vibes/docs/logics/coordinator-sublane-development-flow.md` の callback sweep watermark contract) が持つ。本節が固定するのは *uptake は durable Redmine event を正本とし、pane ACK / process 終了を gate へ昇格せず、authoritative workspace 一意性・generation fence・historical replay suppression を維持する* ことである。
...
- これは `### 完了チェックリスト (handoff-worthy state を完了扱いする前に実行する)` をチェックリストから定義へと強化する: `callback_result` field (`sent` / `blocked` / `not-attempted`、沈黙は不可) は state を complete にする要素の一部であり、後回しにできる step ではない。送信を試した outcome は `injection_stage` も持つ。`implementation_done` を記録して自分の same-lane Codex にだけ通知した sublane は、`implementation_done` を完了して*いない*。lane 横断の callback outcome journal がまだ負っているままである。
- callback outcome journal は少なくとも `state`、`durable_anchor` (`#<issue_id> j#<gate_journal_id>`)、`callback_result` を載せる。送信を試した場合、`submitted_confirmed` は成功 evidence、`not_sent` は replay command を伴う回復可能な blocked、`uncertain_partial` は blind retry 禁止の reconcile 待ちとして記録する。禁止されるのは、callback journal がまったく無いまま state を done と扱うこと、または uncertain outcome に retry command を添えて done に昇格することである。
...
## Sublane hibernate (プロセス解放) と early hibernate

退役 (retirement) は issue が **closed** の lane を破壊的に片付ける。だが、issue がまだ **open** のまま lane の常駐 process (gateway / worker pane) だけを畳みたい局面がある。**hibernate** はそのための非破壊的な disposition である: issue は open のまま、worktree / branch / 未 push commit / lane metadata / durable callback route はすべて保存し、managed pane だけを閉じる。retire ではない (retire の precondition は issue closed)。idle-timeout kill でもない (bare な idle pane は hibernate signal ではない)。`active -> hibernated` への lifecycle CAS であり、逆遷移 (`hibernated -> active`) は resume が所有する。
...
**hibernate は close ではない。** hibernate を issue close、dogfood 成功、owner close approval のいずれにも読み替えない (`## ACK / delivery / completion の分離` の一般化)。early hibernate が release issue へ委譲するのは dogfood の execution/evidence であり、source issue の close authority と owner close approval は委譲しない。hibernate した lane の issue は依然 open であり、その close は US-level audit → `## Owner 承認の集約` → owner_close_approval という coordinator の通常経路を通る (release issue へは移らない)。
...
現行の自動経路は、pass 全体の外部変更上限を callback delivery / retire / hibernate で共有する。候補生成時と実行直前に workspace・issue・lane・generation・revision・worktree・branch・review・integration・CI・callback debt を再読し、2 回の完全な snapshot が一致し、候補が 1 件だけの場合に限り退役処理を呼ぶ。不明・複数候補・lease loss・snapshot drift・先行 mutation/uncertain は typed zero-mutation である。結果は `retired | already_retired | blocked | deferred | uncertain` と固定 reason で報告し、例外や作用不明は同じ pass の後続 mutation を止める。通常利用ではローカル絶対 path を指定させず、workspace registry と lane lifecycle の記録から解決する。

この自動経路の完了範囲は managed process の解放と lifecycle row の terminal disposition までである。Git には worktree path と lane identity を削除 invocation 自身で不可分に再照合する通常系 primitiveがなく、local branch も expected tip と「他 worktree が保持していない」を単一の非 force 操作で同時に保証できない。そのため成功した退役も physical cleanup は `cleanup_blocked` として、`git worktree remove` と `git branch -d` の operator runbookへ渡す。force と remote branch 削除は禁止する。

### 本標準が緩めない境界

- **自動化は既存の安全契約を再利用する。** supervisor は CLI subprocess や散文解析を使わず、公開 `sublane retire` と同じ preflight / intent rail を typed API 経由で呼ぶ。新しい scheduler、queue、`workflow step` の候補探索責務は追加しない。
- **物理 Git 清掃を退役成功へ読み替えない。** lifecycle が `retired` でも worktree / local branch の削除完了は別事実であり、`cleanup_blocked` の間は operator runbook の負債として残る。
...
### Drain queue projection と process retention

Drain 順序は「何をどの順で処理するか」を定めるが、coordinator が別の判断のために必要とするのは「**いま active process を保持し続ける必要があるか**」という集約 verdict である — 例えば `## Sublane hibernate` の early hibernate 判断で「この lane を hibernate してよいか」を読むときである。本 projection は、上の coordinator-blocking 語彙 (`## 最小の coordinator-blocking state 語彙`) と actionability 軸 (delegable かどうか) を、固定 drain bucket ごとに集約したものである。新しい state machine を作らず、同じ read model を集約するだけである:

- **drain bucket** — `callback / review / owner / integration / close / blocked / retirement / release_dogfood` の固定 8 分類 (Drain 順序 + 委譲済み dogfood bucket)。`release_dogfood` は、TestPyPI / installed dogfood の execution/evidence が専用 release issue へ委譲された lane の terminal bucket であり (`## Sublane hibernate` early hibernate。close authority は委譲せず coordinator に残る)、coordinator-blocking な残務が無い lane に**限り**ここへ入る (委譲済み dogfood が live な review / callback / owner / integration / close / blocker を隠さない)。
- **ownership split** — 各 bucket を `coordinator_actionable` / `delegated_in_flight` / `non_actionable_wait` で数える (`## 最小の coordinator-blocking state 語彙` の actionability)。
- **process_retention verdict** — `coordinator_actionable` な lane が coordinator-blocking bucket (`callback / review / owner / integration / close / blocked`) に 1 つでもあれば `hold` (process を保持)、無ければ `releasable`。`retirement` と `release_dogfood` は holding bucket から**意図的に除外**する: retirement は batch 可能な cleanup cadence、release_dogfood は release issue へ委譲済みであり、いずれも single では feature-lane coordinator の process 常駐を強制しない (これが review-approved + integrated な lane を early hibernate させる不変条件である)。
- 判定は durable record から導出し、pane からは導出しない。`releasable` は「何も残っていない」ではなく「coordinator が今すぐ手を動かすべき blocking drain が無い」であり、retirement / release_dogfood の残務は projection に併記される。
...
- **内部 beta の pre-production publish (例: TestPyPI internal beta) を publication checkpoint に結線しない。** 内部 beta 配布が「先に `origin/main` 昇格 / Redmine Version close を通す」ことを前提にすると、`Version close → origin/main → 内部 beta → 昇格前提の後続 → Version close` の循環になる。内部 beta は exact reviewed integration head を publication checkpoint とは独立に配布してよい (公開履歴を進めない)。公開昇格 checkpoint は `origin/main` promotion にのみ適用し、pre-production 配布 gate には適用しない。この分離を実装する採用 repo の runbook 側 (`mozyo_bridge` では `vibes/docs/logics/release-flow.md` の internal-beta 非循環 gate) が具体手順を持つ。
...
## `coordinator_assistant` の安全使用境界

`coordinator_assistant` は、main / default coordinator unit で coordinator の隣に置かれる**非権威的な補助 actor**の正式な provider-neutral 語彙である。これは model、vendor application、pane 名、起動 command の名前ではない。具体の binding を説明する必要がある場合だけ `coordinator_assistant via claude`、`coordinator_assistant via codex` のように role と provider を分けて書く。既存文書や履歴の `main-unit Claude` / `main lane Claude` / `default-lane Claude` は、現在または過去の Claude binding を表す互換表記であり、actor の canonical name ではない (Redmine #11858 main-unit assistant safety boundary、#11850 multi-lane PoC、#14240 provider-neutral coordinator assistant terminology に由来)。

この抽象化が必要なのは、provider の差し替えを authority の変更にしないためである。provider 名を actor 名にすると、起動 command / model / vendor を変更するたびに権限規約まで書き換えるか、古い brand 名から誤った authority を推測することになる。role contract を `coordinator_assistant` に固定すれば、どの provider を binding しても同じ allowed / forbidden 境界を維持できる。
...
`coordinator_assistant` の出力は **draft / input であって決して evidence ではない**。coordinator は、それを判断に変える前に source file、durable work record、command 出力と突き合わせて確認する。assistant の結果は pane scrollback と同じく、確認すべき pointer であって durable な事実ではない。

### 許可される用途 (安全な coordinator context 節約)

以下は coordinator が `coordinator_assistant` へ渡してよい具体的なタスクである。いずれも権威的な判断を生まない:
...
- 作業が適切に durable record で gate された lane (専用の sublane または worktree) へ移された**後**の通常実装。標準の implement → record → review フローに従う。この時点の actor はもはや `coordinator_assistant` ではなく、境界づけられた lane の `implementation_worker` である。

### 禁止される用途 (coordinator に残すもの)

`coordinator_assistant` は owner 窓口や gate 判断の行動を一切行ってはならない。binding された provider への依頼がどのような言い回しであっても、以下は coordinator に残る:

- owner への質問、または owner close approval の要請 / 収集 / 承認確定 (`## Owner 承認の集約` を参照 — owner 承認は単一の coordinator role に収束し、決して `coordinator_assistant` には収束しない)。
...
assistant provider に渡された依頼はこれを変えない。`## Claude / Codex 役割境界` と同様、命令形の言い回し ("やって", "go ahead") は意図を表すのであって、境界を越える権限を与えるものではない。

### `implementation_worker` との違い

`implementation_worker` と `coordinator_assistant` はどちらも owner 窓口ではないが、権限が異なる:

- **`implementation_worker`** は境界づけられた実装 actor である。自分の lane で diff を生み、implementation_done / review_request を durable record に記録し、検証を実行する。Claude は現在の代表的な provider binding 例であって role 名ではない。
- **`coordinator_assistant`** は固有の実装 lane を持たない。作業が明示的に gate された lane へ移されるまでは要約、抽出、draft、scratch 専用である。その場で実装させると、未 review の編集が audit / owner 窓口の正本のすぐ隣に置かれ、本節が防ぐ authority collapse を起こす。

要するに: `implementation_worker` は自分の lane の gate の下で実装し、`coordinator_assistant` は coordinator を補助する。provider が同じでも、この 2 role は同義ではない。

### 本節が緩めない境界

- **assistant の出力は input であり、evidence ではない。** `coordinator_assistant` が生み出すものは、coordinator が正本と突き合わせて確認し記録するまで durable にはならない。
- **owner 窓口と gate 判断は coordinator に残る。** 本節は coordinator の context を節約するものであって、`Close Approval Separation`、`## Owner 承認の集約`、`## Policy / skill authoring 境界` の各境界を動かさない。
- **operator 固有の方針を OSS 既定に持ち込まない。** どの provider / model / pane / command を `coordinator_assistant` に binding するか、日常的にどの具体タスクを offload するか、private な優先順位付けは operator の runtime policy である (採用 repo の public / private 境界規約を参照。`mozyo_bridge` では `vibes/docs/rules/public-private-boundary.md`)。portable な部分は、*assistant の出力が input-not-evidence であり、安全な補助タスクは引き受けてよいが、owner 窓口・gate 判断・gate 外実装を引き受けてはならないこと*である。
...
- 親 US を持つ `leaf_issue` dispatch は独立した durable owner/operator decision anchor がある場合だけ許可し、task-level review の必要性から leaf dispatch を推定しない。
...
- review_result の journal には v2 marker と同一 journal で `review-finding-manifest` sidecar (#14971) を必ず併記する。承認でも count=0 の明示的空集合 sidecar が必須である。canonical producer `workflow callbacks --emit-gate --gate review_result` が write できない環境 (opt-in unset) では `--render-note-only` の出力 note を 1 journal としてそのまま貼り、sidecar や `set_digest` を自作・省略しない。manifest を欠く review_result は retire / integration fence が finding 母集合を証明できず、lane が恒久 blocked になる (#15699)。承認の render は write と同一の generation admission fence (`--review-generation-json` + `--consumer-id`、identity exact-match、single-consumer lease) を通過して初めて出力される。
...
- worker の完了報告・pane 状態・transport ACK を Review Gate approval や integration 完了と読み替えない。判断の正本は durable record である。
...
### Coordinator-Owned Operational Config Direct Edit (mozyo-bridge product 全体の方針)
