...
The product-level orchestrator does not require Redmine or Asana specifically. Its
provider-neutral contract is `vibes/docs/specs/ticket-system-neutral-orchestrator.md`.
The per-system branches below are adapter-specific intake rules: they preserve each provider's
durable semantics without making that provider part of the core state machine.
...
This entrypoint is reflected minimally into the central scaffold presets (the Asana and Redmine `agent-workflow.md` presets) and the shared `mozyo-bridge-agent` skill workflow reference, so that scaffolded repos reach the behavior through their generated thin routers without copying this full logic doc into runtime surfaces. Keep that reflection minimal and ticket-system-aware: each preset must keep its own semantics distinct (Asana task / comment vocabulary versus Redmine issue / journal gate vocabulary), and neither preset nor the skill reference copies this document verbatim. When this entrypoint changes, update the per-system summaries in those surfaces rather than re-expanding the full logic into them.
