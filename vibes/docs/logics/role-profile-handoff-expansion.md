...
- **後方互換 (additive)**: `RoleProfileResolution.adr_context` は default `None` の追加 field。`to_structured_dict()` は常に `adr_context` key を持ち (未解決時は明示 `None`)、`profile_source` / `profile_version` / `unresolved_placeholders` の既存契約と template 本文は不変である。`profile_version` は template 本文の pointer であり、send 時に解決される ADR context はこれに含めない (`record_contract_text()` が別 heading で追記する)。
