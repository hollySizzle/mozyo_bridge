...
同一 tab 内で 2 pane の位置だけを入れ替える (split 方向は保つ)。

```sh
herdr pane swap <pane-a> <pane-b>
```

- 効果: pane-a と pane-b の表示位置を交換する。assigned name / route は不変。
- 引数 signature は実行時に `herdr pane swap --help` で確認する (2026-07-12 実測は「位置交換のみ」という behavior を記録したもので、完全な argv を固定していない)。
...
```sh
# 0. 現状確認: 対象 pane id と元 tab の tab_id、現在の split 方向を実測
herdr pane list            # ← 出力形式 / --json は `herdr pane list --help` で確認
...
- 検証: 戻した後に layout を実測し、対象 pair の `direction` が `down` になっていることを確認する (`herdr pane list` 等の layout 読み。`direction` field 名・読み command は `--help` で確認)。
...
- 検証: `direction` が `right` になっていることを実測確認する。
