...
`#11850` の cockpit サブレーン PoC では、issue ごとに git worktree を作り、lane / pane / branch / Redmine gate を対応させる運用が有効だった。運用哲学、bandwidth/admission、pipeline fill は [[logic-coordinator-sublane-development-flow]] を正本とする。
...
- N 本並列開発の operator policy (上限・命名規約など)。repo-local dogfood の soft profile は [[logic-coordinator-sublane-development-flow]] に置くが、core CLI default にはしない。
...
- 社内固有の削除条件・並列上限・命名規約は private operating policy 側に置く。mozyo_bridge dogfood の local soft profile は [[logic-coordinator-sublane-development-flow]] に記録してよいが、portable core default ではないと明示する。
