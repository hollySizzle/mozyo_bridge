...
- lane の commit が main / release branch に到達済み、または `git cherry -v <base> <branch>` 等で patch-equivalent と確認できる。metadata-only terminal retire (`sublane retire --retire-hibernated-bound`) では、この patch-equivalent 確認は散文でなく機械照合する: literal ancestor (`git merge-base --is-ancestor`) が成立しない場合、coordinator の durable `patch_equivalent` integration disposition を `--integration-disposition-json` で渡すと、retire が exact journal shape (source head / integration head / commit map / stable patch-id / origin reachability) を action-time 再読し、real git から stable patch-id と origin 到達性を再計算して一致するときだけ terminal CAS を許す (Redmine #14066)。missing / stale / mismatch / unreadable evidence は zero-write。偽の integration branch を渡す回避は成立しない (patch-id は recomputed proof であり claim ではない)。
