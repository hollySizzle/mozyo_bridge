...
**lane tab 細分化 (採用、Redmine #13411)**: 単一 sublane host workspace 内で、
非 default lane ごとに専用 herdr tab を割り当て、gateway + worker を同 tab 内の
split pair として並置する (`herdr tab create --workspace <host> --label <lane key>` +
`agent start --workspace <host> --tab <tab_id> [--split right]`)。これにより
7 lane = 14 loose pane ではなく 7 tab に整理され、owner intent (#13377 j#73654
「親・子・孫 3 ウィンドウ」+ 密度懸念) と整合する (#13380 spike j#73668 で実現可能と
...
tab へ移行する。owner は display knob (#12391 範疇) としていつでも override 可。
...
この live projection への接続 actuator は #12473 で実装した:
`mozyo-bridge handoff delegate-grandchild-stamp` が宣言済み delegation chain を
#12465 projection foundation で検証し (cycle / unknown parent / depth>2 / off-contract
kind を fail-closed)、孫が depth 2 の `implementation` lane に derive することを確認した
うえで、discovery read path が読む `@mozyo_lane_kind` / `@mozyo_delegation_parent` を
live pane へ stamp する (`set-option -p`、preview-by-default / `--apply`)。
`DEPTH` 列の元になる `delegation_depth` と `delegation_root` は parent chain
から derive され pane option には書かない。stamp は本 doc の固定 invariant どおり
display / audit breadcrumb のみで、routing authority を持たず direct cross-lane Claude
