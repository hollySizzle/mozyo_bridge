...
- formatter contract では `issue_subject` / `issue_role` も必須である。実行前に `mozyo-bridge session boundary-prompt --help` を確認し、runtime が `--issue-subject` / `--issue-role` を提供する場合は `--issue <id> --journal <id> --issue-subject <s> --issue-role <r> ...` を使う。runtime が旧 surface で両 flag を持たない場合は、helper 出力を完成形とせず、同じ必須 field を手動で補った prompt を作り、runtime promotion gap を Redmine に記録する。field を黙って省略しない (Redmine #13479 / #13618)。default は pasteable markdown、`--json` は prompt field + `prompt_markdown` + 絶対 `repo_root`。formatter 正本は `session_boundary.build_boundary_prompt`。
...
- lane を記す場合は Git branch/worktree・registered lane metadata・live routable runtime を別 state として区別し、routable でない lane は欠落 layer を名指し (branch-only / lane-unregistered / runtime-unavailable) で fail-closed にする。metadata absent + live runtime presentも `lane-unregistered` とし、runtime stateを別途併記する。`target lane` label を live routable lane と同一視しない (正本 [[spec-session-continuity-user-harness]] `### Routable lane state の区別`)。backend=herdr の `sublane list --lane <label> --json` は live row だけでも非空になり得るため、`lane_record_missing` / `lane_slots_missing` を含む row semantics と live slot 実測を合わせて判定し、`agents targets` empty candidate を「routable lane 不在」の根拠にしない。
...
- agent process のassigned name/live stateと、実際にcommandを実行するshellのsender attestationを別 stateとして記録する。TUIがliveでもcommand shellに`MOZYO_WORKSPACE_ID` / `MOZYO_AGENT_ROLE` / `MOZYO_LANE_ID`が無ければstandard dispatchはblockedであり、手動env注入やraw sendへfallbackしない。認証切れ/agent死とも区別する (正本 [[spec-session-continuity-user-harness]] `### Sender command-shell / auth state`)。
