...
- **cross-lane / cross-session Claude direct boundary を弱めない**。cross-lane は target lane Codex gateway 経由、same-lane Claude direct は既存 guardrail が許す範囲のみ ([[logic-cockpit-sublane-operating-model]]、[[skill-workflow-reference]])。本 doc は pane lifecycle の判断を足すだけで、この routing を変えない。
