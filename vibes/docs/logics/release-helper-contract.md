...
- `release check drift` — `release-flow.md` の `Canonical Renderer / Skill Mirror Drift` を 1 command として再現する。`mozyo-bridge scaffold canonical --check` (router pair + governed workflow pair, Redmine #10345 / #10426)、`scripts/sync_plugin_skill.sh --check` (plugin full mirror, Redmine #10663)、`scripts/sync_legacy_project_skill.sh --check` (legacy project Claude skill partial mirror, Redmine #14580) を順に subprocess として実行し、いずれかが drift を検出したら strict-fail (exit 1) で `result: blocker` を返す。sub-check は互いに独立に実行する (先行 sub-check が blocker でも後続を skip しない)。sync script が存在しない場合も blocker とする — 何も走らなかった gate を clean と報告しないため。helper 自身は worktree を mutate せず、復旧 command (`mozyo-bridge scaffold canonical` / `scripts/sync_plugin_skill.sh` / `scripts/sync_legacy_project_skill.sh`, いずれも `--check` なし) を stdout に verbatim で列挙する。ただし復旧案内は **sub-check ごとに、その drift class を実際に解消できる内容** で出す。plugin mirror は full `rsync -a --delete` なので再実行が全 class を解消するが、legacy partial mirror の pin 外 reference だけは sync が拒否する class であり、reviewed delete または pinned set 更新を案内する (Redmine #14580 review j#90322 F1: 解消できない案内を出すと operator が exit 1 のまま同じ command を回す)。判断 (例: drift を accept して release を進める) は operator に残す。
...
| `Canonical Renderer / Skill Mirror Drift` | `release check drift` | strict-fail 検査として実行 |
