...
- **台帳の carrier は instance dictionary へ直接置く** (`object.__getattribute__(exc, "__dict__")`)。`setattr` も `__context__` 代入も型の `__setattr__` を経由するため、属性代入を拒否する exception 型には**両方同時に無効**になる——`__context__` へ chain する第二 carrier を書いて、まさにその型で失敗することを実測した。**動かない carrier を 2 つ並べるより、動く carrier を 1 つ置く。**
