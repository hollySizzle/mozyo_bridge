...
- **carrier が受け取れなかった occurrence は失わない。** retention は「control-flow を返す」だけでなく **append が完了したかを伝える**。未完了分は unwind 内に queue し、次の retention 機会と teardown 終了時に retry する。pop は append 成功後だけなので retry しても重複しない。carrier interrupt 自体も occurrence として queue する。**note は残るが ledger が空、という状態を作らない**——note は rendering であって authority ではない。carrier が最後まで回復しなければ記録は到達不能になる (foreign binding を上書きしないのと同じ境界)。
