...
- `references/safety.md` 等の project-scope mirror が canonical (`skills/mozyo-bridge-agent/references/safety.md`) からドリフトしないよう、canonical を先に編集し、その content を mirror へ写す (`PluginMarketplaceTest` が plugin mirror のドリフトを検出するのと同じ思想)。この partial mirror の drift は `LegacyProjectSkillMirrorTest` (`tests/integration/e_130_governance_distribution/f_150_skill_plugin_distribution/test_legacy_mirror_tracked_tree.py`, Redmine #13483。Redmine #14666 で move-only 移設) が自動検出する: mirror する `references/{project-map,release,safety,workflow}.md` が canonical と byte 一致であること、partial set が pin されていること (canonical の `redmine-issue-authoring.md` / `subagent-delegation.md` は意図的に非 mirror)、`SKILL.md` は Claude adapter stub として意図的に canonical と非一致であることを固定する。
...
- **`(st_dev, st_ino)` は、その inode が pin されている間だけ identity である** (Redmine #14652)。inode 番号は解放されると次に作られた file へ払い出され得る。実測: `python:3.12-slim` の overlayfs `/tmp` で staging entry を `unlink` → 同名再作成すると **20/20 で番号が再利用**され、差し替えられた別 file が「自分のもの」として比較を通り `os.replace` で pinned reference として install された。tmpfs と APFS は **0/20** で再利用しないため macOS では緑になり、#14651 が Linux の capability 誤判定を直して module 全体の `platform_unsupported` fail-fast が外れるまで露出しなかった。番号を identity に戻すのは **open descriptor** である —— descriptor が在る限り inode は解放されず番号も払い出されない (同じ overlayfs で創出 descriptor を保持したまま計測して **0/20**)。したがって:
  - **staging descriptor は ownership proof を最後に使い終わるまで開いたままにする。** 使用点は swap 直前の検査と `_release_staging` の unlink の 2 つで、close はその後に置く。
  - **pin が無い状態では ownership を答えない。** 比較を「弱い証明」として通すのではなく `unproven` として fail-closed にし、install も unlink もしない。**pin の無い比較は弱い証明ではなく証明ではない。**
  - **close は最後になったので、遅延 write error の報告点を `fsync` へ移す。** 以前は close が swap より前にあったため、close が返す遅延 write error が「install 前に止める」検出器になっていた (j#90467 R9-F1)。現在は staging のうちに `fsync` で報告させ、失敗したら何も install しない。**close の返り値は引き続き violation にする** —— `fsync` 以後に書き込みが無いので backstop だが、黙る backstop は backstop ではない。
- **残る窓 (residual) は 2 か所ある。効果が異なるので個別に記す。**
  - **ownership 検査 → `os.replace`**: identity 検証と rename の間。検証直後に staging name を canonical と同内容の外部 hardlink へ差し替えると sync は exit 0 となり、mirror entry がその inode を指し得る (**foreign inode の install**)。現行 A-F は hardlink を regular file + byte parity として許すため mirror 外 write も mode 変更も起きず、内容が異なる場合や symlink は post-audit が nonzero にする。**descriptor pin はこの窓を閉じない** —— pin が保証するのは「検査の答が信用できる」ことであって、答の後に名前が動かないことではない。
...
**teardown の失敗経路は 3 channel を分けて扱う。** unwind 中の cleanup (staging entry の release / staging descriptor の close) は `owned_descriptors._teardown_during` の 1 経路に集約し、call site ごとに規則を決め直さない (Redmine #14580 review j#90477 R11-F1 / j#90482 R12-F2 / j#90487 R13-F1〜F3 / j#90492 R14-F1・F2)。**2 action の順序は release → close で固定する** —— release は ownership proof を参照し、その proof は close が手放す descriptor に依存するため (上記 #14652)。両者は独立に実行され、片方の失敗がもう片方を skip させないという規則自体は変わらない。
...
- **state machine の入口も durable にする。** commit 境界を直しても、**その authority に入る手前の arrival が local 変数だけ**にあれば全体は lossless にならない。single slot に持つと ordinary exception で捨て、control-flow occurrence で**上書き**し、retry 上限で取りこぼす。正: **未 admit の arrival は list で保持**し、interrupt occurrence は追加であって置換ではない。admission は identity で idempotent にして何度でも再試行できる形にし、**ordinary failure でも retry 上限でも、抜ける前に必ず admit を試みる**。残る不可避点 (queue へ append する命令そのものへの signal) は 1 箇所に閉じ込め、実装が守れる境界だけを書く。**control-flow を catch したことと、handler body が raise しないことは別**。`except` 節の本体は、それを捕まえた `try` の**外側**にある通常のコードなので、そこで発生した nested control-flow は rail を脱出し後続 action を飛ばす。handler body は **全体が guard 内にある helper への call 1 行**にし、occurrence 生成・追加をその中で行う。nested は 1 段だけ「捕まえた側と自分自身」の保持を試みてから握る (surface させると残りの teardown を失うため)。**priority は guard 内の状態に依存させず、引数から return 式で直接決める** —— guard 内で代入していると、その代入前に nested が来ただけで最初の control-flow が return からも台帳からも消える (引数として手元にあるのに回復しない)。**回復できるものを「明示した境界」と呼んで残さない。** **guard より前に 1 行でも置かない** —— 局所変数の初期化のような無害に見える行も、`try` header と同じく protected range の外側であり、そこへの到達は helper を脱出して残りの cleanup を落とす。「occurrence が既に在るか」は **call-time に束縛される private default parameter の sentinel** で判定する (`is _MISSING`)。**guard 前 statement も判定専用の nested region も置かない** —— binding の有無 (`UnboundLocalError`) に問う形は動くが nested region を 1 つ増やすので採らない (棄却理由であって現行規則ではない)。閉じられない残余は主に **region 境界**であり、外側にもう 1 段 guard を巻いても露出する境界が移るだけで消えない。ただし **それは「境界を増やしてよい」理由にはならない** —— 境界が 1 つ増えるたびに脱出面が 1 つ増える。「不可避」を主張する前に、その region が本当に必要か実測で確かめる（`UnboundLocalError` 判定用の nested region は、**call-time に束縛される default parameter の sentinel** に置き換えれば丸ごと消えた）。残余は **role を明示した固定集合として pin し、増えたら test が落ちる**形にする。**syntax label（`try:` で始まる等）で自動承認すると、region を足すたび静かに許容が広がる。** 採用構造で残るのは outer `try` header / その `except` header / inner `try` header / absorbing handler の `except` header と `pass` / `return` の **6 行**で、これを実測して pin する。うち 5 行は region 境界、`pass` は **statement でありながら脱出する**（handler body は必ず何か 1 命令を持ち、それを囲むものが無い）。したがって **「statement は 1 行も脱出しない」という定式化は誤り**であり、6 行の固定集合として書く。**docstring / governed doc / test の 3 者で同じ 6 行・同じ保証外範囲を書く** —— 実測より狭い記述は、本 issue で 2 度 defect を隠した（境界集合を assume した結果）。**離脱時の最終 admit も同じ 2 rail に載せる** —— 「priority はもう決まっている」は、main loop が **ordinary failure で抜けた場合には偽**なので、そこで観測した control-flow も surface させ occurrence として保持する。**「不可避な最後の 1 命令」は、その手前で回復できた分まで捨てる根拠にはならない。**
