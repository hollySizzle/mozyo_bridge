...
- **台帳の carrier は primary の型に一切依存させない。** 実測で棄却した順に: `setattr` と `__context__` 代入は**どちらも型の `__setattr__` を経由**するので属性代入を拒否する型に両方同時に無効 (第二 carrier を書いてまさにその型で失敗することを確認)、`object.__getattribute__(exc, "__dict__")` は **subclass が定義した `__dict__` data descriptor を dispatch する**ので、その property が例外を投げれば retention が消え、control-flow を投げれば rail ごと抜けて未実行の action が生まれる。採用形は `BaseException` 自身の descriptor を bind して instance dictionary を取り (subclass へ dispatch し得ない)、**attribute 構文で生成できない key** (空白入り文字列) に、**実装が生成した module-private 型**だけを exact type で受け入れて置く。既存値は adopt も mutate もしない。identity key (`object()`) は「衝突しない」点で更に強いが、**非文字列 key は instance dict の復元を壊し `pickle.loads` が失敗する**ため実測の上で採らない。**fallback を書いたら、fallback が必要になる当の条件下で実測する。**
