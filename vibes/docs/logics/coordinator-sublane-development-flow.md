...
- **marker の authority 読取は「単一の strict reader」に集約する。判定対象は current declaration のみ**: 共有 marker parser は body を dict へ last-write-wins で畳み、さらに component / key / value の前後 whitespace を正規化する。**表示・routing にはそれでよいが、authority 判定に使ってはならない** — 重複 key は consumer に届く前に消え、`gate = integration_disposition` のような canonical producer が描画し得ない body が綺麗な field に化ける。
  ...
  - authority consumer は **uncollapsed な raw component を読む同一の strict reader** を共有する。対象は **effect (send / actuation / admission) に到達する全 reader**: terminal retire (issuer 解決 / conflict 検査 / strict evidence)、Hibernate authority (basis declaration / review_request selector / park evidence / dogfood receipt)、bound-pair owner approval (convergence / composer-discard)、callback sweep (dispatch anchor / round generation / progress 分類)、recovery action key、recovered worker anchor。**consumer 分類は名前ではなく call-chain が effect に届くかで判定する** — 「watermark 計算」「key 導出」「宛先解決」という名前はいずれも send / actuation に到達していた。**分類そのものを regression test で pin する**。pin は 2 本要る: (1) 共有 lenient fold を呼んでよい module の allowlist、(2) **marker body を自前 parse する module の禁止** — 独自 parser を持つ reader は共有 symbol 名の sweep では原理的に見つからず、実際に dispatch authorization / dispatch disposition が lenient なまま残っていた。より厳格な private parser は例外として理由付きで allowlist する (共有 reader へ寄せると緩むため)。個別 consumer を 1 つずつ潰す方式は本US で 3 round 連続して漏れを出したため採らない。
    ...
    - **`lane` / `lane_generation` は「envelope の field」であって「その renderer の引数」ではない** (Redmine #14717): 同じ field を書く renderer が複数あるなら、**全員が同じ authority を呼ぶ**。実測ではそうなっていなかった — `render_lane_envelope` が #14694 で raw 検証へ硬化した後も、**dispatch marker の renderer だけが `str(value or "").strip()` のまま**残っていた。結果、同じ `lane` が evidence marker では refuse され dispatch marker では黙って trim され、同じ `lane_generation` が envelope では正の `int` を要求され dispatch marker では `0` / `-5` / `1.5` / `True` / `abc` / `٣` を通した。**「片方を直した」は「その field を直した」ではない。**
      - **type は string 化の前に判定する**。`str(value)` を通してから規則を当てると、判定対象は caller の値ではなく **その関数が今作った文字列**になる。`__str__` が綺麗な token を返す object は全規則を通過し、`or ""` は型誤りを「欠損」に化けさせる。
      - **whitespace は raw 値について問う** (leading / trailing を含む)。正本は central `### Hibernate Evidence Marker Contract` の「renderer は parser が拒否するものを書かない ... marker separator (`:` `]` `[` 空白) を含む値は、書き込み時点で producer error として拒否する」であり、**normalize ではなく拒否**である。untrimmed な入力を持つ caller は **自分で・意図して** trim してから「この値がその値だ」と主張する。実装上その位置は argv 境界であり、domain producer ではない。
      - **`lane_generation` は型が 2 つある field である** (argv 由来の `str` と in-repo caller の `int`)。両者は `is_canonical_positive_decimal` / `is_canonical_positive_int` という **1 つの shape を 2 つの型から見た対**で判定し、`require_canonical_generation` がその対を呼ぶ 1 箇所になる。**producer を直して parser を `int()` のままにしない** — `dispatch_generations` はこの規則の consumer 側であり、`lane_generation=٣` を round `3` と読んでいた。
      - **未解決の divergence (要裁定)**: 値 validator が 2 つある。`validate_marker_field_value` は `:` `=` `[` `]` + whitespace を拒否し、`require_marker_token` は `:` `[` `]` + whitespace を拒否して **`=` を許す**。body scanner は component を **最初の `=` で partition** するので `lane=a=b` は round-trip し、central 正本の separator 列挙にも `=` は無い。#14717 は dispatch marker の `lane` を **envelope と同じ `require_marker_token`** に寄せた (同一 field は同一 authority、かつ legitimate な lane id を狭めない) が、**この bullet の上段が `=` を含めて書いている**点との差は残っている。どちらの列挙が正本かは未裁定であり、**片方の producer だけを黙って動かさない**。
