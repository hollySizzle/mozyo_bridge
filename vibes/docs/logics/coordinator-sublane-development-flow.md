...
Version 作成と issue の Version 割り当て tool があるため、現行 Redmine-only 運用では bucket を
Redmine Version / issue `fixed_version` として作成・割り当てる。ここでの bucket は
roadmap bucket、execution bucket、conflict bucket、acceptance / run window を含む横断 grouping
であり、UserStory を bucket layer として増やさない。一方で Version rename / lock / close / delete と
...
- Version は作業単位そのものではない。作業単位は Task / Test / Bug などの leaf issue、
  受け入れ単位は UserStory、実行中の local state は lane / worktree / journal で扱う。
  Version はそれらを横断して「どの bucket に属するか」を見せる軸である。
...
- Version bucket を細かく切ることで roadmap が押し出される場合、押し出した scope を不可視にしない。
  ...
  fixed_version を動かさない状態は bucket 管理として不十分である。
- 将来 Redmine custom field (`execution_bucket` / `lane_set` 相当) や workflow DB が実行 bucket identity
  を持てるようになった場合は、Version を roadmap / milestone 側へ戻し、実行 bucket を custom field 側へ
  分離してよい。ただしその切替は別の規約更新と Redmine journal を伴う。切替前は Version / fixed_version
  を bucket 正本として扱う。
- この custom-field 実行 bucket source は #12922 で読み取り側が実装済みである。`#12919` の
  provider 境界 (`LaneBucketProvider`) に sibling provider
  `RedmineCustomFieldLaneBucketProvider` を追加し、configured custom field の **値** を
  execution bucket として正規化する (bucket id / name は値そのもの。Version status / dates は
  持たない)。`workflow dispatch-plan --bucket-source custom-field`
  (`--custom-field-id` / `--custom-field-name`、任意で `--allowed-bucket`) が opt-in 経路であり、
  fixed_version provider と同じ dispatch-plan 正規化結果を返す。未設定値は `no_execution_bucket`、
  multi-value は `ambiguous_source`、allow-list 外は `disallowed_value` として fail-closed する。
  既定の `--bucket-source` は `fixed-version` のままであり、本 command は project の source of truth を
  切り替えない。custom field を正本に昇格する切替は依然として上記の規約更新 + journal を要する。
  Redmine schema / field の作成・管理はこの実装の範囲外である。
