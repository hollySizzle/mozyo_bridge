...
- ★**hard carve-out は marker の自己申告では晴れない**。`scope=no_change_investigation` は必要条件にすぎず、release / tag / publish / package distribution、production verification・外部副作用、credential / auth / permission / billing、destructive / migration を示す **recognized durable fact** が 1 件でもあれば拒否する。判定は **structured surface のみ**で行い、prose keyword scan を authority にしない。`direct_owner` provenance はこの carve-out の escape ではない。guardrail / preset / router / skill / scaffold は repository change そのものなので前段の zero-change で既に落ちる。
  ...
  - ★★**writer を authenticate できない記録系では、latest-wins は「訂正」と「上書き」を区別できない**。実測: 先行 `carve_out_check: release` の後に別 note で `none` を置くと clear になった。この workspace は全 role が単一 source-system account で書く (ruling #14219 j#86718) ため、**`_KIND_ISSUER` に token を足しても writer 検証にはならない** (`hibernate_issuer_policy` 自身が policy binding であって authentication ではないと明記)。→ **stated reason が record のどこかに 1 件でもあれば恒久的に拒否**する (append による解除を認めない)。正当な再判定は **新しい lane generation** で表現され、それは waiver の envelope を無効化する。**actor 認証が解決不能である事実は doc と docstring に明記し、layered defense が代替であることを開示する** (不足なら writer authority の追加裁定を要する)。
