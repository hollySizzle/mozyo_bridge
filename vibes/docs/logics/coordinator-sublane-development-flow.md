...
- ★★★**安全上の重みは head coverage に置く**。admit の根拠は「**後継の approved Review Gate が、この lane の exact head を検証している**」ことである。後継の `review_result` marker が持つ head を読み、shared grammar が Marker Contract v2 で `review_request` の head と一致することを既に検証しており、それが declaration head と literal 一致し、live probe が同 head を lane の実 head として要求する。**この三者一致は、本 route が読むどの marker からも捏造できない**。correlation marker 2 本は pointer integrity の check として残すが、authority ではない。
...
- ★**writer authority の gap は閉じていない、と書く**。finding 1 への対応で変わったのは gap が閉じたことではなく、**admission がその gap に依存しなくなった**ことである。`hibernate_issuer_policy` は author 引数を取らない policy binding のままで、本 gate は `_KIND_ISSUER` / `_KIND_RULING` に登録しない。
