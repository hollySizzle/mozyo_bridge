...
- ★★★**authority は「mozyo command 境界で記録された coordinator decision」に置く** (#15166 design direction j#102092)。「この audit failure」と「その後継の acceptance」を結ぶ binding は **coordinator の判断**であり、journal から導出できないことは 3 round の実測で確定した。置き場の正本は [[logic-managed-state-model]] の `state_kinds` である:
  ...
  - したがって decision は `mozyo-bridge sublane audit-failure-terminal record` が mozyo-owned store へ書き、retire がそれを読む。★**Redmine journal をいくら書いても decision record は生成できない** — これが 3 つの refuted 案に無かった性質であり、主張はここまでである。**人間の authenticate はしない** (gap は不変)。
