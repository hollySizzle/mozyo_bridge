...
- **artifact には「row を運ぶもの」と「authority を運ぶもの」の 2 種類がある (R18-F1)**: inventory を「row を保持し得る file」(DB / `-journal` / `-wal` / `-shm`) だけで完成させると、**seal 本体やその temp のような authority-bearing artifact を数え落とす**。`operational` と書かれた seal temp は「この store は grant し得た」という **durable な証拠**であり、これを inventory 外に置くと pair loss が fresh install に見える。規律: **lifecycle authority artifact も lock 下で `lexists` により再読**し、**main seal absent + operational temp + store 喪失 = unknown-owner loss として fail-closed**。ただし **current build の `initializing` temp のみ**が残る中断は、その build の `reserve()` が operational を強制する以上 grant され得ないため **resume を許す** (安全側で product を brick させない)。**曖昧 (unknown / unreadable / 複数) な temp は absent に畳み込まず fail-closed**。
