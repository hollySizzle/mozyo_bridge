...
- **相関する 5 つの record**: (1) `review_finding_verdict` gate — 中央 preset が既に義務化している governed 記録を**読むだけ**で、新しい authority を起こさない。最新の verdict gate が declaration の名指す round を対象とし、全 finding が literal `accepted` であること (未記入 template 行は `accepted` ではないので拒否)。
  ...
  - 被覆を検査するには round の finding 集合が要るが、**canonical な finding identity grammar は存在しない**: preset の `## Gate: review` template は per-finding id を持たず、実 record は `#### F1` (#14577 j#93648) / `### Finding 1` (#14755 j#99057) / `### finding_1` (verdict j#93656) と三様である。ここで「F1 ≡ finding_1」の正規化を起こすのは **finding identity の第二定義**であり採らない。
  - 採った形: **round 自身が、round 自身の journal で、自分が挙げた finding 集合を enumerate する** (`superseded_failure_round_findings`)。enumeration は finding を挙げた authority (reviewer) の側に置かれ、過少申告で得をする declaration writer の側には無い。verdict の finding 集合とこの集合の **exact set equality** を要求し、各 finding が literal `accepted` であることを要求する。verdict 側は finding 宣言行と `verdict:` 行が **1 対 1 に対応**しない記録 (未回答の finding / 名指しの無い verdict / 同一 id の重複) を部分読みせず丸ごと拒否する。
  - これは **central review gate の一般契約を変更しない**。本 opt-in route だけが読む additive marker であり (`superseded_failure_successor` と同型)、enumeration を持たない review は本 route を使えないだけで既存 consumer は不変。**writer の authentication は依然行っていない** (ruling #14219 j#86718)。establish できるのは「verdict が、verdict journal 自身とは別の record が述べた finding 集合に答えている」ことだけである。(2) **後継 issue 側の `superseded_failure_successor` acknowledgement** — 元 issue の record だけでは任意の approved issue を後継と名乗れるため、**名指された後継自身の record が本 issue と本 round を名指し返す**ことを要求する双方向束縛。(3) 後継の Review が `approved` かつ後継自身が close 済み (= successor 未完の拒否)。(4) `## Gate: close` family の close 記録 (= open issue の拒否)。(5) live 側。
