...
- **2 つの authority を分ける —— attempt は lease、send は fence**: 順序は `lease acquire → decision read → boundary re-read → record → final live read → fence reserve → send → mark_delivered → lease release`。
...
- **acquire は own ではない**: lease に TTL がある以上、**死んでいない、ただ遅いだけの owner** が期限切れ後も生存し reclaim される分岐が必然的に生じる。よって **durable publication の直前と send の直前で、authority へ owner token を再照合**する。ownership を失っていれば **zero-publication / zero-send**。「死んだ owner は送っていない」という論拠は **send にしか効かない** —— publication は lease だけが守っている。
