...
- ★**現状: 本 route は admit しない**（typed refusal。上記 writer authority 未解決）。**refusal は Close 前から見える**: 抑止は `owner_close_approval` / `close` の **close family 全体**に効かせ、gate 集合を列挙して将来の追加漏れが test で落ちるようにする（#14695 review j#93856 F1 = 片方の分岐にだけ置いて「Close 前に見える」と comment に書いた欠陥）。
