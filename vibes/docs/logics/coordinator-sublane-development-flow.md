...
- **既存 fence を緩めない**: 通常の review generation fence と #14539 exemption は不変で、本 route は同一 fence への**3 本目の independent route**として admit 側にだけ効く。measured route を渡した場合 `--latest-generation-admissible` へ fall back しない。waiver より新しい review round / pending callback / post-waiver head mutation / foreign・stale な issue・lane・generation は zero-close / zero-retire。
