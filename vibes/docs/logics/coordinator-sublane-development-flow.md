...
- ★★★**以下は #15195 着地後に admit する契約であり、現在何かを守っている防御として読まない** (#15166 design direction j#102092)。「この audit failure」と「その後継の acceptance」を結ぶ binding は **coordinator の判断**であり、journal から導出できないことは 3 round の実測で確定した。置き場の正本は [[logic-managed-state-model]] の `state_kinds` である:
  ...
  - ★★**authority-bearing store の書込み境界を宣言 scope 外へ逸らさない** (#15166 review j#102147 finding 2 / j#102840 findings r7f1-r7f3、実測)。store は SQLite DB ではなく JSON document + nonce sidecar + lock artifact である。filesystem root descriptor から各 home component を `dir_fd` + `O_DIRECTORY|O_NOFOLLOW` で walk/createし、home FD取得後はpath文字列を再解決しない。JSON / nonce / lockの全操作はhome FD相対で行う。lockはregular single-link fileだけを許し、sidecar初期化を含むread-modify-write全体でexclusive lock、`read` / `is_initialized`ではconsistent snapshotを得るshared lockを保持する。home componentのmissing / symlink / unsafe type、およびlockのmissing / symlink / hardlink / unsafe type / identity drift / acquisition failureはtyped refusalでfail-closedにし、read pathはhomeやlockを作成しない。これは #15195 のreceipt authority gapを解決しないため、同issueが着地するまでterminal routeの恒久typed refusalは維持する。
