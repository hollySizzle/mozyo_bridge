...
- **1 つの reservation が attempt 全体を所有する**: 順序は **reserve → boundary read → record → final live read → send** とし、abort は `release`。通知は **record の journal** を指す (原 dispatch ではない)。
  - **reserve を先に取る理由**: fence の `BEGIN IMMEDIATE` row が **唯一の実 serialization authority** である (Redmine には無い)。これを attempt 全体に渡って保持することで **record publication が atomic** になる。record を reservation の外で書くと、並行 sweep が両方とも空の pre-read を通過して **duplicate record を publish し、以後 ambiguity (`>=2`) で recovery が恒久喪失**する。
  - **abort は `release` であり `cancelled` ではない**: `reserve` は `cancelled` を **terminal never-send** として扱うため、transient failure に使うとその anchor を恒久封鎖する。`release` は **まだ `reserved` の行のみ削除**し「この attempt は無かった」を表現する (outcome が記録済みの行は決して消さない)。
  - **retry と atomicity は両立させる**: 「reserve を後ろへ動かして retry を救う」と atomicity が壊れ、「reserve を前に出して cancel で戻す」と retry が壊れる。**reserve 前倒し + release** が両立解である。
- **CAS が無い store では窓は 0 にならない。最小化し、それ以上を主張しない**: record の journal id `R` は **記録時点の position** であって **将来の write と CAS しない**。したがって `R` invariant は action-time authority の代替にならない。**final live read を reserve 後・send 直前**に置いて窓を read→send に最小化する。**「これが上限」と主張しないこと** —— この主張は実測で否定された (verify 後・reserve 中に着地した gate が replay された)。
- **すべての durable read は観測点である**: idempotency 目的で足した read も含め、**そこで見た事実が判断に反映されないなら、それは新しい窓**である。recorder の pre-write read が gate を見ているのに再分類しないと、**false な stall record を書いてから correction を足す** ことになり acceptance 3 (first-pass、事後訂正に頼らない) に反する。矛盾を見たら **書かずに abort** する。
- **送信可否は「progress が在るか」だけで決まる**: 前後関係 (`record_stale_at_write`) は「log に残った record が真だったか」を分けるためのものであり、生きている lane を poke しない判断そのものは順序に依らない。
