...
- **strict evidence の envelope identity は retire 自身の引数へ束縛する**: lane-enveloped evidence を要求しながら `workspace` / `lane` / `lane_generation` / `integration_branch` を捨てるなら identity fence として成立しない。期待値は retire の引数 (`--evidence-workspace` / `--evidence-lane` / `--evidence-lane-generation` / `--integration-branch`) から取り、**observation file からは取らない** (同じ untrusted document が自分の binding を証明することになるため)。期待値が欠けている場合は当該検査を飛ばさず fail-closed にする。`--lane-label` は envelope の `lane` に流用しない — lane registry は `lane_id` と `lane_label` を区別し envelope は前者を持つため、同一視は fence ではなく category error になる。
- **矛盾する integration disposition 宣言は authority path で fail-closed**: 同一 journal が `merge` と `explicit_deferral` を併記する record は一意に解釈不能であり、記載順で admission が反転してはならない。判定は governed な structured surface を**全て**列挙する: `disposition:` field 行、workflow-event marker field、heading 自身の inline `: <value>` 形。いずれか 1 つでも単数読み (`search`) を残すと、その surface が迂回口になる。表示 projection の歴史的 leniency は parser / consumer を分離して保持し、authority consumer 側が事前に strict な問いを立てる。canonical 化して同値になる重複 (`merged` と `merge`) は 1 宣言として畳む。
