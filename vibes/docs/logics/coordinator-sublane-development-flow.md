...
- **readable subset を authority にしない**: same-gate の unreadable sibling が同一 note にあれば、その gate の読取は **note 全体を fail-closed** にする。readable な兄弟だけ返すと clean + forged 併記の note が clean な note と同一に読める。別 gate の unreadable marker は当該 gate の関知外 (過剰 fail-closed を避ける)。
