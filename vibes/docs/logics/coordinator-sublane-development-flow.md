...
product-level orchestrator の provider-neutral な authority / event / recovery contract は
`vibes/docs/specs/ticket-system-neutral-orchestrator.md` を正本とする。本書はその contract を
Redmine adapterで運用する `mozyo_bridge` repository 固有spineであり、Redmineを製品必須provider
として定義しない。
