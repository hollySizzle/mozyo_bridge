...
- **workspace identity の attestation**: fence key は workspace で partition される。blank / 誤った id は **別 row を reserve する**ため、同一 recovery が二度送られる。id は durable anchor から **測定**し、明示指定は override でなく **一致検査**とする。測っていない partition は fence ではない。
- **durable record が先、pointer は後**: stall 分類と再通知は durable event である (`skills/mozyo-bridge-agent/references/workflow.md` `### 検出と再通知を durable journal として記録する`)。よって **reserve → record → send** の順で、通知は **その record の journal** を指す (原 dispatch ではない)。record が書けない / 解決できない場合は send を **cancel** し、fence key を解放する。理由が durable に残らない送信は「無言の re-poke」であり禁止。
