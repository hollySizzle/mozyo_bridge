...
- **`origin/main` が新規 lane の base、review 承認後の integration target、通常開発の唯一の canonical branch である** (Redmine #14761 owner intent)。UserStory close 後の自律 push 先は `origin/main` であり、`.mozyo-bridge/config.yaml` の `sublane_integration.integration_branch` は `main` である。config と本節が同じ答えを返すことが、この採用宣言の検証可能な形である。
...
- **staged topology の checkpoint 規範は本 repo に適用されない。** Redmine Version close は `origin/main` 昇格 checkpoint ではなく、`push_waiting` 相当の待機 state も存在しない。統合済みか未統合かは `### Lane State Classes` の `integration_waiting` と integration disposition journal で表現する。`origin/main` を守るのは per-Version checkpoint ではなく、UserStory ごとの Review Gate approval、owner close approval と Close Approval Separation、`## Completion Semantics` の 2 層 push authority (#13026) — 実装者は issue / lane branch のみ push し、integration branch を前進させるのは review 承認後の coordinator — である。
