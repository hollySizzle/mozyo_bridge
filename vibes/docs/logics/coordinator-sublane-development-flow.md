...
- **strict evidence の envelope identity は「実測した retire 対象」へ束縛する**: lane-enveloped evidence を要求しながら `workspace` / `lane` / `lane_generation` / `integration_branch` を捨てるなら identity fence として成立しない。かつ**期待値の出所が caller 側にあってはならない** — observation file 由来は「同じ untrusted document が自分の binding を証明する」ことになり、専用 CLI flag 由来でも caller が evidence と同じ値を指すだけで一致してしまう (どちらも自由変数)。期待値は **retire 対象 lane の lifecycle row から実測**する (workspace / lane / generation)。target を解決できない場合は当該検査を飛ばさず fail-closed。`--integration-branch` は retire policy の実入力なので argv 比較でよい。
- **automated evidence として Hibernate contract を再利用するなら issuer 条項も引き継ぐ**: central `### Hibernate Evidence Marker Contract` は `integration_disposition` の writer を coordinator に固定する。envelope だけ検証して issuer を落とすと、contract の一部だけを借りたことになる。role は **note の gate 構造から policy として**解決し (marker 本文の自己申告や caller assertion では代替しない)、committed config blob を basis anchor とする。basis を確立できない場合、および 1 note が複数の authority gate を名乗る場合は unresolved として fail-closed。
- **矛盾する integration disposition 宣言は authority path で fail-closed。判定対象は current declaration のみ**: 同一 journal が `merge` と `explicit_deferral` を併記する record は一意に解釈不能であり、記載順で admission が反転してはならない。判定は governed な structured surface を**全て**列挙する: `disposition:` field 行、workflow-event marker field、heading 自身の inline `: <value>` 形。**さらに各 surface の内部**も検査する — 共有 marker parser は body を dict へ last-write-wins で畳むため、`disposition=a:disposition=b` は consumer に届く時点で重複が消えている。surface を数えるだけでは「各 surface の中身は単数」という未検証の前提が残る。一方、この conflict 判定を**履歴全体へ適用してはならない**: strict evidence / lenient disposition / journal id と同じ current declaration 1 件へ束縛する。issue-global にすると、valid な新 declaration が supersede した後も古い malformed record が永久に block し、latest-wins に反する。表示 projection の歴史的 leniency は parser / consumer を分離して保持する。canonical 化して同値になる重複 (`merged` と `merge`) は 1 宣言として畳む。
