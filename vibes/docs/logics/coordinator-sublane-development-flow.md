...
- **open review round は combined heading でも close/owner_close の progression を抑止する**: glance grammar の F10 不変条件 (open review round が conflicting `close` / `owner_close` を抑える) は無条件の contract だが、structured marker を持つ journal にしか適用されていなかった。heading だけの `## Gate: Review Request + Close` は `close` へ縮約されて `retire_ready` に落ち、**lane が自分の open review round を飛び越えて retire する**。判定は heading 経路にも適用する。`review_request` は常に open、`review` は `結論` が承認**でない**場合 (要修正・pending の双方) に open とする — pending を open に数えるのは、結論を確定できなかった review の fail-closed な読みだからである。承認済み review + Close の combined は正当な governed 組合せなので抑止しない。
...
- **terminal retire は strict integration evidence を読む (lenient fold は authority ではない)**: `glance_integration_disposition` は表示用 projection であり、(1) どの commit についての disposition かを持たず、(2) 同一 journal 内の矛盾する `disposition` 宣言を記載順で解決する。自動 retire の evidence には lane-enveloped な strict grammar (`hibernate_evidence_integration`、#14219 T2b) を使い、その `head` (reviewed source head。`integration_head` とは別値) を covered commit と literal 一致させる。legacy な lane-unbound `## Integration disposition` note は glance では今後も有効だが、terminal retire を自動 admit させる根拠には**ならない**。journal ordering は identity の代替にならない。
- **矛盾する integration disposition 宣言は authority path で fail-closed**: 同一 journal が `merge` と `explicit_deferral` を併記する record は一意に解釈不能であり、記載順で admission が反転してはならない。表示 projection の歴史的 leniency は parser / consumer を分離して保持し、authority consumer 側が事前に strict な問いを立てる。canonical 化して同値になる重複 (`merged` と `merge`) は 1 宣言として畳む。
