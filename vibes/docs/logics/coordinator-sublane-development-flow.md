...
#### no-change direct-owner Review waiver (#14695)

#14539 の exemption は「作業はあったが auditor が実装主体へ昇任した」場合を扱う。**repository 変更が 0 の investigation** はその契約外で、review generation が存在しないため標準 `sublane retire` が `stale_review_generation` で恒久停止する (#14613 j#93256 の waiver + Close に対し j#93262 で再現)。裁定は #14695 j#93412 (同一 consultation の先行 Answer j#93406 を supersede)。

- **authority token は `no_change_review_waiver`**。`codex_direct_edit` と同じく **issue 全体・latest-wins の authority fact** であり lifecycle gate ではない (heading allowlist に入れない)。canonical writer role は `coordinator`、ruling pointer は `redmine:#14695:j#93412`。**`approval_source=direct_owner` は provenance 軸であり writer 軸ではない** — 両者は連言で、worker / gateway の自己 waiver は成立しない。
- **canonical structured marker だけが mint する**。field 集合・順序・値域を閉じ、`gate` / `version` / `approval_source` / `decision` / `scope` / `issue` + envelope (`workspace` / `lane` / `lane_generation` / `head`) を exact に要求する。lane correlation は既存 `hibernate_evidence_envelope` を再利用し、第二の envelope grammar を作らない。heading は**宣言 (shadow)** には足りるが mint しない。同一 note の same-gate な数えられない marker は note を fail-closed にする (#14539 と同一規則)。
- ★**zero-change は「record 側」と「live 側」の連言でのみ証明する。git 単独では証明できない** — commit を一度も産まなかった lane と、産んだ commit が既に integration branch へ merge 済みの lane は、どちらも `<integration>..<branch>` が 0 件になる。
  - **record 側**: issue **全履歴**に commit field / `changed_paths` / change-bearing gate / integration disposition が 1 件でもあれば不成立。merge 済み lane を排除するのはこの半分である (統合済みの作業は commit record と integration disposition を必ず残す)。**conflicting / malformed な change candidate を「無い」と読み飛ばさず invalid にする**。
  - **live 側**: live branch head が marker `head` と literal full-SHA 一致 (= post-waiver mutation 検出)、integration branch に対する lane 固有 commit が 0、lane worktree が clean。integration branch が後に進んでも lane head がその ancestor であることは許容し、current integration head との literal 一致は要求しない。測定不能は「変更なし」と推定せず typed refusal。
  - ★**live 測定は全 probe を lane checkout 自身で行い、checkout の branch identity を先に確定する** (#14695 review j#93576 F2)。head / ahead を `--branch` から **repo root** で解決し clean だけを `--worktree` で測ると、両者が同一 checkout である保証が無く、**`--branch` を integration branch に向けるだけで foreign head + 0 ahead + clean という「無変更」読みが只で得られる**(実測: actual HEAD `156b384f` に対し measured head `735a5f88`)。head は checkout の `HEAD`、ahead は `<integration>..HEAD`、clean も同 checkout で測る。worktree 未指定時に repo root へ fallback しない (fallback がこの decorrelation そのもの)。
  - ★**branch identity は fully-qualified ref で照合する。`--abbrev-ref` の literal 一致では detached を弾けない** (#14695 review j#93638 F2)。detached checkout で `rev-parse --abbrev-ref HEAD` は literal **`HEAD`** を出力し、`--branch` は任意文字列を受けるため、**caller が `--branch HEAD` と名付けるだけで detached tree が「無変更 lane」として測定に通る**(実測: `head=<full SHA>, commits_ahead=0, worktree_clean=True`)。「detached は branch 名と一致しない」という前提は *caller が `HEAD` と言えない* ときにしか成立しない。よって `rev-parse --symbolic-full-name HEAD` が **`refs/heads/<branch>` と exact 一致**することを要求する (detached ではこれも `HEAD` を返すため構造的に弾ける)。`--branch HEAD` 自体も明示拒否する (`refs/heads/HEAD` と名乗る抜けを塞ぐ)。
- ★**negative claim は input の完全性に依存する**。positive な authority (gate が存在する) は journal の部分集合では捏造できないが、**negative claim は「落とす」だけで成立してしまう** — commit を宣言した journal を 1 件外せば record は「変更を宣言していない」ことになる。したがって本 route は #14539 と違い **caller 供給の observation file を受け取らず、credential-gated Redmine read で issue 全履歴を live に読む** (#14066 の patch-equivalent retire と同じ理由)。
- ★**hard carve-out は marker の自己申告では晴れない**。`scope=no_change_investigation` は必要条件にすぎず、release / tag / publish / package distribution、production verification・外部副作用、credential / auth / permission / billing、destructive / migration を示す **recognized durable fact** が 1 件でもあれば拒否する。判定は **structured surface のみ**で行い、prose keyword scan を authority にしない。`direct_owner` provenance はこの carve-out の escape ではない。guardrail / preset / router / skill / scaffold は repository change そのものなので前段の zero-change で既に落ちる。
  - ★**detection は heading と marker の双方を読む。marker だけでは何も検出できない** (#14695 review j#93576 F1)。これらの gate には **marker producer が存在しない** — `render_workflow_event_marker` は callback-bearing な `GATE_BEARING_KINDS` (`blocked` / `implementation_done` / `owner_close_approval_waiting` / `review_request` / `review_result`) 以外を `ValueError` で拒否する。したがって marker 専用 detector は**この repo が出力し得ない形**を探していることになり、実際には `## Gate: production_verification` を持つ record が `clear=True` で `ok` まで到達した。carve-out が実際に書かれる形は **heading** である。combined heading は `+` / `/` / `,` で分割して全 part を検査する (refusal trigger なので over-detection が fail-closed 側)。
  - ★**token vocabulary を自作しない**。R1 は独自語彙を起こしたため何も検出しなかった。語彙は **central preset `### Owner Close Approval Delegation` の carve-out 列挙を正本**とし、drift test で正本本文と束縛する (preset が carve-out を足したら hole ではなく赤い test として現れる)。
  - ★★**resolution は canonical field を読む。代理指標も caller flag も不可** (#14695 review j#93638 F1)。この判定には **正本 field が既に存在する** — central preset の `## Gate: owner_close_approval` template が定める **`carve_out_check: none | <該当理由>`** である。`none` → clear、非 `none` の非空値 → declared (拒否)、不在・空・自己矛盾 → unresolved (拒否)。未記入の template 行 (`none | <該当理由>` のまま) は literal `none` ではないので declared 側へ落ちる (記入されていない template は何も判定していない)。field は `owner_close_approval` gate を **structurally 宣言している journal** からのみ読み (qualify してから読む)、latest-wins・supersede-by-EXISTING を適用する。
    - 失敗の系譜を残す: R1 は caller の `gates_resolved` flag (呼出側が常に真)、R2 は issue の **`work_unit` 宣言**を代理にした。後者の正本用途は `leaf_issue` / `user_story` による **Review authority routing** であって impact classification ではなく、実測で **`carve_out_check: production_verification` を明記した record が `clear=True` で admit** した。**2 回とも「自分が判定すると称している当のものを読んでいない」という同一の形**である。
  - ★★**determination は lane envelope へ束ね、単独の field 読みにしない** (#14695 review j#93704 F1)。waiver marker 側は envelope (`workspace` / `lane` / `lane_generation` / `head`) を retire 対象の lifecycle row と exact 照合し issuer policy も通すのに、**determination だけ素の field 読み**だと issue 上の任意 journal が最新値で上書きできる。同一 authority 内で強度が非対称になる。→ canonical marker に closed field `carve_out=none` を必須化し (schema `version` を bump)、**同時に** governed `carve_out_check` field が同結論であることを要求する **連言**にする。marker 単独を証明にしない (j#93412 §3) を保ちつつ、書き換えに **exact envelope + exact head の偽造**を要求できる。
  - ★★**writer を authenticate できない記録系では、latest-wins は「訂正」と「上書き」を区別できない**。実測: 先行 `carve_out_check: release` の後に別 note で `none` を置くと clear になった。→ **stated reason が record のどこかに 1 件でもあれば恒久的に拒否**する (append による解除を認めない)。
    - ★**この恒久拒否には訂正経路が無い** (#14695 review j#93776 F2)。fold は issue 全履歴を読み **generation を一切参照しない**ため、新 lane generation へ進んでも旧 journal の reason は同じ snapshot に残り先に拒否する。**「新 generation で再判定できる」と書いてはならない** (R4 でそう書いて実装と矛盾した)。generation 単位の再判定を可能にするなら determination 自体に generation authority を持たせた**別契約**が要る (未実装の設計課題)。
  - ★★★**自己申告を何個連言しても writer authority にはならない** (#14695 review j#93776 F1)。R4 は marker の `carve_out=none` と governed `carve_out_check: none` の連言で writer を縛ったつもりだったが、**両者は同一の非認証 actor が同一 issue へ書ける**。lane worker は自 lane の `workspace` / `lane` / `lane_generation` / `head` を当然知っているので、**envelope 一致は「偽造」ではなく単なる記入**であり writer を識別しない。author も receipt も持たない well-formed record が `ok` で admit した (実測)。
    - この記録系は **全 role が単一 source-system account** で書き (ruling #14219 j#86718)、`hibernate_issuer_policy` 自身が policy binding であって authentication ではない (author 引数を取らない) と明記する。したがって **`_KIND_ISSUER` に token を足しても、自己申告 field を増やしても、writer 検証にはならない**。
    - **採った方針は typed refusal** (issue Acceptance が明示的に許容する「未対応なら Close 前に typed refusal する」)。route を **構造的に admit 不能**とし、`WRITER_AUTHORITY_RESOLVABLE` という**単一 flag** に集約する。fold 群 (marker 検証 / zero-change / carve-out / live 測定) は全て live のまま test も維持し、refusal は **最後の conjunct** に置く (他の欠陥は各々の真の理由で診断され続ける)。glance の `review_waived` も同じ flag で倒す (**片方だけ生かすと glance と retire が再び食い違う**)。
    - actual coordinator action に検証可能に束縛された writer / receipt authority の設計は **coordinator 所有の未解決課題**。
  - ★**field も canonical quote-aware 行から読む。宣言だけ quote-aware にしない** (#14695 review j#93704 F2)。gate qualification を `canonical_note_lines` 経由にしながら **field だけ raw note 全体へ regex** を掛けていたため、**fenced code block 内にしか無い `- carve_out_check: none` が determination を解決し、最終 admission が `ok` になった**。同一関数内で「宣言は quote-aware・値は raw」という非対称を作らない。引用された contract 例示や過去記録の転記が authority になる。
  - ★**vocabulary の drift test は双方向に張る**。実装 token → preset (各 token が preset のどの語句に対応するかを宣言し照合) と、**preset → 実装** (carve-out 列挙の各項目が detection token か、名指しした別 conjunct のどちらかで説明されること) の両方。前者だけでは**取りこぼしを検出できない** — 実際 `外部副作用` は token を持たず `## Gate: external_effect` が admit していた。preset が carve-out を足したら後者が赤くなる。preset 側が日本語で書く項目 (`data 削除` / `外部副作用`) があるため、token を機械的に空白置換して照合する素朴な方法は成立しない。
- **projection の一致**: glance / Close / terminal retire は同一 fold を共有し、`review_conclusion` を `approved` に捏造しない。derived fact (`review_waived` = valid + 未 supersede + zero-change) を **fold 側で 1 つ**作り、consumer 側で条件を組み直さない。ただし retire へは **supersession 半分 (`review_waiver_unsuperseded`) と zero-change を別々に渡す** — 畳んだ boolean を渡すと change-bearing な record が「新しい review round に supersede された」という**偽の理由**で拒否され、存在しない review を operator に探させる。typed refusal は各々が真であって初めて価値がある。
- ★**現状: 本 route は admit しない**（typed refusal。上記 writer authority 未解決）。**refusal は Close 前から見える**: 抑止は `owner_close_approval` / `close` の **close family 全体**に効かせ、gate 集合を列挙する（#14695 review j#93856 F1 = 片方の分岐にだけ置いて「Close 前に見える」と comment に書いた欠陥）。
  - ★**drift test の母集合は「守りたい集合」ではなく独立正本から取る**（#14695 review j#93879 F3）。`_CLOSE_FAMILY_GATES` を assert して**同じ集合を loop する** test は、gate が classifier に増えて family 集合へ入れ忘れられても green のままで、comment / ImplDone が約束した「追加漏れ検出」を実現していなかった。母集合は **`GATE_KINDS`**（新 gate が必然的に現れる場所）から取り、全 gate の期待 state を表として固定する。併せて「**waiver が実際に変える gate 集合 == close family**」も測り、抑止が効かなくなっても表が偶然通ることを防ぐ。
- ★★★**未解決 review は close family の integration 判定より先に解決する**（#14695 review j#94005 F1）。integration precedence を先に評価すると、**commit-bearing lane では未 review の作業が `integration_waiting` へ流れ**、fence は zero-change fixture でしか効かない。「integration は review approval の後」という規約どおり、**review を先に問う**。
- ★★★**保持するのは bool ではなく最新 round の typed outcome**（#14695 review j#94005 F2）。`unresolved` の 1 bit へ潰すと **`changes_requested`（= `implementing`、non-blocking）/ pending（= `review_waiting`）/ blocker（= `blocked`）の区別が消える**。実測で、要修正 lane が coordinator-blocking へ変わって **pipeline 全体を止め**、明示 blocker が audit 待ちへ**弱まった**。**live round に対する既存規則をそのまま再適用**する（第二の mapping 表を作らない）ことで、live と post-Close の読みが乖離しない。blocker は conclusion 文字列ではなく **round 自身の flag** に載っている（conclusion は `pending` のまま）ので、そこから取る。
- ★★**pending / unresolved review は Close 後も zero-close・zero-retire**（#14695 review j#93879 F1 + 同 design ruling）。issue Acceptance が「pending callback/review では zero-close・zero-retire」と明記しており、**Close journal が後から来ても review の解決にはならない**。よって「最新 review round が未解決か」を **Close を跨いで保持する fact** として持ち、close family より**優先**する。これは waiver 固有ではない **generic correction** であり、「通常 Review generation fence を維持」の範囲内（裁定）。**no-waiver control にも同じ欠陥があることは、明示 Acceptance を無効にしない** — control 一致だけを pin して禁止された終端を「正しい結果」として固定してはならない。
- ★**declaration の ordering に validity を混ぜない**（#14695 review j#93879 F2）。「**valid** waiver が in force か」と「**valid/invalid を問わず** declaration が current か」は別 fact である。前者を後者に流用すると、**current だが malformed な waiver が「無い」と読まれて Close へ進む**（terminal route は拒否するのに glance だけ通る）。supersede-by-EXISTING は declaration 側に適用する。
- ★**unsupported 判定にも review-round supersession を効かせる**（#14695 review j#93856 F2）。「waiver が record に存在したか」だけで判定すると、**新しい approved Review が supersede した後も古い waiver が lane を永久に block** し、glance が通常 Review generation route を汚染する。基点は *存在* ではなく **unsuperseded**。supersede 後は **waiver 無し control と同一 projection** へ戻ることを test で固定する。以下の記述は **proposed / 未有効**の契約であり、「現在何かを守っている防御」として読んではならない。
- **既存 fence を緩めない**: 通常の review generation fence と #14539 exemption は不変で、本 route は同一 fence への independent route として（有効化されれば）admit 側にだけ効く。measured route を渡した場合 `--latest-generation-admissible` へ fall back しない。waiver より新しい review round / pending callback / post-waiver head mutation / foreign・stale な issue・lane・generation は zero-close / zero-retire。

#### superseded failure round の terminal retire (#14755)

#14539 の exemption は「作業はあったが auditor が実装主体へ昇任した」場合、#14695 の waiver は「repository 変更が 0 だった」場合を扱う。**review generation は存在し、しかも失敗した**場合はどちらの契約外で、標準 `sublane retire` が `stale_review_generation` で恒久停止する。再現は #14577: Review j#93648 が `changes_requested`、finding は j#93653 / j#93656 で全て accepted、到達できなかった acceptance は後継 #14697 の Review j#93727 `approved` で取得、lane は j#93757 で superseded failure として task_close 済み、head は `origin/main-next` の ancestor で固有 commit 0 — それでも preflight は j#93759 / j#94006 / j#94319 の 3 回とも fail-closed / zero mutation。

- **失敗 round は失敗のまま終端する**。authority token は `superseded_failure` で、`codex_direct_edit` / `no_change_review_waiver` と同じく **issue 全体・latest-wins の authority fact** であり lifecycle gate ではない (heading allowlist に入れない)。★**この route は最新 round が `changes_requested` であることを要求する** — approved round は通常 fence の担当であり、本 route を「もう 1 つの合格経路」にしない。`--latest-generation-admissible` の虚偽 assert も、後継 approval の流用も行わない。
- **declaration は pointer であって代替ではない**。marker は「どの round か」「どの verdict gate か」「どの後継か」を**名指すだけ**で、名指された事実は record 側から改めて読み直す。これが #14695 との構造的な差である — waiver は「owner が review 不要と判断した」という事実が **marker の中にしか存在しなかった**ため、authenticate 不能な writer が authority のすべてになり typed refusal を選んだ。本 route の事実は declaration の外に独立して存在する。
- **相関する 5 つの record**: (1) `review_finding_verdict` gate — 中央 preset が既に義務化している governed 記録を**読むだけ**で、新しい authority を起こさない。最新の verdict gate が declaration の名指す round を対象とし、全 finding が literal `accepted` であること (未記入 template 行は `accepted` ではないので拒否)。
  - ★★**「存在する verdict が全て accepted」は被覆ではない** (#14755 review j#99057 finding_1、実測)。2 finding の round に対して 1 件だけ `accepted` を書いた verdict journal がこの条件を満たし、**terminal admission が開いた**。Acceptance は「未受領 finding」を独立に拒否と定めており、live-zero は repository change を bound するがこの未完を補わない。
  - **母集合は本 route が宣言しない**。R2 は `superseded_failure_round_findings` として「round 自身が round 自身の journal で enumerate する」marker を置いたが、review j#99065 が実測で差し戻した: enumeration と finding を **同一 actor が同一時点に同一 record へ書く**ため **矛盾しうる第二の record が存在せず**、renderer の production caller も 0 だったので、手書きの過少列挙が R1 と同じ terminal admission を一段上で開いた (review prose 2 件 + marker `findings=1` + accepted 1 件 → `admissible=True`)。★**新しい correlate を足したら「この主張と矛盾しうる record が、別の actor / 別の時点に存在するか」を先に問う**。
  - 採った形: **#14971 の canonical authority (`resolve_review_finding_authority`) を consume する**。母集合は (a) canonical review producer が review prose と**同一 structured input から原子的に**生成する `review-finding-manifest` sidecar、または (b) pre-contract round については、既存 journal を書き換えずに置く append-only `review-finding-attestation` を、**別 journal の direct-owner `review-finding-ruling` が exact に名指した**場合のみ、のいずれか。attestation 単独は authority 0、相反は fail-closed、supersession は新しい ruling が直前 ruling と別 attestation を明示的に名指した場合のみ (#14971 j#99084)。本 route はこの契約を**再定義しない**。
  - verdict 側は finding 宣言行と `verdict:` 行が **1 対 1 に対応**しない記録 (未回答の finding / 名指しの無い verdict / 同一 id の重複) を部分読みせず丸ごと拒否し、authority の finding 集合との **exact set equality** を coverage → 値判定の順で要求する。★**owner ruling 付きの過少 attestation も coverage check で落ちる** — ruling は「その集合が母集合である」ことの authority であって、verdict がそれを被覆したことの authority ではない。
  - `ruling_issuers` port は consumer の責務として `review_finding_legacy_issuer` が埋める。**これは authentication ではない** (ruling #14219 j#86718)。establish するのは「ruling journal が、#14971 j#99084 が writer contract を定めた**唯一の gate だけ**を canonical gate structure で宣言している」= ruling が別目的の journal へ紛れ込めないことだけであり、route が最終的に依拠するのは direct-owner ruling そのものである。gate は gate-bearing kind ではなく workflow-event marker を render できないため、`review_finding_verdict` と同じく **heading が正本 surface**である。
  - これは **central review gate の一般契約を変更しない**: manifest sidecar は generic recognized-channel 集合に入らず、既存 `review_result` consumer の projection は不変 (#14971 の compatibility disposition)。(2) **後継 issue 側の `superseded_failure_successor` acknowledgement** — 元 issue の record だけでは任意の approved issue を後継と名乗れるため、**名指された後継自身の record が本 issue と本 round を名指し返す**ことを要求する双方向束縛。(3) 後継の Review が `approved` かつ後継自身が close 済み (= successor 未完の拒否)。(4) `## Gate: close` family の close 記録 (= open issue の拒否)。(5) live 側。
- ★**live-zero が本 route の consequence を bound する**。admit できるのは **integration branch に対する固有 commit 0 + worktree clean + declaration head と literal 一致**の lane だけなので、admit しても **統合は起きず、approval も生まれず、未 review の作業を持つ lane は終端に到達しない**。未統合 / dirty の拒否はこの理由で必須条件である。live 測定は #14695 と同一の `measure_lane_change` を使い、全 probe を lane checkout 自身で行い branch identity を fully-qualified ref で先に確定する。
  - ★★**live 測定の対象 branch が declaration の lane であることも要求する**。`measure_lane_change` は「checkout がその branch 上にあること」までは構造的に保証するが、**どの branch を測るかは caller の選択**である。再現 (#14577) のように **lane head が既に integration head と一致している場合、その head に載る任意の checkout (coordinator の primary repo を含む) が live conjunct をすべて満たす** — head 不動の主張が lane についての主張でなくなる。よって `--branch` が declaration envelope の `lane` と一致することを要求する (本 workspace の lane worktree は branch 名 = lane label であることを実測)。
  - ★★**その live-zero を測る integration branch は caller が選べてはならない**。「integration branch に対する固有 commit 0」は `--integration-branch` に対して測るので、**caller が lane 自身の branch (あるいは lane head に載る任意の branch) を指すだけで conjunct が空虚に真になる** — route 全体を bound しているはずの条件が自己充足する。よって期待値は **committed config の `sublane_integration.integration_branch`** から解決し、`--integration-branch` と declaration の `integration_branch` の**三者一致**を要求する。config が branch を宣言していなければ期待値が無いので拒否する (推測しない)。これは lane identity を lifecycle row から測る #14539 j#91797 F2 と同じ原則の適用であり、**「caller が選べる値は何も fence しない」**。
    - ★**ただし過大主張しない**: これが塞ぐのは **argv の自由** (偶発的・便宜的な誤指定) であって、lane checkout に write できる相手ではない — config が何を宣言していても、その名前の ref を lane head に作れば同じことになる。durable record から読める何物もここは閉じられない (#14695 j#93776 F1 が否定した「偽造が必要」型の過大主張を繰り返さない)。
    - **committed 名の exact 一致だけを受け、`<remote>/<name>` は受けない**。どの remote を正当と見なすかは本 issue に裁定が無い解決規則だからである。結果として **committed 名を解決できない checkout では ahead が測定不能となり拒否**する (実測: #14577 lane worktree は `main-next` を持たず `origin/main-next` のみ → `commits_ahead=None`)。これは authority の欠落ではなく **運用前提** (lane checkout へ当該 branch を fetch する) であり、coordinator 所有である。
- ★**writer authority は本 route でも未解決であり、解決したとは書かない**。この記録系は全 role が単一 source-system account (ruling #14219 j#86718) で、`hibernate_issuer_policy` は author 引数を取らない policy binding である。したがって本 gate は **`_KIND_ISSUER` / `_KIND_RULING` に登録しない** — gate を登録するにはその gate を**名指した ruling** が要り、writer contract を決めていない record を anchor に仕立てるのは #14661 j#92715 が指摘した欠陥そのもの (`is_anchored` は通るのに契約を述べていない record を指す) だからである。**「相関を増やせば authentication になる」とは書かない** (#14695 j#93776 F1)。代わりに上記 live-zero が consequence を bound するという**前提**を明示し、その前提の妥当性自体を裁定に出す。
- **tie は strict 側**: declaration と同一 journal に review round が併記された record は「この round は終端した」と「review round がある」を同時に主張しており順序で解けないため、round が立つ (= 拒否) と読む。これは `codex_direct_edit` 側の tie policy であり、waiver 側の純粋 ordering とは問いが違うので **call site で明示**する (#14695 j#94260 が逆向きで terminal unblock を生んだ教訓)。
- **glance projection は変えない**。closed かつ最新 round が `changes_requested` の lane は現在 `implementing` に落ちるが、これは **declaration の有無に依らない既存の一般挙動**である (対照実験で実測)。本 route は retire 専用 authority であり glance consumer を持たないため、`review_waiver_unsupported` のような投影を足さない。ここを変えると closed changes_requested lane 全体を本 issue の権限で巻き込むことになる (#14695 R5-F1 と同じ対照実験の結論)。GateFacts へ足したのは round journal id の**素の export** 1 本だけで、derived な ordering fact は増やしていない。
- **既存 fence を緩めない**: 通常の review generation fence、#14539 exemption、#14695 waiver はいずれも不変で、本 route は同一 fence への independent route として admit 側にだけ効く。measured route を渡した場合 `--latest-generation-admissible` へ fall back しない。declaration より新しい / 同一 journal の review round、未 accepted finding、未 acknowledge・未完の後継、foreign な issue・lane・generation・integration branch、post-declaration head mutation、dirty worktree、固有 commit ありは zero-write で拒否する。

#### 正式 Review Gate を持たない audit failure lane の terminal retire (#15166)

#14755 は「review round が存在し、しかも `changes_requested` で失敗した」場合を扱う。**review round が 1 件も存在しない**場合はその契約外であり、標準 `sublane retire` が `stale_review_generation` で恒久停止する。再現は #15164: worker trace は `## Gate: implementation_done` j#101783、round 1 の verdict は `## Independent audit — round 1` j#101792 で、同 journal 自身が `formal_review_gate: review_request journalが無いため本journalを``## Gate: review``として記録しない` と明記している。到達できなかった acceptance は後継 #15165 の `## Gate: review` j#101810 `承認` で取得、両 issue とも task_close 済み・close 済み、lane は commit を 1 件も作っていない — それでも preflight は j#101825 で fail-closed / zero mutation。

- **既存 3 route が届かない理由は実測で確定している** (推測ではない)。#15164 の実 journal を `fold_issue_gate_facts` に通すと `review_round_journals=()` / `latest_gate=close` (j#101822) / `zero_change.proven=True` / `commit_bearing=False`。したがって (a) 通常 fence は読むべき review generation が無く、(b) #14755 は `latest_round_gate == review` かつ `conclusion == changes_requested` を必須条件とするため `superseded_failure_names_another_review_round` で永久に拒否し、`review_finding_verdict` gate も存在しないので coverage も満たせず、(c) #14695 waiver は `waiver_writer_authority_unresolvable` で構造的に常に拒否する。★**既存 route を広げない**: #14755 を広げることは「review round が無い record を review round として読む」ことであり、その route の中核前提を壊す。独立した第 4 の route を足す。
- **authority token は `superseded_audit_failure`**、後継側 acknowledgement は `superseded_audit_failure_successor`。`codex_direct_edit` / `no_change_review_waiver` / `superseded_failure` と同じく **issue 全体・latest-wins の authority fact** であり lifecycle gate ではない (heading allowlist に入れない)。★**surface ごとに token を分ける**: 失敗した Review Gate について書かれた終端と、gate ですらなかった audit 記録について書かれた終端は別の問いに答えるので、一方の record が他方の契約を満たしてはならない。field 集合も異なる (`review_journal` / `verdict_journal` ではなく `audit_journal` 1 本)。
- ★★★**「別 issue に書かれた acknowledgement」は独立確認ではない** (#15166 review j#101880 finding 1、実測)。R1 は安全上の重みを「source declaration と successor acknowledgement が互いを名指し合うこと」に置いたが、**両者とも authenticate できず、その相関は 2 つの marker の中にしか存在しない**ため、同一の非認証 actor が 2 issue へ置けば成立する。これは #14695 が恒久的に拒否している形そのものであり、#14755 が admissible である理由 (「事実が declaration の外に独立して存在し、declaration はそれを相関するだけ」) の反対側に落ちている。R1 はその一文を引用した上で反対側に落ちた。★**「別 record に書かれている」は「別 authority である」を意味しない。同一 writer が両方を書けるなら、連言は 1 つの自己申告である。**
- ★★★**head coverage も authority にはならない** (#15166 review j#101909 finding 1、実測)。R2 は安全上の重みを「後継の approved Review Gate が本 lane の exact head を検証している」ことへ移したが、**zero-change lane では head 一致がほぼ無償**である: commit を 1 件も作っていない lane は integration head の上に座るため、その base に載る**無関係な approved issue はすべて同じ head を共有する**。再現では後継 review の本文を無関係な作業に置き換えても admit した。head coverage が bound するのは *どの repository state か* であって、*誰の作業が review されたか* ではない。
- ★★★**enumeration も authority にはならない** (#15166 review j#102074 finding 1 / scope decision j#102081)。R3 は許可 tuple を code に enumerate したが、(a) その enumeration を decision API の keyword 引数として公開しており caller が差し替えられた (実測)、(b) より根本的に、issue id を package に固定する方式は**同型 lane が現れるたびに製品変更を要求する**ため supported rail ではなく個別移行である。★**authority を decision API の引数にしない**。authority-bearing な API が代替 authority を受け取る契約は、caller 1 つを source-inspection しても閉じない。
- ★★★**本 route は現在 admit しない (typed refusal)** (#15166 design consultation j#102184 への coordinator 裁定)。decision の **writer が本当に coordinator runtime だったか**を establish できる primitive が本 workspace に無く、5 round の各案が実測で refute された: 相互 acknowledgement (j#101880) / head coverage (j#101909) / in-package enumeration (j#102074・scope j#102081) / mozyo-owned decision store (j#102147、そして j#102181 の隔離 temp repo 再現 — writer attestation が読むのは caller 自身が作れる anchor・config・env だけである)。`herdr_identity_attestation` は自身の契約で permission verdict へ昇格できず、`HERDR_PANE_ID` は「まだ authority ではない」と code が明記するため、live-inventory conjunct は**再現経路は塞ぐが要求は閉じない** (Option A は不採用)。**#15195 が Herdr runtime 発行の action-bound receipt を供給するまで恒久 typed refusal**を保持する (#15195 blocks #15166)。#14695 の `WRITER_AUTHORITY_RESOLVABLE` と同一の形であり、flag 1 本で表す — 他の fold / fence / probe は live なまま維持するので、#15195 着地時に入るのは authority check であって再実装ではない。★**refusal は最後に置く**: record 自身の欠陥はそれぞれの reason で先に診断し、`coordinator_receipt_authority_unresolvable` は「record は正しいが authority が無い」を意味する (operator の行き先が違う)。
- ★★★**以下は #15195 着地後に admit する契約であり、現在何かを守っている防御として読まない** (#15166 design direction j#102092)。「この audit failure」と「その後継の acceptance」を結ぶ binding は **coordinator の判断**であり、journal から導出できないことは 3 round の実測で確定した。置き場の正本は [[logic-managed-state-model]] の `state_kinds` である:
  - `desired_state` (「mozyo が command 境界で作成/採用/mark/rename しようとした構成・意図」) の authority は **mozyo-owned persisted state** である。
  - `side_effect_permission` は「**persisted desired state + durable workflow gate + action-time live preflight を照合した結果**」と定義されている。本 route の連言はこの定義そのものである。
  - したがって decision は `mozyo-bridge sublane audit-failure-terminal record` が mozyo-owned store へ書き、retire がそれを読む。★**Redmine journal をいくら書いても decision record は生成できない** — これが 3 つの refuted 案に無かった性質である。**人間の authenticate はしない** (gap は不変)。
  - ★★★**「どこに保存したか」は「誰が書いたか」ではない** (#15166 review j#102147 finding 1、実測)。R4 は store が mozyo-owned であることを coordinator decision の根拠としたが、actor identity を持たない `argparse.Namespace` からの書込みが成功した。保存先の分類は writer の証明ではない。★**writer 境界は writer の内側に置く**: store の `record()` 自身が #13613 と同一の action-time sender attestation (`evaluate_dispatch_sender`) を実行し、process env を **workspace anchor / committed coordinator provider binding / coordinator default lane** と照合する。command 側だけに check を置くと caller 1 つを記述するだけで、store 直呼びを塞げない。env の presence だけでは attest にならない (wrong-but-nonempty は落ちる)。
  - ★★**authority-bearing store の書込み境界を宣言 scope 外へ逸らさない** (#15166 review j#102147 finding 2 / j#102840 findings r7f1-r7f3 / j#103084 finding r8f1、実測)。store は SQLite DB ではなく JSON document + nonce sidecar + lock artifact である。filesystem root descriptor から各 home component を `dir_fd` + `O_DIRECTORY|O_NOFOLLOW` で walk/createし、home FD取得後はpath文字列を再解決しない。JSON / nonce / lockの全操作はhome FD相対で行う。lockはcurrent euid所有・mode `0600`のregular single-link fileだけを許し、random tokenとopened lockのdevice/inode/ctimeを一つのgenerationとしてJSONへ永続束縛する (old tokenをrecreated inodeへ複製しても同一generationにはならない)。nonceもbytesだけでなくsidecarのdevice/inode/ctimeをJSONへ束縛し、同内容を別inodeへ複製したbootstrap/store identityを拒否する。fresh storeはexclusive nonce claimをlock pathより先にdurable化し、そのoperationが作成したlockだけを初期generationとして採用するため、artifact-freeなpreexisting/recreated lockをbootstrapとして使わない。sidecar初期化を含むread-modify-write全体でexclusive lock、`read` / `is_initialized`ではconsistent snapshotを得るshared lockを保持し、snapshot前後とpublication直前直後に同じvisible generationを再確認する。publication rollback intentはrename前に登録し、rename後canonical inodeをstaged inodeと照合するため、kernel rename成功後のwrapper/signal例外やstage pathname ABAでpublicationを見失わない。snapshotしたJSON inodeはloadからretentionまで明示的に束縛し、差替えを新しい旧authorityとして採用しない。既存JSONはpublication前に旧inodeそのものをhome FD相対のdurable hardlinkとして保持し、rollback時はその保持inodeをwrite/truncateせずatomic renameで戻す。そのためrollback中のcrash/write/fsync failureでも、以前に成功したdecisionをpartial/empty JSONで消さない。canonical targetが自分のpublished inodeと一致しない、またはexact rollbackを完了できない場合はforeign targetとretained old inodeを上書きせず保持し、visible lockをatomic poison generationへ差し替える。poison自体が失敗しても、durable rollback linkは作成operationだけが許されるmarkerであり、別operationの `read` / `record` は未解決markerをtyped refusalするためfailed publicationをauthorityとして採用しない。durable canonicalのexact readback後に最後のrollback marker nameをunlinkした地点をirreversible commit pointとし、その後のcleanup dir-fsync failureはcommit済みdecisionをfailed writeと誤報しない。reboot後にmarkerが復活した場合は別operationとして拒否する。portable POSIXにinode-CAS rename/unlinkは無いため、最終照合後のfresh artifact除去・rollback renameの競合保証はgeneration-bound supported writer間であり、同一euidのlock非準拠hostile mutationまでatomic CASしたとは主張しない。成功を返したdecisionが別generationのsupported writerに消されることを許さない。JSONまたはnonceが一度存在した後のmissing lockはstore lossでありwriterも再作成しない。home componentのmissing / symlink / unsafe type、およびlockのmissing / symlink / hardlink / unsafe owner/mode / unsafe type / identity drift / acquisition failureはtyped refusalでfail-closedにし、read pathはhomeやlockを作成しない。これは #15195 のreceipt authority gapを解決しないため、同issueが着地するまでterminal routeの恒久typed refusalは維持する。
  - r8f1 publication marker / commit-point 補足: staged JSONとは別のpublication-intent nameをwriterがoriginal snapshotを開始する前にdurable化し、exact readbackまで作成operationだけが許すmarkerとして保持する。したがってsnapshot drift、retention前のforeign差替え、fresh stage ABA、poison全失敗、process stopが重なっても、次のread/writeは残ったintent markerを拒否する。durable exact readback後の最後のrollback marker unlinkをcommit pointとし、その後のdirectory fsync / descriptor close / wrapper・signal例外はcommit済みdecisionをrollbackせず、failed writeとも誤分類しない。
  - decision は workspace / lane / lane_generation / lane_revision / source issue / audit journal / successor issue / successor review journal / head / integration branch へ exact に束縛し、retire は全項目を**独立 source から再測定**して drift を拒否する。
  - record command 自身も **argv を authority にしない**: workspace / generation / revision は lifecycle row、head は lane checkout の `rev-parse`、integration branch は committed config から測る。operator の判断は Redmine 4 identity (source issue / audit journal / successor issue / successor review journal) だけである。
- ★★**単回実行は lifecycle revision で表す**。decision は決定時点の `revision` に束縛され、lane row を変更する retire は既存 CAS でその revision を進める。したがって 1 decision が authorize できる mutation は最大 1 回であり、第二の consumption ledger を作らない (それは lifecycle generation と食い違いうる)。
- ★★**tracker の現在 status を action-time に fresh read する** (#15166 review j#101880 finding 2、実測)。`## Gate: close` は lane 自身の *信念* であり、tracker が *事実* である。Redmine の status-only reopen は `## Gate:` note を伴わないため journal fold から構造的に不可視で、source 側は caller の `--issue-closed` assertion しか無く、**successor 側は current status の入力が存在しなかった** (過去の Close gate だけで完了扱い)。source / successor 双方の `status.is_closed` を各々 1 回の fresh issue-detail GET で読み、応答が exact issue を identify することを要求する。tri-state で扱い、**「open」と「読めない」を別 reason に分ける** (前者は再オープン、後者は再取得と、operator の行き先が違う)。
- ★★**本 route は「audit が失敗結論だった」ことを証明しない、と明記する**。結論は prose であり governed surface ではない。ここに prose 文法を起こすのは #14755 review j#99065 の欠陥 (自分自身しか証人がいない主張) を一段上で繰り返すことになる。review j#101880 は帰結を実測した (audit note を単なる進捗メモへ置換しても admit した) が、**答えは prose を parse することではなく、admission が prose に依存しない形へ移すこと**である。`audit_journal` は operator 向けの pointer であり、head coverage が既に担っている以上の admission weight を持たない。残る連言は次を establish する:
  1. **この issue のどこにも approval が存在しない** — record の review round が 0 件。stale・流用・誤読される approval がそもそも無く、本 route が「通常 fence をすり抜ける第二経路」になり得ない。**round が 1 件でもあれば拒否**し、通常 fence または #14755 へ差し戻す (未回答の `review_request` 単独も round として数える — 未応答は「review が owed」であり「review が無かった」の対極である)。
  2. **名指された audit 記録が実在し、lifecycle gate ではない** — issue 自身の履歴に当該 journal id が一意に存在し、`fold_issue_gate_facts` を当該 journal 単体に適用して gate を宣言しないこと。gate でない record はいかなる consumer からも approval として投影され得ない。これが「source の失敗記録」と「successor の approved Review Gate」を**別事実として保持**する fence である。
  3. **後継が本 issue と本 audit 記録を名指し返す** — pointer integrity の check として要求する。authority ではない (上記 finding 1)。
  4. **consequence が bound される** — record が zero change を宣言し、かつ live lane が integration branch に対して固有 commit 0 + worktree clean + declaration head と literal 一致。head coverage と併せて「検証済みの state であり、他の state は存在しない」が成立する。
- ★**本 route は「この issue が review に合格した」とは主張しない**。head coverage は *どの commit state が検証されたか* についての主張であって、*誰の acceptance が満たされたか* についての主張ではない。lane が admit されるのは terminal + zero-change によってであり、後継の verdict を移送することによってではない。
- ★**zero-change は live-zero の代わりではなく併置する**。「ahead 0」は *統合済みの作業* の見た目でもある (#14695 が自ら述べた境界)。review を経ずに統合された作業が本終端へ到達してはならないので、`fold_zero_change_record` の verdict を独立の連言として要求する。live 測定は #14695 / #14755 と同一の `measure_lane_change` を使い、`--branch` が declaration envelope の `lane` と一致することも要求する (再現のように lane head が integration head と一致する場合、その head に載る任意の checkout が live conjunct を満たすため)。integration branch の三者一致 (committed config = `--integration-branch` = declaration) と、それが塞ぐのは argv の自由であって lane checkout への write ではないという過大主張の禁止は #14755 と同一で、複製せず同じ規律を適用する。
- ★**re-open 検出は review round の順序ではなく lifecycle gate 全体で問う**。「declaration と同一以降に**認識された lifecycle gate が 1 つも無い**」を要求する。これは review round の ordering 問題を包含し (round は gate である)、さらに declaration 後に別種の gate を記録して作業を再開した lane も捕捉する。tie は strict 側 — 同一 journal に終端宣言と lifecycle gate を併記した record は「永久に終端した」と「lifecycle event がある」を同時に主張しており順序で解けない (#14695 j#94260 の教訓)。運用上の帰結として **declaration は Close より後の、独立した append-only journal に書く**。これは既存 journal を書き換えないという #15166 の non-goal とも一致する。
- ★**writer authority は本 route でも未解決であり、解決したとは書かない**。#14755 と同じ理由で本 gate も `_KIND_ISSUER` / `_KIND_RULING` に登録しない。establish するのは上記 4 連言だけである。
- **glance projection は変えない**。本 route は retire 専用 authority であり glance consumer を持たない。declaration の有無で lane の classified state が動かないことを対照実験で固定する。
- ★**writer authority の gap は閉じていない、と書く**。decision record が行うのは gap を閉じることではなく、**authority を journal surface から `desired_state` 層へ移す**ことである。`hibernate_issuer_policy` は author 引数を取らない policy binding のままで、本 gate は `_KIND_ISSUER` / `_KIND_RULING` に登録しない。人間の authenticate は依然として不可能であり、そう書く。
- ★**decision の置き場を config にしない**。`SublaneIntegrationConfig` の正本が「policy intent であって authority ではない」と明記し、authority 形の key を schema が拒否する。config が供給してよいのは caller が選べない期待値 (`integration_branch`) までであって、admit する権限ではない。decision は `desired_state` であり config policy ではない。
- **既存 fence を緩めない**: 通常の review generation fence、#14539 exemption、#14695 waiver、#14755 terminal はいずれも不変で、本 route は同一 fence への independent route として admit 側にだけ効く。measured route を渡した場合 `--latest-generation-admissible` へ fall back しない。coordinator decision 不在、decision の drift、decision の revision 陳腐化、decision store の紛失・置換・読取不能、review round のある record、open issue、tracker が open を報告する source / successor、tracker status が読めない場合、declaration 以降の lifecycle gate、不在 / gate 形 / declaration より新しい audit journal、change 宣言のある record、未 acknowledge・未完の後継、後継 approval が decided head を検証していない場合、foreign な issue・lane・generation・integration branch、post-declaration head mutation、dirty worktree、固有 commit ありは zero-write で拒否する。
- **検討して採らなかった代替 (R3 時点の記録)**: 「exact identity を committed config へ enumerate して一般化を止める」案は採れない。`SublaneIntegrationConfig` の正本が **「これは policy intent であって authority ではない」「owner-approval / close / callback / durable-anchor invariant をここで無効化する key は意図的に存在しない」** と明記しており、`SUBLANE_INTEGRATION_KEYS` は閉じた 4 key 集合で、authority 形の key は `_reject_boundary_and_unknown_keys` が拒否する。terminal の allowlist を config に置くことは、その宣言済み境界を跨ぐ。config が供給してよいのは **caller が選べない期待値** (`integration_branch`) までであり、admit する権限ではない。
...
本 flow は publication checkpoint doctrine を採用する。branch topology の宣言義務、staged topology における integration (統合) と publication (公開) の 2 層分離・Redmine Version close = `origin/main` 昇格 checkpoint・無応答分岐の `push_waiting` 記録・readiness checklist・手動 close 手順、single-canonical-branch topology における統合の自律性と据え置かれる gate 群、両 topology 共通の release gate 分離の詳細規範は、#13170 / #14761 により配布側 skill `references/workflow.md` の `## Publication checkpoint (integration 層と publication 層の分離)` にある。本 spine は再掲しない (pointer 化)。doctrine 導入の出所は owner_intent (Redmine #13126 j#71777 確定事項 5) / Codex triage (#13126 j#71786 item 4)、topology 選択の出所は owner_intent (Redmine #14761) である。

**本 repo が採用する branch topology: single-canonical-branch (`origin/main`)。** 配布正本 `### branch topology を宣言する (staged / single-canonical)` が要求する宣言はこれである。

本 repo 固有の結線 (配布正本と矛盾させない採用差分):

- **`origin/main` が新規 lane の base、review 承認後の integration target、通常開発の唯一の canonical branch である** (Redmine #14761 owner intent)。統合 push を trigger するのは **Review Gate approval** であって UserStory の close ではない (順序は下の bullet で固定する)。`.mozyo-bridge/config.yaml` の `sublane_integration.integration_branch` は `main` である。config と本節が同じ答えを返すことが、この採用宣言の検証可能な形である。
- **`main-next` は凍結された旧 staging branch であり、新規 write target ではない。** #13126 j#71900 / j#71905 で `origin/main-next` を実際の integration target として運用していたが、#14761 の owner intent により single-canonical-branch topology へ移行した。収束は force push / history rewrite を使わない一度きりの非 fast-forward merge で行い、`origin/main` は移行時点の両 branch の history を祖先として保持する。`origin/main-next` は rollback 用の凍結 ref として保持し、即時削除しない。新規 lane を `origin/main-next` から cut しない。
- **staged topology の checkpoint 規範は本 repo に適用されない。** Redmine Version close は `origin/main` 昇格 checkpoint ではなく、`push_waiting` 相当の待機 state も存在しない。統合済みか未統合かは `### Lane State Classes` の `integration_waiting` と integration disposition journal で表現する。`origin/main` を守るのは per-Version checkpoint ではなく unit ごとの gate であり、その順序は **UserStory ごとの Review Gate approval → coordinator の integration disposition (`origin/main` への統合 push) → Close Approval Separation を伴う owner close approval → Close Gate (issue の close)** である。*統合* を gate するのは Review Gate approval、*issue の close* を gate するのが owner close approval であり、**owner close approval は統合の前提条件ではない** (統合を close 待ちにすると review 承認済みの統合が無期限に停止する)。これに `## Completion Semantics` の 2 層 push authority (#13026) — 実装者は issue / lane branch のみ push し、integration branch を前進させるのは review 承認後の coordinator — が重なる。
- release gate (release tag / package version bump / TestPyPI / PyPI publish) は topology から独立した別 gate であり、引き続き direct_owner 承認を要する。`origin/main` への統合はそれ自体では release ではない。
- Redmine Version 操作面 (rename / close / lock / delete) の safety 境界と capability gap の正本は #12651 ([[logic-redmine-version-operation-surface]]) が持つ。close 操作を実行する MCP tool は epic_ladder #13136 で整備予定である。
...
- **early hibernate** (feature lane の process 解放を ticket close / installed dogfood に直列結合しない): 配布正本は skill `references/workflow.md` `## Sublane hibernate (プロセス解放) と early hibernate` と central preset `## Completion` の hibernate 節。repo-local runtime は `sublane hibernate` の 2 park basis (`dependency` / `early_hibernate`)。early hibernate は same-lane Review Gate approved + integration (`main`。`### Publication checkpoint の採用 (integration / publication 2 層)` の single-canonical-branch topology) + required CI green を basis とし、**dogfood の execution/evidence を専用 release issue へ委譲する (source issue の close authority と owner close approval は委譲せず coordinator の通常経路に残る)**。common safety gate (pending review/callback/integration/work/prompt・dirty/unpushed・identity 不明) は従来どおり fail-closed だが、**owner close approval pending は basis 依存**で early hibernate では blocker にしない (owner_waiting でも発動可、両 basis 成立時は early を優先)。retire (issue closed 前提) とは別 disposition であり、`## サブレーン退役` を置き換えない。
