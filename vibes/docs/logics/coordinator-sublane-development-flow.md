...
- **supersede-by-EXISTING は gate と change scope の両方に適用する**: 新しい malformed gate は古い有効 gate を **shadow** し、同様に新しい `changed_paths` 宣言は — 項目が空でも `commit` を欠いていても — 古い proven scope を **shadow** する (declaration は valid だからではなく**存在によって** supersede する)。片方だけに適用すると、新 commit の path 集合が未証明のまま古い commit の scope に対して exemption が成立してしまう。field を持たない journal は「宣言していない」だけなので supersede しない (不在は宣言ではない)。
