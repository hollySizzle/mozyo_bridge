...
- **durable record が先、authority に触るのは後**: stall 分類と再通知は durable event である (`skills/mozyo-bridge-agent/references/workflow.md` `### 検出と再通知を durable journal として記録する`)。順序は **record → reserve → send** とし、通知は **その record の journal** を指す (原 dispatch ではない)。**reserve を先に取って record 失敗時に `cancelled` へ倒す設計は誤り**である: `DispatchOutboxFence.reserve` は `cancelled` を **terminal never-send** として扱うため、transient な write 失敗がその anchor の recovery を **恒久封鎖**する。record は marker pre-read により idempotent なので reserve 前に書いてよく、失敗しても durable authority に一切触れないため次の sweep がそのまま retry できる。fence の職掌は **send の at-most-once に限定**する。
- **CAS が無い store では position で締める**: Redmine に CAS は無い。record 自身の journal id `R` が **durable な serialization point** であり、送信前に「`R` より前に qualifying gate が存在しないこと」を verify する。`R` より前の gate は「record を書いた時点で既に verdict が古かった」ことの証明 (`record_stale_at_write`)。なお **送信可否は「progress が在るか」だけで決まる** —— 前後関係は「log に残った record が真だったか」を分けるためのものであり、生きている lane を poke しない判断そのものは順序に依らない。
