...
管制塔の lane 配置は **leaf 数最大化問題** として扱う。入力は Redmine issue / journal、review /
...
- dispatch する leaf は、file / module / command surface、runtime invariant、route identity、
...
- target: main coordinator に加えて active implementation sublane 4 本。
- burst: active implementation sublane 5 本目は、既存 review / owner / blocker / retirement queue を飢えさせず、merge conflict / module_health baseline conflict / shared invariant conflict の期待コストが低いことを管制塔が記録した場合のみ許容。
- stop: 6 本目以降の active implementation sublane は、explicit owner / operator decision を durable issue に記録せず開かない。
