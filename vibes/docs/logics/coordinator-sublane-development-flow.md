...
- ★★★**以下は #15195 着地後に admit する契約であり、現在何かを守っている防御として読まない** (#15166 design direction j#102092)。「この audit failure」と「その後継の acceptance」を結ぶ binding は **coordinator の判断**であり、journal から導出できないことは 3 round の実測で確定した。置き場の正本は [[logic-managed-state-model]] の `state_kinds` である:
  ...
  - ★★**authority-bearing store の書込み境界を宣言 scope 外へ逸らさない** (#15166 review j#102147 finding 2、実測)。DB と nonce sidecar の両方について、open / write の前に **`lstat` で link 自体**を検査し、symlink / 非 regular file を拒否する。検査しないと symlink 一つで home 外へ書け、nonce による store identity 保護自体が無効化される。
