...
- ★**hard carve-out は marker の自己申告では晴れない**。`scope=no_change_investigation` は必要条件にすぎず、release / tag / publish / package distribution、production verification・外部副作用、credential / auth / permission / billing、destructive / migration を示す **recognized durable fact** が 1 件でもあれば拒否する。判定は **structured gate token のみ**で行い、prose keyword scan を authority にしない。さらに **recognized gate inventory を解決できない場合も既定 clear にせず typed refusal** とする (#14613 の実 record は非 canonical heading のため gate 認識 0 で、ここで正しく落ちる)。`direct_owner` provenance はこの carve-out の escape ではない。guardrail / preset / router / skill / scaffold は repository change そのものなので前段の zero-change で既に落ちる。
