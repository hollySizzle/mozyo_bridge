...
- ★★**本 route は「audit が失敗結論だった」ことを証明しない、と明記する**。結論は prose であり governed surface ではない。ここに prose 文法を起こすのは #14755 review j#99065 の欠陥 (自分自身しか証人がいない主張) を一段上で繰り返すことになる。代わりに **安全上の重みを実際に担っている連言**を establish する:
  ...
  3. **後継が本 issue と本 audit 記録を名指し返す** — source 側 declaration だけでは tracker 内の任意の approved issue を後継と名乗れるため、**別 issue の record に、別 actor が、別時点で**書かれた双方向 acknowledgement を要求する。
  4. **consequence が bound される** — record が zero change を宣言し、かつ live lane が integration branch に対して固有 commit 0 + worktree clean + declaration head と literal 一致。
...
- **既存 fence を緩めない**: 通常の review generation fence、#14539 exemption、#14695 waiver、#14755 terminal はいずれも不変で、本 route は同一 fence への independent route として admit 側にだけ効く。measured route を渡した場合 `--latest-generation-admissible` へ fall back しない。review round のある record、open issue、declaration 以降の lifecycle gate、不在 / gate 形 / declaration より新しい audit journal、change 宣言のある record、未 acknowledge・未完の後継、foreign な issue・lane・generation・integration branch、post-declaration head mutation、dirty worktree、固有 commit ありは zero-write で拒否する。
