...
- ★★★**本 route は general terminal ではなく bounded migration とする**。「この audit failure」と「その後継の acceptance」を結ぶ binding は **coordinator の判断**であり、本 workspace にそれを担える authority (actual coordinator action に検証可能に束縛された writer/receipt authority) は存在せず、その裁定は本 issue の権限外である。したがって binding は record から導出せず、**code に enumerate する** (`SANCTIONED_MIGRATIONS`: source issue / audit journal / successor issue / successor review journal / lane / generation / head / integration branch の exact tuple)。enumerate されていない tuple は record が何を示していても拒否する (`not_an_enumerated_audit_failure_migration`)。★**journal の書き手は entry を追加できない**という 1 点がこの選択の理由であり、route の拡張は diff に現れる code 変更になる。head coverage と correlation marker は pin の上の conjunct として残す (drift 検出) が、authority ではない。
  - ★**「相関を増やす」試行は 2 回とも実測で否定された**。R1 の相互 acknowledgement (同一 writer が両方を置ける)、R2 の head coverage (zero-change lane では無償)。**新しい correlate を足す前に「この主張と矛盾しうる record が、別 actor / 別時点に存在するか」を問う** (#14755 の教訓) に加えて、**「その相関は、守ろうとしている population において実際に希少か」**も問う。
  - ★★★**authority を decision API の引数にしない** (#15166 review j#102074 finding 1、実測)。R3 は enumeration を test seam として keyword 引数で公開し「application は渡さない」と説明したが、reviewer が decision 関数へ直接 pin を渡すと admit した。**authority-bearing な decision API が代替 authority を受け取る契約は、caller 1 つを source-inspection しても閉じない**。enumeration は module scope から読み、引数を置かない。test が package 自身の定数を自プロセス内で書き換えるのは、exported function から到達できる経路ではないため別問題である。
...
- ★**writer authority の gap は閉じていない、と書く**。pin が行うのは gap を閉じることではなく、**一般化を拒否することで gap を渡る必要をなくす**ことである。`hibernate_issuer_policy` は author 引数を取らない policy binding のままで、本 gate は `_KIND_ISSUER` / `_KIND_RULING` に登録しない。general route の解禁には別途 writer/receipt authority の裁定が要る。
- ★**pin の置き場を config にしない**。`SublaneIntegrationConfig` の正本が「policy intent であって authority ではない」と明記し、authority 形の key を schema が拒否する。config が供給してよいのは caller が選べない期待値 (`integration_branch`) までであって、admit する権限ではない。
- **既存 fence を緩めない**: 通常の review generation fence、#14539 exemption、#14695 waiver、#14755 terminal はいずれも不変で、本 route は同一 fence への independent route として admit 側にだけ効く。measured route を渡した場合 `--latest-generation-admissible` へ fall back しない。review round のある record、open issue、tracker が open を報告する source / successor、tracker status が読めない場合、declaration 以降の lifecycle gate、不在 / gate 形 / declaration より新しい audit journal、change 宣言のある record、未 acknowledge・未完の後継、後継 approval が本 lane head を検証していない場合、foreign な issue・lane・generation・integration branch、post-declaration head mutation、dirty worktree、固有 commit ありは zero-write で拒否する。
- **検討して採らなかった代替**: 「exact identity を committed config へ enumerate して一般化を止める」案は採れない。`SublaneIntegrationConfig` の正本が **「これは policy intent であって authority ではない」「owner-approval / close / callback / durable-anchor invariant をここで無効化する key は意図的に存在しない」** と明記しており、`SUBLANE_INTEGRATION_KEYS` は閉じた 4 key 集合で、authority 形の key は `_reject_boundary_and_unknown_keys` が拒否する。terminal の allowlist を config に置くことは、その宣言済み境界を跨ぐ。config が供給してよいのは **caller が選べない期待値** (`integration_branch`) までであり、admit する権限ではない。
