...
- **exemption・Close・integration は同一 target commit へ束縛する**: 3 つの fact が「issue 内のどこかに存在する」ことの連言は、それらが**同じ作業について**であることを意味しない。terminal retire は (a) Close gate が宣言する commit が exemption の coverage を証明した commit と **literal 一致**すること、(b) integration disposition の journal がその change scope 宣言より**古くない**ことを併せて要求する。両者を欠くと、旧 commit の Close / merge 証跡で新 commit を未統合のまま retire できる。短縮 hash は full hash と照合しない (safety fence 側に resolve 対象の repository が無いため、長さ不一致は fail-closed)。integration 側が順序判定なのは、governed disposition record が commit field を持たないためで、durable grammar が実際に支える最強の相関である。
