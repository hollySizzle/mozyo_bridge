...
#12321 は visibility / reporting foundation を据えた。**#12324 (Version #241 `v0.10.10 module health ratchet / regression policy`)** はこの gate を report-only / allowlist-heavy な段階から、通常開発で再肥大化を止める regression policy へ引き上げる。本 issue で追加した契約は後段の「期限付き allowlist」「ratchet 判断記録」の節に記す。
