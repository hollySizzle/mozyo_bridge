...
- Redmine #15428「rc2→rc3 upgrade contract (store migration 手順) を README と release ledger へ反映する」
  - 何が変わるか: RC 系列間 (rc2→rc3) の upgrade は CLI 差し替えだけでは完了しない。旧 session 停止後・新 runtime での最初の managed launch 前に home-scoped store の手当てが必要になる: (1) attestation store の旧 shape は managed launch preflight が fail-closed で拒否するため `mozyo-bridge herdr attestation-store migrate --write` (backup-first / idempotent) で forward migration する。(2) 旧 shape の launch-generation store が新 runtime 視点で corrupt と報告される場合は、旧 pair 閉鎖後に `mozyo-bridge herdr launch-generation-store rebuild --write` で backups/ へ rotate する (healthy store と live generation 保持中は拒否される)。手順の正本は README「Herdr support and breaking upgrades」の「Identity store migration between release candidates」(Z690 live 実測: Redmine #15422)。
  - 何もしない場合: upgrade 後の managed launch は store 手当てが済むまで preflight で fail-closed に拒否され続ける (silent 破損はしない。既存 store は byte 不変のまま launch だけが成立しない)。
  - 元へ戻す方法: 両 rail とも書込み前に旧 store を `~/.mozyo_bridge/backups/` へ退避する。migration 前の runtime へ戻す場合は backup から手動復元する (in-place downgrade は従来どおり非サポート)。
- Redmine #15242「既定queue-enterのEnter吸収を安全に補完」
  ...
  - 何もしない場合: 新しいmanaged launchは`pane_bound_v2`を記録してそのまま利用できる。候補版より前から動く`pane_bound_v1`／legacy pairはterminal provenanceを持たないため、Herdr queue-enterとrollback closeがfail-closedし、以前のような楽観的successやpresent targetの互換closeは行わない。`session-start`再実行はlive pairをadoptするだけでv2へ更新しない。record-less non-default scratch pairだけは`herdr session-retire` preflight→明示承認付きexecute→positive absence→fresh `session-start`で更新できる。lifecycle-managed active pairとdefault coordinatorの汎用receipt-refresh railは未成立であり、session continuityを約束するreleaseでは引き続きblockerである。例外として、全環境をownerが管理する少人数cohort向けの初回1.0 release-candidate系列（1.0 GA前の修正版RCを含む）に限り、破壊的cutoverの明示受諾を前提にfresh named Herdr sessionへ切り替えてよい。Herdr 0.8.0はdefault sessionをdeleteできず、stopだけでは保存済みlayoutを消さないため、旧defaultは停止後も再利用しない。公開前に受入対象環境で、旧defaultの`running=false`、全launch／observation／action入口が同じnamed session socketへ束縛されていること、exact artifactからfresh launchした両slot、実handoffの`submitted_confirmed`、stale／duplicate／unavailable／旧v1のzero-sendを順に再読して確認する。一つでも満たさなければ例外は不成立としてreleaseを止める。この限定例外は#15246 v1 pairのv2更新rail不足の技術的解決・close・一般移行runbook成立を意味しない。
  - 元へ戻す方法: v2 pairをまだ作っていない環境は`terminal_transport.backend: tmux`へ戻してprocessを再起動できる。v2 receiptをshared homeへ書いた後は0.20.1へのin-place downgradeを行わない。0.20.1はv2 receiptを未知としてcloseを拒否するため安全側だが、v2 stateを収束できず、0.20.1で再launchすると再びv1を作る。breaking cutover smokeがv2書込み後に失敗した場合も旧defaultを再起動せず、候補版readerでstartup／rollback debtを収束し、named sessionをstop→delete→positive absenceまで確認してから、修正版candidateでfresh named sessionを作成する。delete不成功またはabsence未確認なら停止状態を維持し、同名sessionを再起動しない。Git worktreeとbranchはsession外に残るが、terminal conversationとlayoutは復元対象にしない。別途承認したdowngrade計画なしには旧runtimeへ切り替えない。
...
内部 beta の TestPyPI 手動配布は Redmine Version close を **先に要求しない**。#13601 当時の本 repo は staged topology (staging `main-next` + 公開 `main`) にあり、publication checkpoint が Redmine Version close 前の `origin/main` push を禁じていたため、内部 beta を publish するために公開履歴を先に進める必要が生じると `Version close → origin/main → TestPyPI → #13528/#13527 → Version close` の循環になっていた。これを壊すため、exact reviewed integration head を main 固定の workflow から直接 publish する。

本 repo は #14761 で single-canonical-branch topology (`origin/main`) へ移行し、Redmine Version close はもはや `origin/main` 昇格 checkpoint ではない (`skills/mozyo-bridge-agent/references/workflow.md` の `### branch topology を宣言する (staged / single-canonical)` / `### single-canonical-branch topology`、採用宣言は `vibes/docs/logics/coordinator-sublane-development-flow.md` の `### Publication checkpoint の採用`)。それでも本節の gate は **緩めない**: internal beta は release gate 側の carve-out であって統合 checkpoint の副産物ではなく、exact candidate に対する下記の fail-closed 照合は topology とは独立に維持される。`origin/main` へ統合されたこと自体は internal beta publish の承認ではない。
...
- automatic main-CI dev publish path は後方互換に維持する。`testpypi` environment の required reviewer は owner 決定 (Redmine #15255 j#105330、2026-08-15) で撤廃済みであり、automatic / exact-candidate いずれの path も deployment approval 待ちにならない。approval authority は Redmine gate と trusted build job の fail-closed 照合、environment 側は main-only deployment branch policy が残る。
