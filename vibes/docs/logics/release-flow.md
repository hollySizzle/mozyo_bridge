...
| TestPyPI publish workflow dispatch (exact candidate) | `mozyo-bridge release publish --testpypi --source-sha <40-hex> --expected-version X.Y.Z --source-ref <origin ref>` | dispatch 後の判定は operator (Redmine #13601) |
...
- trusted build job が fail-closed 照合 (HEAD == source_sha / source_ref の origin tip == source_sha / 2-file version mirror == expected_version / 同 SHA の `Test` (`test.yml`) success / expected_version が TestPyPI 未使用) を実行し、build job と OIDC publish job を分離する (`id-token: write` + `environment: testpypi` は publish job のみ)。
...
3. 必要なら `Publish to TestPyPI` workflow で TestPyPI rehearsal を行う (`mozyo-bridge release publish --testpypi --source-sha <40-hex> --expected-version X.Y.Z --source-ref <origin ref>`, Redmine #13601)。
