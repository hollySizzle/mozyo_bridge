...
- 親 project coordinator が、過剰な test hint なしに依頼を読み、自 project config
  (`project.env` 相当) や durable work context から mozyo_bridge を delegated child として
  選ぶ。
...
Redmine #12499 の現在の合格目標は #12546 autonomous parent real-machine smoke であり、
その後に #12461 3-window delegated route retirement / cleanup verification が続く。

layout gate の履歴根拠は closed #12622「Source/Test layout を Redmine 番号付き Epic/Feature
階層へ全面再移行する」である。#12590「Epic/Feature slug 階層へ全面展開」は Epic-level で
止まり `features/` root と旧 `domain/` / `application/` 実装が残ったため、#12590 j#66014 /
j#75661 の判断で #12622 へ supersede した。したがって #12590 を current hard gate として
扱わない。source/test layout は #12622 で Redmine-numbered Epic/Feature path へ再移行済みで
あり、以後の smoke / scenario が古い巨大 file 配置へ引き戻されないことは履歴として確定して
いる。
...
3. #12709「GK3500 修正後 祖父・親・子・孫 実機 smoke rerun」を exploratory rerun gate として
   実行する（`### GK3500 ticketless rerun gate` の順序に従う）。
4. #12546 autonomous parent real-machine smoke を full acceptance として実行する。
5. #12461 3-window delegated route retirement / cleanup verification を #12546 後に実行する。
...
- 親が `project.env` を読んで判断すべき内容の先回り要約。
...
- parent project gateway が domain/design answer を吸収せず、domain 判断が必要な相談を
  ticketless no-anchor work-intake として child coordinator へ渡している（Redmine anchor の
  作成・選択は child coordinator が所有する）。親が `implementation_not_needed` /
