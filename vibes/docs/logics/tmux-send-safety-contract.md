...
`injection_stage` authority は partial delivery を `uncertain` とし、外側の gateway command / 本文の blind
retry を許可しない。durable record が task completion の正本である点も不変。
