...
| `rolled_back` | `wait_for_text` が landing_timeout 内に marker を観測できなかった。`C-u` rollback を発行し、**rollback 後に再 capture して typed input の residual (marker または body tail) を探す** (#12450)。residual が見えない → Enter は押していない。ただし **capture に residual が無いことは clear の証明ではない** (staged prompt が可視領域外にあり得る、#12450 j#63612)。よって honest に「clear 未検証」と記録し、`clear された` とは主張しない。 | `blocked` / `marker_timeout` |
| `rolled_back_pending` | 上と同じく marker 未観測で `C-u` を発行したが、**再 capture で typed input の residual (marker または body tail) が composer に見えている** (受信側 TUI が `C-u` で composer を clear しなかった、#12450 j#63612 で marker のみ check が false-cleared を踏んだため body tail も見る)。Enter は押していない・何も submit されていないが composer は clean ではない。clean rollback とは記録せず、明示的な pending-composer recovery (staged prompt を submit するか clear する。再送しない) を提示する。 | `blocked` / `marker_timeout_pending` |
...
| `rolled_back` | `C-u` rollback を発行し、再 capture で residual (marker / body tail) が見えなかった。Enter は未押下。ただし clear は検証できていない (capture-absence は証明ではない、#12450 j#63612)。 | durable record を読み、resend 前に `mozyo-bridge read <target>` で composer が staged でないことを確認する。staged なら submit / clear し、再送しない。確認済み clear のときだけ retry。retry は contract 外の判断。 | pane に断片が残っていても rendered text を ACK / delivery 正本にしない。durable record を直接読みに行く。 |
| `rolled_back_pending` | `C-u` rollback を発行したが、再 capture で typed input の residual (marker / body tail) が composer に見えている (TUI が `C-u` で clear しなかった、#12450)。Enter は未押下・未 submit だが composer は clean ではない。 | **再送しない** (staged prompt が重複する)。`mozyo-bridge read <target>` で pending prompt を確認し、staged prompt を submit する (`mozyo-bridge keys <target> Enter`) か clear する。durable anchor が source of truth。 | pane に staged prompt が残るが、rendered text を ACK / delivery 正本にしない。durable record を読みに行く。 |
...
- 受領方法: durable record に attempted command + `DeliveryOutcome` を残す。`C-u` rollback を発行し Enter は押さない。#12450 以降は rollback 後に **再 capture して typed input の residual (marker または body tail) を探す**: residual が見えれば `blocked` / `marker_timeout_pending` (TUI が `C-u` で clear しなかった)、residual が見えなければ `blocked` / `marker_timeout` を emit する。後者でも **clear は検証できていない** (capture-absence は証明ではない、#12450 j#63612) ので「clear 済み」とは主張せず、resend 前に `read` で composer を確認する honest wording にとどめる。pending 側は staged prompt を submit するか clear する明示的 recovery を提示し、**再送はしない** (重複するため)。いずれの場合も rendered text を ACK / delivery 正本にしない。
...
3. landing_timeout 内に marker を観測できない場合は、必ず `C-u` rollback を発行し (composer clear を試行)、Enter は発行しない。die 出口を確保する。#12450 以降は rollback の後に **再 capture して typed input の residual を探す** (`rollback_and_detect_residual`、marker と body tail を `wait_for_text` と同じ wrap 補修一致で見る)。residual が見えれば `blocked` / `marker_timeout_pending`、見えなければ `blocked` / `marker_timeout` を emit する。**residual 不在を clear の証明として扱わない** (#12450 j#63612): marker_timeout 側も「clear 未検証、resend 前に read で確認」という honest wording にとどめ、pending 側は staged prompt を submit / clear する明示 recovery を durable に提示し再送しない。いずれも rendered text を ACK 正本にしない (条件 5)。
