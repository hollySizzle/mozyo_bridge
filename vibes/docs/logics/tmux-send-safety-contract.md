...
- 非射程の related work:
  ...
  - queue-enter rail を v0.4 normative default に flip する CLI / handoff 実装: Asana `1214825307842391` (parent `1214825156046950`、v0.4 lineage、本 contract の `### Contract Default vs CLI Default (Transient Gap)` 解消を担う)
...
- 上流 ACK state machine (`submitted` / `acknowledged` / `pending_submit` / `rolled_back` / `delivery_failed` / `stage_failed`) を、現行 tmux CLI surface (`--mode queue-enter` (v0.4 contract が定める normative default; CLI binary 上の `--mode` default flip は Asana `1214825307842391` が所有、本 contract land 時点では shipped default は依然 `standard`) / `--mode standard` (v0.4 contract 上は strict explicit fallback; CLI binary 上は child task land まで現行 default として継続) / `--mode pending` / 手動 `read` / `--no-submit`) にどう射影するか。詳細は `## Default Delivery Promise (v0.4)` の `### Contract Default vs CLI Default (Transient Gap)` 節参照。
...
### Downstream Surfaces (本 task の射程外)

本 contract のピボットを反映する CLI / handoff 実装、distributed docs / rules / skill refs / preset surfaces、tests / smoke / workflow verification は別 child task が所有する。

- CLI / handoff 実装 (CLI binary 上の `--mode` default flip): Asana `1214825307842391`
- distributed docs / rules / skill refs / preset surfaces: Asana `1214825156844993`
- tests / smoke / workflow verification: Asana `1214825156769677`

これらが land する前は、`CLAUDE.md` / `AGENTS.md` / `agent-workflow.md` / `safety.md` などの operator-facing guidance が strict-default 文言のまま一時的に残ることがあるが、source-of-truth contract は本文書である。downstream surfaces 側の文言は本 contract に合わせて順次更新される。

### Contract Default vs CLI Default (Transient Gap)

v0.4 で本 contract が固定するのは **normative default rail** だけである: agent pane handoff の正規路を `queue-enter` rail に置き、`standard` を strict explicit fallback として残す、という policy / contract の宣言。CLI binary の `--mode` 実装 default が `queue-enter` に flip するのは child task Asana `1214825307842391` が land したときであり、本 task の射程外。

このため、本 contract が v0.4 として land した直後の transient state を honestly stated:

- **本文書 (source-of-truth contract)**: `queue-enter` が v0.4 normative default、`standard` が strict explicit fallback。
- **shipped CLI binary**: `--mode` の implementation default は依然 `standard` (`src/mozyo_bridge/application/cli.py` の現行 `default=MODE_STANDARD`)。operator が `--mode queue-enter` を明示しない send は strict rail に乗る。本 contract land 後、Asana `1214825307842391` の implementation が land するまでこの状態が続く。
- **operator-facing distributed docs / rules / preset surfaces** (root `CLAUDE.md` / `AGENTS.md` / `agent-workflow.md` / `safety.md` / scaffold presets / skill refs): 旧 strict-default 文言が一時的に残ることがある。これらの v0.4 default 反映は Asana `1214825156844993` が所有する。

本 contract の各箇所で出てくる「`--mode queue-enter` (default since v0.4)」「v0.4 以降は queue-enter rail が default」等の wording は、いずれも **本文書が定める v0.4 normative default** を指す。shipped CLI binary が現時点でその通りに振る舞うことを意味しない。operator が「contract が言う v0.4 default」と「現行 binary 上の `--mode` 暗黙値」のいずれを参照すべきか迷う場面では、(a) 明示的に `--mode queue-enter` または `--mode standard` を指定する、(b) child task `1214825307842391` の land 状況を確認する、のいずれかで曖昧さを除く。

ギャップ解消の time horizon:

1. 本 contract (v0.4) が land する (本 task `1214824751741628`)。
2. CLI binary の default を flip し、admission gate / outcome wording を本 contract と一致させる (Asana `1214825307842391`)。
3. distributed docs / rules / preset / skill refs の文言を v0.4 contract に揃える (Asana `1214825156844993`)。
4. tests / smoke / workflow verification を v0.4 default 前提に更新する (Asana `1214825156769677`)。

step 1 land 時点では step 2〜4 はすべて pending であり、contract と shipped state の不一致は意図的に許容される transient gap である。step 2 land 後はこのギャップ自体が解消する。
...
v0.3 で追加。`### 適用条件 (Entry Conditions)` を「`--mode queue-enter` 経路に乗ったときに CLI が同時に守る machine-checkable な admission gate」として固定する。本節は実装 task `1214785367563471` が直接の実装対象とする仕様であり、CLI 側の admit / reject は本節の signal セットだけで決まる。v0.4 contract が `--mode queue-enter` を agent pane handoff の normative default に置いたので、CLI binary 上で `--mode` default が flip した後 (Asana `1214825307842391` land 後) は明示的 opt-in なしの send でも本節の admission gate を通過しないと typing は起きない。v0.4 contract land から CLI flip までの transient state では、operator が `--mode queue-enter` を明示したときにのみ本 admission gate に乗る (`## Default Delivery Promise (v0.4)` の `### Contract Default vs CLI Default (Transient Gap)` 参照)。
