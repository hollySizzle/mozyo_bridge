...
| `rolled_back` | `wait_for_text` が landing_timeout 内に marker を観測できなかった。`C-u` rollback を発行し、**rollback 後に再 capture して composer を検証する** (#12450)。再 capture で marker が消えている → rollback verified、Enter は押していない。 | `blocked` / `marker_timeout` |
| `rolled_back_pending` | 上と同じく marker 未観測で `C-u` を発行したが、**再 capture で typed prompt が composer に残っている** (受信側 TUI が `C-u` で composer を clear しなかった、#12450)。Enter は押していない・何も submit されていないが composer は clean ではない。clean rollback とは記録せず、明示的な pending-composer recovery (staged prompt を submit するか clear する。再送しない) を提示する。 | `blocked` / `marker_timeout_pending` |
...
- 受領方法: durable record に attempted command + `DeliveryOutcome` を残す。`C-u` rollback を発行し Enter は押さない。#12450 以降は rollback 後に **再 capture して composer を検証する**: marker が消えていれば `blocked` / `marker_timeout` (rollback verified)、typed prompt が composer に残っていれば `blocked` / `marker_timeout_pending` (TUI が `C-u` で clear しなかった) を emit する。後者は clean rollback と記録せず、staged prompt を submit するか clear する明示的 recovery を提示し、**再送はしない** (重複するため)。いずれの場合も rendered text を ACK / delivery 正本にしない。
...
3. landing_timeout 内に marker を観測できない場合は、必ず `C-u` rollback を発行し (composer clear を試行)、Enter は発行しない。die 出口を確保する。#12450 以降は rollback の後に **再 capture して composer を検証** し (`rollback_and_verify_cleared`、`wait_for_text` と同じ wrap 補修一致を使う)、marker が消えていれば `blocked` / `marker_timeout` (rollback verified)、typed prompt が残っていれば `blocked` / `marker_timeout_pending` を emit する。後者は staged prompt を submit / clear する明示 recovery を durable に提示し再送しない。いずれも rendered text を ACK 正本にしない (条件 5)。
