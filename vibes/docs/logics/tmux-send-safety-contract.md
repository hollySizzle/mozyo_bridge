...
herdr_queue_enter_retry:
  ...
  causal_observation:
    - idle / turn-ended 系列では first Enter と各 extra Enter より先に working-transition wait を arm する
    - busy 系列 (ADR-0002 / #15537) は wait を attribution に使えないため arm を要求しない。
...
admission は試みるが、first Enter と observation は post-body generation 再確認と deadline check、および idle /
turn-ended 系列では wait arm が成功した場合に限る (busy 系列は #15537 の wait 非依存 full effect fence を通す)。sub-millisecond 値を `0.001` 秒へ切り上げて actuation budget を広げない。非有限値は本文
...
- **Normative default rail (v0.4 contract 以降)** は `--mode queue-enter`: Claude / Codex agent pane に対する queue-oriented delivery 経路。Layer B deterministic admission gate が admit したときだけ本文を一度 type する。tmux backend は marker 未観測でも `C-u` rollback を行わず、既存 marker retry 後に観測あり → `sent` / `ok`、未観測 → `sent` / `queue_enter`。Herdr backend は v0.21 により idle / turn-ended 系列では同一 generation の causal turn start が確認できた場合だけ `sent` / `ok` とし、busy 系列 (ADR-0002 / #15537) では full effect fence 後の composer clear を証拠に `sent` / `queue_enter` を返す。どちらの証拠も無い未確認は precise `blocked` reason + non-zero へ fail-closed する。durable record (Asana task comment / Redmine journal) は引き続き source of truth であり、queue 受理を task 完了の signal にしない。CLI binary 上も commit `93dc953` (Asana `1214825307842391`) で `default=MODE_QUEUE_ENTER` に flip 済み。詳細は `## Default Delivery Promise (v0.4)`、`## Queue-Enter Default Rail`、`### Enter-Only Retry` を参照。
- Herdr の既定 queue-enter は、本文1回・first Enter zero-or-oneを保つ。idle / turn-ended baseline の first Enter は post-body generation 再確認・wait arm・deadline check が成功した場合だけ発行し、busy baseline (ADR-0002 / #15537) は wait 非依存の full effect fence (generation / composer / screen / state) を通して発行する。各 extra Enter では同一 target / launch-generation / current-composer / screen / runtime-state gate を **fresh に**通す (idle / turn-ended 系列は加えて先に wait を arm)。timeout-only 系列は public default 30秒/2秒の absolute budget と policy 上限まで Enter-only retry を反復できるが、wait error は次の Enter を許可せず即時停止する。causal event と coherent generation が揃えば `sent` / `ok`。busy baseline は working-transition wait を attribution に使えないため (ADR-0002 / #15537) pending-observer 要求を waive して Enter を発行し、injected body が current composer から消えた事実を submission 証拠として `sent` / `queue_enter` を返す (causal claim なし・本文再入力なし・deadline と generation gate は維持)。それ以外は precise `blocked` reason + non-zero であり blind retry しない。tmux queue-enter、herdr standard、pending はこの更新の対象外。
