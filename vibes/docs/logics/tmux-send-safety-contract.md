...
- v0.12 で更新 (Redmine #14249 R5、review j#95911): v0.11 の subreason を **全 reader surface**へ。 (a) pasteable / persisted markdown に `subreason` / `basis` を closed vocabulary token として 固定 render する (v0.11 は wire JSON にしか載せておらず、`next_action` だけが「読め」と指示していた)。 (b) terminal stderr を **subreason 別の repair** にする。v0.11 は全 refusal へ「lane の worktree binding を repair せよ」と出しており、`identity_unattested` / `foreign_workspace` は当該段に到達せず、 `lifecycle_store_upgrade_required` はその repair で解消しない。 (c) refusal payload に **filesystem path を入れない**。`detail` は wire / record / stderr の 3 経路へ 同時に出るため、v0.11 が補間した raw exception は個人 home path を 3 箇所へ publish していた。 exception は type 名のみとする。rail 挙動・pane selection・wire `Reason` 集合は不変。
- v0.13 で更新 (Redmine #14249 R6、review j#95995): v0.12 の per-subreason repair を **`next_action` にも適用**する。v0.12 は stderr だけを subreason 別にし、`next_action` は 6 subreason で同一のままだった — その本文は原因列挙に `lifecycle_store_unreadable` を含まず、 全件に「lane の worktree binding を repair せよ」と案内していた。`next_action` は **wire JSON と pasteable / persisted record の両方に載る**ため、読み手が実際に行動する面で 誤 repair が残っていた。`auto_target_repo.subreason` から repair を導出する形にし、 subreason を `next_action` 本文へ inline で載せる。`reader_upgrade_required` へ写る subreason は同 reason 自身の repair が既に唯一正しいので不変。rail 挙動・wire `Reason` 集合は不変。
- v0.14 で更新 (Redmine #14249 R7、review j#96019): v0.13 の per-subreason repair の **fallback を safe-generic 化**する。v0.13 は subreason が欠損 / 未知のとき v0.12 で誤りと確定した旧文面へ戻っており、原因一覧に `lifecycle_store_unreadable` を欠き、 存在しない payload を読めと指示し、全件へ binding repair を案内していた。 fallback は情報が最も少ないときに通るので、**原因を推測せず全 refusal に真な generic repair** のみを返し、未知 token は「この build が知らない subreason である」と提示する。 併せて **reason と payload の coupling を helper 内で検証**し、containment fence の reason は payload の内容に関わらず自身の `--workdir` repair を返す。rail 挙動・wire `Reason` 集合は不変。
- v0.15 で更新 (Redmine #14249 R8、review j#96042): v0.14 が新設した **unknown subreason の表示経路を bounded / path-free 契約に適合**させる。v0.14 は未知値を 無検証で backtick 内へ補間しており、newline が Markdown 行境界を越え、POSIX / Windows absolute path が durable 文面へ出て、10,000 文字 token が 10,530 文字の `next_action` を作り、 unhashable / 非 Mapping payload は例外を投げた。**表示側の境界で値自身を検証**する: `str` 型 / `[a-z][a-z0-9_]*` / 明示的最大長。満たさなければ raw を出さず stable placeholder。 malformed payload は例外ではなく generic fallback へ閉じる。wire `Reason` 集合・rail 挙動は不変。
- v0.16 で更新 (Redmine #14249 R9、review j#96049): v0.15 の検証を **検証済み値の使用**まで拡張する。v0.15 は文字を検証して caller の object を返しており、 `str` subclass の `__format__` が interpolation 時に newline と absolute path を再注入し、 unhashable / raising `__hash__` が table lookup で例外を出した。**exact builtin `str` へ 正規化してから lookup と表示に使う** (既存 idiom `str.__str__`、review j#86068 / j#86081)。 payload は **exact `dict`** のみ受理し、adversarial `Mapping.get` / equality / `__bool__` は generic fallback へ閉じる。wire `Reason` 集合・rail 挙動は不変。
...
- v0.21 で更新 (Redmine #15242 既定 queue-enter の turn-start 補完): **herdr backend の既定 `queue-enter` を causal な turn-start 観測と、厳格な Enter-only fallback に接続する**。本文は一度だけ type し、最初の Enter も一度だけ発行する。最初の Enter でこの送信に帰属できる turn start を確認できなかった場合に限り、target identity、衝突しない launch generation、現在の composer tail にある exact marker+body、startup / modal / trust / login / selection screen の非該当、runtime state の read 成功を送信直前にすべて再確認し、追加 Enter を **高々一度**発行できる。`busy` は queue semantics を維持するため追加 Enter の候補から除外しないが、busy snapshot や busy baseline に重なった event だけで submission を confirmed にしない。`awaiting_input` / `turn_ended` から先に arm した working-transition event と、前後で一致する collision-free generation を揃えた場合だけ causal confirmation とする。確認不能は `uncertain` のまま blind retry を禁止する。queue 専用 telemetry と delivery-ledger 分類を維持し、standard rail の `turn_start_outcome` と混ぜない。public `queue-enter` window は初回待機・必要な間隔待ち・追加 Enter 後の再待機を含む **単一の absolute budget** で、各 wait は remaining budget を超えない。interval は最初の Enter から追加 Enter までの最小間隔であり、初回待機が interval 以上なら追加 sleep しない。window / interval のどちらか `0`、または正値でも `0.001` 秒未満なら追加 Enter 無効（first Enter と初回観測は維持）、非有限値は pre-injection `invalid_args` とする。tmux queue-enter、herdr `standard`、`pending` は変更しない。
...
### Runtime Fingerprint Gate (Redmine #12612)

delivery rail dogfood、pre-smoke gate、release-adjacent runtime verification では、実行した `mozyo-bridge` がどの code surface を読んだかを durable record に残す。`mozyo-bridge --version` は package version 文字列しか示さず、同じ version を名乗る installed artifact と `origin/main` source tree の挙動差を検出できないため、単独 evidence にしない。

portable な記録規律 (version 文字列単独を evidence にしない / fingerprint を durable record に残す / 不一致は `blocked`・`environmental` で PASS に混ぜない / 実行 surface の無承認自己修復禁止) の配布正本は、#13060 により `skills/mozyo-bridge-agent/references/workflow.md` の `## Runtime fingerprint 検証規律` にある。本節は `mozyo_bridge` の dogfood / smoke に対する具体 field 一覧と alignment 手順の正本である。

最低限、次を Redmine journal / delivery record に記録する:

- command surface: bare CLI (`mozyo-bridge`) / source CLI (`PYTHONPATH=src python3 -m mozyo_bridge`) / TestPyPI・PyPI installed CLI など。
- executable path: `which mozyo-bridge`、または source CLI なら `python3` と cwd / `PYTHONPATH`。
- imported package path: `import mozyo_bridge; print(mozyo_bridge.__file__)` 相当。
- version string: `mozyo-bridge --version` と、source CLI を使う場合は `PYTHONPATH=src python3 -m mozyo_bridge --version`。
- expected feature probe: gate が依存する behavior の存在を機械的に確認する。例: `standard_target_admission` を要求する dogfood では、実行 surface の `orchestrate_handoff` / `cli_handoff` に該当 symbol / flag が存在すること。
- git / origin anchor: source CLI の場合は `git rev-parse HEAD` と `git status --short --branch`、installed artifact の場合は install source (pipx / TestPyPI / PyPI / local path) と判別できる情報。

fingerprint が期待 behavior と一致しない場合、その dogfood / smoke は `blocked` または `environmental` として記録し、PASS evidence に混ぜない。operator が明示承認しない限り、agent は pipx reinstall、local install、version bump、tag、publish で実行 surface をその場修正しない。修正が必要な場合は follow-up issue を作り、runtime surface を揃えてから dogfood を再実行する。

local bare / pipx runtime alignment は source-runtime smoke の PASS evidence とは分離する。dogfood / smoke 中に installed runtime の drift を検出した場合は、まず source CLI の fingerprint と結果を durable record に残し、installed runtime 側は `environmental` follow-up として扱う。source-runtime smoke が green になった後、別 Task で owner / operator approval を得て bare CLI と source CLI の `doctor runtime` fingerprint を揃える。この alignment は release、tag、publish、version bump、distribution proof ではない。

dogfood 結果は active Redmine issue に短く記録する:

- runtime fingerprint (上記 gate の command surface / executable / imported package / version / feature probe)
...
| `submitted` | `wait_for_text(marker)` が true を返し、Enter を発行した。`--mode standard` (codex は v0.6 / #13166、claude は v0.7 / #13262) では加えて Enter 後に受信 pane の新規出力活動 (turn 開始) を read-only 観測し、活動が観測できた場合にのみここへ倒す。 | `sent` / `ok` |
| `submitted` (unconfirmed turn) | `--mode standard` rail (codex / claude) で marker 観測 + Enter 発行後、観測 window 内に受信 pane の turn-start 活動が観測できなかった。Enter が busy / redraw composer に吸われた疑い。C-u rollback も再送も行わない (marker+body は一度だけ type 済み)。sender は受信 pane を再読して turn 開始を確認する (codex: v0.6 / #13166、claude: v0.7 / #13262)。 | `blocked` / `turn_start_unconfirmed` (新規 reason; v0.6 で追加) |
...
| `rolled_back` | `C-u` rollback を発行済みで Enter は未押下。多くの場合 composer は clear されるが、TUI state によっては marker / body が rendered のまま残ることがあり、rendered text を「clear された証拠」として扱わない。 | durable record を読み、handoff を別経路で再開する。retry は contract 外の判断。 | pane に marker / body 断片が残っていても rendered text を ACK / delivery 正本にしない。durable record を直接読みに行く。 |
...
- 受領方法: durable record に attempted command + `DeliveryOutcome` (`blocked` / `marker_timeout`) を残す。`C-u` rollback を発行し Enter は押さないが、receiver composer が実際に clear されたことは tmux capture から検証できない (TUI state によっては marker / body が rendered のまま残り得る)。rendered text を ACK / delivery 正本にしない。
...
3. landing_timeout 内に marker を観測できない場合は、必ず `C-u` rollback を発行し (composer clear を試行)、Enter は発行しない。die 出口を確保し、`DeliveryOutcome` も `blocked` / `marker_timeout` で正確に emit する。`C-u` 発行は sender 側の事実であり、receiver composer が clear されたことの検証ではない (条件 5 の通り rendered text を ACK 正本にしない)。
...
strict `--mode standard` rail が `marker_timeout` (`rolled_back`) を返したとき、それは **「配達されなかった」ことの証明ではない**。この state を「非配達」と読むと誤って再送・un-notified 昇格・重複 dispatch を招く。

### 実測 (measured instances)

- **#13262 j#72493** (self-send): strict rail が `marker_timeout` を報告したが、対象 composer には payload が残存し、後続で submit され得る状態だった。
- **#13261 j#72520 → #13262 j#72525 で false-blocked と確認**: 別 pane 宛の strict send が `marker_timeout` を返したが、durable record 側の観測で実際には receiver が payload を受領していた (blocked は偽陽性)。

いずれも、tmux rendered-text の marker 未観測 (character-wrap shape / busy turn 中の scrollback 流出 / TUI redraw 差) が `marker_timeout` を誘発した一方で、composer は payload を保持し後で submit し得たケースである。

### なぜ `marker_timeout` が非配達を意味しないか

- `C-u` rollback の発行は **sender 側の事実にすぎない**。receiver composer が実際に clear されたかは tmux capture から検証できない (本 contract `## State Transition と Pane の見え方` の `rolled_back` 行 / `## Blind Enter を避ける条件` 条件 5 / #12188)。TUI state によっては marker / body が rendered のまま残り、後で operator / queue により submit され得る。
- したがって `marker_timeout` は「landing を sender が観測できなかった」だけを意味し、「payload が届かなかった」ことは意味しない。

### 運用規則 (operational rules)

1. **strict `marker_timeout` を「not delivered」と読まない**。偽陽性 blocked を前提に扱う。
2. **正本は durable record (issue journal) polling**。sender も receiver も、配達可否の authoritative fallback は pane rendered-text ではなく durable record (Redmine journal) の polling とする。pane notification は pointer にすぎない。
3. 再送は `## Fallback Policy` の `### Landing marker timeout` に従い、read marker refresh + `--no-submit` budget を経る。単一の `marker_timeout` を根拠に blind-resend / 重複 dispatch / 即時 un-notified 昇格をしない。
...
同一 lane に **同一 receiver role の live pane が複数** 存在し得る。典型は cockpit gateway repair (`mozyo cockpit append` で missing Codex gateway を作り直したとき、Redmine #12226 j#61213) で、同一 `(workspace, lane)` の Claude pane が 2 枚 (`%14` / `%16`) 残るケースである。このとき次の 2 つの failure が durable record 上で混線する (Redmine #12226 j#61221 / j#61224 / j#61228 / j#61249):

- **residual prompt text in a duplicate**: 先行する `--mode standard` 送信が `marker_timeout` で `rolled_back` に倒れると、`C-u` rollback は発行されるが composer が実際に clear されたかは tmux capture から検証できない (本 contract の `rolled_back` 行 / Blind-Enter 条件 5 / #12188 で固定済みの wording)。残骸 text が duplicate pane (`%16`) の prompt に残ったまま、別 pane (`%14`) に最終 dispatch が成立し得る。residual text は `rolled_back` model の想定内であり、`cleared` と誤認しない。
- **receiver / actor record divergence**: 最終 dispatch の delivery record は receiver pane (`%14`) を記録する一方、Implementation Done journal が actor pane (`%16`) を記録すると、durable record 上で receiver と actor が乖離する (#12226 j#61224 vs j#61228)。

### 契約

- **Explicit `--target %pane` は documented escape hatch であり fail-closed しない**。explicit pane を指定した送信を duplicate の存在だけで block すると、inactive duplicate へ届けるための strict `--mode standard` recovery 経路 (`### Landing marker timeout` / Redmine #12162 の inactive-split recovery) を壊す。queue-enter rail の Step 11 は v0.5 standard_target_admission (`#### standard_target_admission (v0.5)`) で inactive split を admission 評価する: 登録済み pane は active 化して通過し、admission 不通過なら fail-closed する。いずれの場合も duplicate advisory は本節の通り diagnostic として surface し、二重の hard block は行わない。
- 代わりに、handoff は **action-time の live snapshot** (`runtime-observability-boundary.md` の Action-Time Live Preflight Boundary; 保存済み projection ではない) から、解決した target と同一 `(workspace, lane)` かつ同一 receiver role に解決する **他の live pane を列挙し、durable delivery record に diagnostic advisory として surface する**。これにより receiver pane と stale-input duplicate の双方が record 上に可視化され、receiver / actor 記録が silent に乖離しない。advisory は receiver of THIS send が `Target pane` であることと、implementation actor も同 pane として記録すべきことを明示する。
- advisory は **`sent` / `sent (queue-enter)` / `marker_timeout` のいずれの outcome でも** surface する (residual text は `marker_timeout` path で duplicate に残るため、この path での surfacing は特に重要)。
- duplicate の role 判定は `resolve_agent_role` (live tmux `@mozyo_*` pane option / window name / process) で行い、pane title には依存しない。同一 lane の Codex gateway pane (role=codex) を duplicate Claude receiver と誤認しない。target が concrete な `(workspace, lane)` identity を持たない (default lane / workspace 不明) 場合は disambiguation 対象が無いため advisory を出さない。
- advisory の pane identity row は **repo-relative / id ベースのみ** で構成し、絶対 cwd / repo_root を含めない (`vibes/docs/rules/public-private-boundary.md`; Redmine journal への貼り付け安全性)。

### `sent / queue_enter` 後の receiver pickup と stale prompt cleanup の境界

- `sent` / `queue_enter` の `next_action_owner` は strict `sent` と同じ `receiver` (durable anchor を読む) であり、**duplicate advisory はこの next-action を上書きしない**。advisory は operator-facing diagnostic であって、receiver の primary 受領契約を変更しない。
- queue 受理されたかどうか (receiver pickup) は sender 側から確認できない (本 contract `## Open Questions` 6)。pickup observability は本 contract の射程外であり、duplicate advisory はそれを代替しない。advisory が担保するのは「どの pane が receiver で、どの pane が stale 入力を抱え得る duplicate か」を durable record 上で曖昧にしないことだけである。
- duplicate pane の stale prompt cleanup (residual text の手動 clear) は operator の判断であり、send-safety contract の自動 action ではない。tool は cleanup を実行せず、cleanup が必要であり得る pane を record 上で名指しするにとどめる。これは「snapshot は pointer であって行動許可ではない」runtime-observability boundary と整合する。
...
#### herdr queue-enter (causal turn-start + 高々1回の追加 Enter)
...
herdr_queue_enter_retry:
  injection:
    body: exactly once
    first_enter: exactly once
    extra_enter: at_most_once
  causal_observation:
    - first Enter より先に working-transition wait を arm する
  ...
  extra_enter_gate:
    ...
    - target identity が注入前と送信直前で exact 一致する
    - collision-free launch generation が注入前と送信直前で exact 一致する
    ...
    - 追加 Enter より先に次の working-transition wait を arm できる
  result:
    - confirmation が揃えば queue 専用 turn-start observation に記録する
    - 揃わなければ uncertain のまま。自動・手動を問わず blind retry を許可しない
```
...
public policy の `--queue-enter-retry-window` は、初回 event wait、必要な interval 待ち、追加 Enter 後の
再待機を合わせた **単一の absolute wall-clock budget** である。各 wait timeout はその時点の remaining
budget で cap し、再 arm しても window をリセット・延長しない。`--queue-enter-retry-interval` は最初の
Enter から追加 Enter までの最小間隔であり、初回 wait が既に interval 以上かかった場合は追加 sleep しない。
window / interval のどちらかが `0`、または正値でも `0.001` 秒未満なら追加 Enter は無効で、最初の Enter と
初回 observation は従来どおり一度だけ実行する。sub-millisecond 値を `0.001` 秒へ切り上げて actuation budget
を広げない。非有限値は本文注入前に `invalid_args` とする。

Herdr の結果は既存 `queue_enter_turn_start_observation` と queue delivery-ledger rail に残す。standard rail の
`turn_start_outcome` を付けると、同じ queue delivery が event rail として再分類されるため禁止する。
confirmation が無い outcome は `sent` という表層語だけで `submitted_confirmed` に昇格させず、共有
`injection_stage` authority で `uncertain` に保つ。durable record が task completion の正本である点も不変。
...
| 7a | Anchor 構造 | CLI args | `normalize_anchor` が `AnchorError` を投げない | 全 rail |
| 7b | Redmine anchor所有関係 | trusted Redmine issue detail read | Redmine sourceではjournalが指定issue配下に存在する。issue/journal不在、所属不一致、provider unreadableはtyped zero-send refusal。Asana/ticketlessはskip | 全 rail |
...
| 11 | standard_target_admission | `pane.pane_active` + minimal admission contract | `pane.pane_active == "1"`、**または** inactive split が standard_target_admission (live pane / strong role match / workspace_id / unambiguous) を満たし activation policy が有効なら rail が `select-pane` で active 化して通過。不通過なら fail-closed。詳細は `#### standard_target_admission (v0.5)` | queue-enter のみ (v0.3 active-split; v0.5 admission に置換) |
...
Step 1–8、13–14 は strict / pending と共通の deterministic 段である。Step 9 は v0.2 で queue-enter に追加済み。Step 10–12 が v0.3 の queue-enter admission 段で、Step 7b は v0.19 の全rail共通gateである。
...
#### standard_target_admission (v0.5)

Redmine #12597。v0.3 の Step 11 は inactive split (`pane_active != "1"`) を **無条件 fail-closed** していた。この gate の本来の懸念は deliverability ではなく visibility だった: tmux は `-t` で addressing された pane が inactive split でも keystroke を届ける一方、receiver agent は構造上 operator が見ている foreground process ではない。owner (Redmine #12597 j#65493) はこの hard block を過剰と判断し、登録済み inactive agent pane が下記 minimal admission contract を満たす場合は rail が **安全に active 化して delivery してよい** ことに改めた。

**minimal admission contract (default 必須条件)**:

1. **live pane** — target が live な pane id に解決している。
2. **strong role match** — 解決 role == receiver を strong かつ non-ambiguous な signal で得ている。queue-enter では Step 9 (`binds_receiver`) が既に保証するため、実効的な新条件は workspace_id present。
3. **workspace_id present** — pane が `@mozyo_workspace_id` を持つ (live snapshot)。非 git でも scaffold/`workspace register` 済み workspace は持つため、git なし unit を壊さない。
4. **unambiguous target** — preflight projection が ambiguous でない。

`lane_id` / per-receiver foreground process allowlist (Step 12) / repo-cwd-branch 検査は **追加 hardening** であって minimal admission には含めない。これらを minimal 条件に格上げして git なし / 非 scaffold の unit test を壊さない。
...
- admission 条件を 1 つでも欠く (典型: workspace_id 無し) 場合、または activation policy が無効 (`--no-target-activation`、pre-#12597 の active-split fail-closed に戻す明示 escape) の場合は、従来通り `blocked` / `invalid_args` で fail-closed し、`build_inactive_pane_fallback_command` の strict-rail recovery (`--target %pane --target-repo auto --mode standard`) を durable record と die 文言に返す。typing も activation も行わない。
- die 文言は `pane_active` と unmet 条件 (または activation-disabled 理由) を含み、operator に「workspace を register する」「strict rail へ切り戻す」のいずれかを示す。

**設定駆動 (config boundary)**: activation / restore の policy は `domain/handoff.py` の `STANDARD_TARGET_ADMISSION_ACTIVATE_INACTIVE` / `STANDARD_TARGET_ADMISSION_RESTORE_PREVIOUS_ACTIVE` 定数を default とし、単一 seam `resolve_standard_target_admission_policy` で解決する。CLI override は `--no-target-activation` / `--restore-previous-active`。mode や policy を各 caller / wrapper に散在 hard-code しない (`notify-*` wrapper は forwarded namespace に未設定のため default に解決される)。
...
- **Pane active=0** (window 内で別 split が前面): v0.5 standard_target_admission (`#### standard_target_admission (v0.5)`) を評価する。minimal admission contract を満たし activation policy が有効なら rail が `select-pane` で active 化して通過する。満たさない (例: workspace_id 無し) か activation 無効化なら Step 11 で `blocked` / `invalid_args` (pre-#12597 と同じ fail-closed + strict-rail recovery)、typing 前死亡。
...
置き、collision-free launch generation と target identity を同一 window 内で照合する。causal start が
確認できなければ、`### Enter-Only Retry` の厳格 gate をすべて満たした場合だけ Enter を高々一度追加する。
本文は一度しか注入せず、tmux queue-enter の marker retry は変更しない。
...
delivery-ledger は queue rail のまま保たれ、causal event + coherent generation が欠ける結果は
`submitted_confirmed` ではなく `uncertain` である。ここで確認するのは turn start までであり、task completion
や ticket gate を確認するものではない。
...
- Herdr の既定 queue-enter は、本文1回・first Enter 1回を保ち、causal start 未確認時だけ厳格な同一 target / launch-generation / current-composer / screen / runtime-state gate の後に追加 Enter を高々1回許す。`busy` は queue 配送を拒否する理由ではないが delivery confirmation でもない。causal event と coherent generation が欠ける結果は `uncertain` であり blind retry しない。tmux queue-enter、herdr standard、pending はこの更新の対象外。
