...
| `submitted` | `wait_for_text(marker)` が true を返し、Enter を発行した。codex `--mode standard` では加えて Enter 後に受信 pane の新規出力活動 (turn 開始) を read-only 観測し、活動が観測できた場合にのみここへ倒す (v0.6 / #13166)。 | `sent` / `ok` |
| `submitted` (unconfirmed turn) | codex `--mode standard` rail で marker 観測 + Enter 発行後、観測 window 内に受信 pane の turn-start 活動が観測できなかった。Enter が busy / redraw composer に吸われた疑い。C-u rollback も再送も行わない (marker+body は一度だけ type 済み)。sender は受信 pane を再読して turn 開始を確認する (v0.6 / #13166)。 | `blocked` / `turn_start_unconfirmed` (新規 reason; v0.6 で追加) |
...
- `--mode standard` (**strict explicit fallback**): typing → `wait_for_text(marker)` → Enter。成功で `sent` / `ok` (`submitted`)。marker_timeout で `C-u` rollback + `blocked` / `marker_timeout` (`rolled_back`)。marker observation を Enter の必要条件とする strict rail を明示的に選ぶ送信用 (例: regression check、brand-new pane で queue-pickup 確率が未確認、observability test、strict landing evidence が監査要件)。v0.4 contract で default ではなくなったが contract からは削除しない。claude receiver への挙動は v0.1 以降変更しない。**codex receiver に限り v0.6 (#13166) で Enter 後の turn-start 検証を追加**: marker 観測 + Enter 発行後に受信 pane の新規出力活動を read-only 観測し、観測ありで `sent` / `ok` (`submitted`)、観測なしで `blocked` / `turn_start_unconfirmed` (fail-closed、C-u rollback も再送もしない)。上の Status / Reason mapping table と Status 節 v0.6 bullet を正本とする。
