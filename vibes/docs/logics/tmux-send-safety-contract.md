...
| `stage_failed` | typing 開始前に target resolution / agent gate / anchor / args validation が失敗。pane には何も typed されていない。envelope planning 段の execution-root 包含検証 (v0.9 / #14249) と、target-resolution 段の `--target-repo auto` frame 解決 (v0.10 / #14249 R2) もここに含まれる。 | `blocked` / `target_unavailable` / `target_not_agent` / `invalid_anchor` / `invalid_args` / `execution_root_outside_target_repo` (#14249 で新規) / `auto_target_repo_unresolved` (#14249 R2 で新規。store が runtime より新しい場合のみ `reader_upgrade_required` へ写す — v0.11) |
