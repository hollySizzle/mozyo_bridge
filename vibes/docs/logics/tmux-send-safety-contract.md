...
| `stage_failed` | typing 開始前に target resolution / agent gate / anchor / args validation が失敗。pane には何も typed されていない。envelope planning 段の execution-root 包含検証 (v0.9 / #14249) もここに含まれる。 | `blocked` / `target_unavailable` / `target_not_agent` / `invalid_anchor` / `invalid_args` / `execution_root_outside_target_repo` (#14249 で新規) |
