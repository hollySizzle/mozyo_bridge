...
- Draft: `v0.3` (durable policy; 実装 freeze ではない)
...
- 非射程の related work:
  ...
  - relaxed rail CLI / code 実装: Asana `1214782240686275` (本 contract に従って実装)
  - relaxed rail を反映する README / rules / skill refs / `CLAUDE.md` / packaged preset surface: Asana `1214782227597692`
  - relaxed rail の tests / smoke / workflow verification: Asana `1214782185227306`
  - relaxed rail の observability / durable annotation 形式化 (任意): Asana `1214782185308162`
...
- `--mode queue-enter` (v0.2 で contract 化、CLI / code 実装は Asana `1214782240686275` 所有): Claude / Codex agent pane に対する queue-oriented delivery 専用 rail。typing → `wait_for_text(marker)` を引き続き試みるが、未観測でも `C-u` rollback せず Enter を発行する。詳細・適用条件・許容ターゲット・durable wording は `## Relaxed Queue-Enter Rail` を正本として参照する。`--mode standard` の挙動はこの rail の追加によって一切変更しない。
...
`queue-enter` rail だけは条件 2 の「`wait_for_text` の `True` を Enter の必要条件とする」を緩める。ただし条件 1 (typing 前 capture-pane / read marker)、条件 3 のうち rollback 方針 (relaxed rail では `C-u` rollback はせず Enter を発行)、条件 4 (operator pending の durable 記録)、条件 5 (rendered text を ACK 正本にしない) はすべて維持する。relaxed rail の Enter 発行は「target が agent pane である」という別根拠を Enter の precondition として要求し、`reason="queue_enter"` で「pre-Enter landing 観測なし」を durable に明示する。これにより、relaxed rail でも未観測 Enter を「無条件 blind Enter」に倒さない。

これにより、`wait_for_text` の精度がどれだけ揺れても、strict rail では `submitted` 判定が揺れるだけで contract そのものは揺れず、relaxed rail では `submitted` (queue-enter) と `submitted (unobserved)` (queue-enter, marker 未観測) の wording 区別が揺れるだけで `Status` enum は揺れない。
...
`queue-enter` rail だけは「marker 未観測でも Enter を発行する」例外を contract として明示する。これは fail-open ではなく、別 rail として opt-in の上で別 outcome (`reason="queue_enter"`) を emit する設計である。詳細は次節を参照。

## Relaxed Queue-Enter Rail

v0.2 で追加した second rail。strict `--mode standard` の挙動は一切変更しない。本節が relaxed rail の正本。
...
strict `standard` rail はこの状況で `C-u` rollback + `marker_timeout` に倒すため、queue-oriented delivery が成立する余地を持たない。relaxed `queue-enter` rail は、この余地を sender が明示的に opt in したときに限り使えるようにし、`marker 未観測のまま Enter を押下した` という事実そのものを `reason="queue_enter"` として durable に記録する。

これは strict rail の弱化ではない。strict rail は marker observation を Enter の必要条件として保ったまま残す。relaxed rail は別 mode flag、別 outcome wording、別 next-action owner で完全に分離する。

### 適用条件 (Entry Conditions)

本節は relaxed rail の **selection 条件** (Layer A: sender が `--mode queue-enter` を選んでよい状況) を記述する。CLI 側の deterministic admission gate (Layer B) は `### Deterministic Preflight Admission Control` が正本であり、Layer B はここで列挙する descriptive な条件のうち machine-checkable な部分を Step ごとに固定する。

すべて満たした場合のみ relaxed rail を選択できる。1 つでも欠けたら `standard` に倒す。

1. sender が CLI flag (`--mode queue-enter` 提案、最終名は実装 task `1214782240686275` が決定) で明示的に opt in している。default は引き続き `standard`。
2. target が Claude / Codex agent pane と判定されている (`ensure_agent_target` が true)。`--force` を使って non-agent pane に強制 send することは relaxed rail では許容しない (strict より厳格)。v0.3 で Layer B はさらに per-receiver foreground process allowlist を要求する (`### Deterministic Preflight Admission Control` Step 12)。
3. durable anchor (`source` + `task_id` / `comment_id` などの組) が `orchestrate_handoff` の入力として揃っている。`mozyo-bridge message` の legacy marker shape では relaxed rail を提供しない (`message` は durable anchor を CLI が知らないため structured outcome を emit できない)。
...
- non-agent process が前面の tmux pane (たとえ `--force` で agent gate を bypass しても relaxed rail は使えない)
- ticket-system 通知用の write-only pane (anchor を持たないため)
- relaxed rail を「全 tmux target に対する universal Enter」として一般化することは禁止する

実装 task が target class を拡張したい場合は、本 contract の改訂を経由する。CLI 側で勝手に拡張しない。

### Marker Semantics

relaxed rail でも `wait_for_text(marker)` は引き続き呼ぶ。これは observability のためであり、Enter の発行可否を決める条件ではない (strict rail との決定的な差)。
...
- typing 自体が `send-keys` 系 transport error で失敗した場合: strict rail と同じく `delivery_failed` 経路。relaxed rail は transport error を覆い隠さない。

`wait_for_text` 実装は relaxed rail のために挙動を変える必要はない。Enter 発行ロジックだけが mode 分岐する。
...
既存の wire fields (`Status` enum / `AckStatus` enum / `next_action_owner` enum) は v0.2 で変更しない。relaxed rail のための拡張は `Reason` enum への新規値 1 つと、`mode` 文字列への新規値 1 つに留める。inspector projection は strict `sent + ok` と完全に同一であり、relaxed rail は inspector 側の解釈を一切変更しない。

### Durable Wording Requirements

`build_delivery_record` が relaxed rail 出力で必ず含める文面:
...
durable record の文面決定は実装 task `1214782240686275` の責務だが、上記 5 点を欠いた文面は本 contract に違反する。

### Failure Modes

relaxed rail 適用時にも以下は strict rail と同じ semantics を維持する。

- `stage_failed` 群 (`target_unavailable` / `target_not_agent` / `invalid_anchor` / `invalid_args`): typing 開始前 die。relaxed rail を選んでも pre-flight gate は strict と同じ。
- `delivery_failed`: typing 後の transport error。`C-u` rollback の試みは strict と同じ未定義のまま。relaxed rail はここを暗黙緩和しない。
- `marker_timeout`: relaxed rail では `marker_timeout` を `Reason` として **emit しない**。代わりに `queue_enter` を emit する。これは「marker miss」と「Enter rollback」を意味的に切り離すための明示的な分離。strict rail での `marker_timeout` は引き続き `rolled_back` を意味する。
- `pending_submit` の意図的経路 (operator が submit を保留する場合) は relaxed rail では選択不可。`--mode queue-enter` と `--mode pending` は排他であり、両方が同時に意味を持つ組み合わせは作らない。

### Deterministic Preflight Admission Control

v0.3 で追加。`### 適用条件 (Entry Conditions)` を「sender が opt-in したときに CLI が同時に守る machine-checkable な admission gate」として固定する。本節は実装 task `1214785367563471` が直接の実装対象とする仕様であり、CLI 側の admit / reject は本節の signal セットだけで決まる。
...
- **Layer A — selection (sender の判断)**: `--mode queue-enter` を選ぶかどうか。これは `CLAUDE.md` の運用規約 (「codex TUI のように marker が wrap される既知 receiver で `marker_timeout` を踏んだときだけ」) に従う sender 側の judgment であり、CLI からは検証できない。
...
本 contract が言う "deterministic preflight" は Layer B のことを指す。Layer A は本 contract の射程外であり、運用文書 (`CLAUDE.md` / `agent-workflow.md` / memory) が責任を持つ。Layer A の判断ミス (例: queue-enter を strict より優先して使ってしまう) は本 contract で防げない。本 contract は「Layer A で queue-enter を選んでしまった後でも、Layer B が deterministic に reject すれば、strict rail と同じ fail-closed が成立する」という保証だけを提供する。
...
queue-enter は generic な `ensure_agent_target` (= `is_agent_process(command)` が真) より厳格な per-receiver allowlist を要求する。ただし以下の **明示的な弱さ** を持つ: 受信側 identity を確証できるのは literal basename の場合だけで、versioned native binary basename (例: `1.0.32-arm64`) の場合は receiver-agnostic regex でしか判定できないため、agent-vs-shell の区別だけが残り、cross-binding 検出力は Step 9 (window-name binding) と operator discipline (Layer A) に retreat する。本節はこの弱さを正直に契約し、過剰な保証を作らない。

| Receiver | Allowed `pane.command` basename | identity 確証度 |
| --- | --- | --- |
| `claude` | literal `claude` または `node` | **strong** — basename が claude / node のいずれか literal で前面なら、receiver と一致していると Layer B が断言できる (Claude Code TUI は実体が Node ランタイムなので `node` の許容は必要)。 |
| `claude` | `VERSIONED_NATIVE_BINARY_RE` (`\d+\.\d+\.\d+(?:[-+].*)?`) にマッチする basename | **weak** — 同じ regex は codex 側でも許容するため、Step 12 単体では「claude が前面である」を確証できない。agent process が前面であることだけを確認し、receiver identity は Step 9 の window-name binding と Layer A の operator discipline (受信 window を勝手に rename しない、claude window に codex を起動しない) に依存する。 |
| `codex` | literal `codex` | **strong** — 同上の理由で receiver と一致していると Layer B が断言できる。 |
| `codex` | `VERSIONED_NATIVE_BINARY_RE` にマッチする basename | **weak** — claude 側と同じ regex のため Step 12 単体で receiver identity を確証できない。Step 9 + Layer A に retreat する。 |
...
- literal basename が **他 receiver の literal basename と一致する** (例: receiver=`claude` で前面が literal `codex` process、receiver=`codex` で前面が literal `claude` or `node` process) — generic `ensure_agent_target` は通すが、Step 12 は cross-binding として **reject する**。これが Step 12 の strong identity ケースの典型用途。
- agent process basename が `is_agent_process` を満たすが上記 receiver 行のいずれにも一致しない場合 — reject。

明示的な弱点 (versioned native binary の cross-binding): Claude Code と codex の両方が将来同じ shape の versioned native binary (例: `1.0.32-arm64`) で配布された場合、Step 12 単体ではどちらの receiver か区別できない。このケースでは:

- Step 9 (`window_name == receiver`) と Step 10 (same-session binding) が一次防御線として残る。operator が `mozyo` startup と `mozyo-bridge init <agent>` の規律を守る限り、window 名は receiver と一致し続けるため、versioned native binary が前面でも誤投入は起きない。
- 一方で、operator が `tmux rename-window` などで window 名を勝手に書き換えた場合、Step 9 はそれに合わせて admit してしまうため、versioned native binary 上では Step 12 でも cross-binding を検出できない。本契約はこのケースを Step 12 単体では検出不可と明記し、Layer A の運用規律 (window 名を agent identity と乖離させない) と将来の追加 signal (Open Question 8) に委ねる。
...
逆に、Layer A の運用ルールで「queue-enter を default にしない」「strict が成功する状況では strict を選ぶ」を `CLAUDE.md` / memory に固定し続けることが、Layer B の決定論と組み合わさって queue-enter rail の安全性を実用上担保する。Layer A の規律は本 contract の `### 強い境界 (Strong Boundaries)` 節および root `CLAUDE.md` が引き続き責任を持つ。

### 強い境界 (Strong Boundaries)

- strict `standard` rail の挙動は relaxed rail 追加によって一切変更しない。`wait_for_text` の戻り値が False のとき strict rail は引き続き `C-u` rollback + `marker_timeout` で fail-closed する。
- relaxed rail を default にしない。CLI flag は引き続き `standard` を default に保つ。
- relaxed rail を universal pane behavior として拡張しない。許容 target は本節で enumerate した agent class に限る。
- relaxed rail の追加によって receiver runtime / receiver-state inspector の semantics は変更しない。`AckStatus` enum、`runtime_phase` 関連、`process.exited` 関連はすべて触らない。`project_last_input` mapping への追加 (`("sent", "queue_enter")`) は strict `("sent", "ok")` と **完全に同一の projection** (`submitted_at = outcome timestamp / ack_status = "submitted"`) を採る。これは上流 `receiver-state-inspector-contract.md` が `ack_status` を `submitted_at` / `acknowledged_at` から derive すると規定しており、`submitted_at` 既知のとき `ack_status="unobserved"` を返すことが構造上不可能であるため、また同 spec の Capability Matrix が tmux compat でも `submitted_at` を populate することを明示しているためである。queue-enter rail の wording-layer 差 (`reason="queue_enter"` / 別 narrative) は `DeliveryOutcome` と durable record にだけ存在し、inspector projection に流出させない。`acknowledged` を仮装することは引き続き許さない (relaxed rail でも `acknowledged_at` は populate しない)。
- relaxed rail を runtime completion / task completion の判定に使わない。pane queue 受理は task 完了の signal ではなく、durable record (Asana task comment / Redmine journal) のみが完了の正本である点は strict と同じ。
- 本 contract が定義していない名称 (例: `--mode relaxed`、`--force-enter`、`--unsafe-send`) で同等の挙動を別 CLI surface に実装することは禁止する。relaxed rail は本 contract の `queue-enter` mode 経路に統一する。
...
- `cloud.md` は本 repository に存在しないことを確認済み (`find . -name cloud.md` 0 件、2026-05-14 時点)。relaxed rail を文書化する router/distribution surface は `CLAUDE.md` (root) と `src/mozyo_bridge/scaffold/presets/asana/CLAUDE.md` / `AGENTS.md` / `agent-workflow.md`、および `skills/mozyo-bridge-agent/references/safety.md` / `plugins/mozyo-bridge-agent/skills/mozyo-bridge-agent/references/safety.md` が候補となる。
- これらの surface 編集は Asana `1214782227597692` が所有する。本 task では実装しない。
- `README.md` 側の `--mode` 説明追加も `1214782227597692` が所有する。本 task では touch しない。
- relaxed rail の test / smoke / workflow verification 反映は Asana `1214782185227306` が所有する。本 task では追加しない。
- relaxed rail の observability / durable annotation 形式化 (任意) は Asana `1214782185308162` が所有する。本 task では着手しない。
...
queue-enter rail について (Asana `1214782240686275` 実装 task が追加する想定の対応; 本 task では実装しない):
...
8. v0.3 Step 12 の versioned native binary cross-binding gap (Codex audit `1214785691963721` 起源)。`VERSIONED_NATIVE_BINARY_RE` は receiver-agnostic なので、両 receiver が同 shape の versioned native binary で配布された場合、Step 12 単体では receiver identity を確証できず Step 9 + Layer A 運用規律に retreat する。これを閉じるための追加 signal の候補 (例: pane title への receiver marker 埋め込み、receiver-installed binary path prefix 検査、wrapper 経由の sentinel 出力) と、それぞれの operator burden / install assumption trade-off は別 task で評価する。本 contract の v0.3 では「弱点を明記して契約する」までを射程とし、closure は contract 改訂を経由する。
...
- strict `--mode standard`: blind Enter は許容しない。typing 前の `capture-pane` 観測 + landing marker `wait_for_text` 成功を Enter の必要条件にし、marker 未観測時は `C-u` rollback で `rolled_back` に倒す (v0.2 でも一切変更しない)。
- relaxed `--mode queue-enter` (v0.2 で新設): Claude / Codex agent pane に対する queue-oriented delivery 専用の opt-in rail。typing 前 `capture-pane` 観測は引き続き必要、agent gate も strict より厳格 (`--force` 不可)。`wait_for_text` は呼ぶが、観測の有無に関わらず Enter を発行する。観測あり → `sent` / `ok`、観測なし → `sent` / `queue_enter` (新規 reason)。`Status` enum / `AckStatus` enum / `next_action_owner` enum は変更せず、`Reason` enum 1 値と `mode` 文字列 1 値の追加だけで wording-layer を表現する。`next_action_for("sent", "queue_enter", receiver)` は strict `sent` と同じ `receiver` owned。`project_last_input` は strict `("sent", "ok")` と完全に同一の `submitted_at = outcome timestamp / ack_status = "submitted"` を採る (上流 inspector contract が `ack_status` を `submitted_at` / `acknowledged_at` から derive する規定に従う; `submitted_at` を null に倒すと `pending_input/ok` と区別できなくなり、また同 spec の Capability Matrix が tmux compat でも `submitted_at` populate を明示しているため)。queue-enter rail の wording-layer 差は `DeliveryOutcome.reason` と durable record narrative にだけ存在し、inspector projection には流出させない。
- pane への notification が届かないことを失敗とは扱わず、durable record (Asana task comment / Redmine journal) を正本とする。`message --no-submit` は codex TUI gate fallback として retries 3 / Asana 試行記録の手順を memory に従って運用する。長期は `--mode queue-enter` への統一が候補 (Open Question 5)。
- 短期 wrap 補修 (`1214765093829972`) と gate compatibility tracker (`1214749106025548`) は本 contract と独立に進む。本 contract は `wait_for_text` の精度向上に依存しない。relaxed rail も `wait_for_text` 改修と独立。
- 上流 ACK spec の現行 mapping (`sent` ↔ `submitted` / `pending_input` ↔ `pending_submit` / `blocked + marker_timeout` ↔ `rolled_back` / `blocked + target_*/invalid_*` ↔ `stage_failed`) を本 contract の正本マッピングとして固定する。v0.2 で追加するのは `sent + queue_enter` ↔ `submitted (queue-enter, marker unobserved)` 1 行のみ。v0.3 は wire enum を一切変更せず、`### Deterministic Preflight Admission Control` で queue-enter 専用の machine-checkable な admission gate (per-receiver foreground process / same-session / `pane_active == "1"`) を追加し、reject はすべて既存 `blocked + invalid_args` / `blocked + target_not_agent` / `blocked + target_unavailable` に集約する。Step 12 (per-receiver foreground process allowlist) は literal basename (`claude` / `node` / `codex`) では strong cross-binding 検出を提供するが、versioned native binary basename (`VERSIONED_NATIVE_BINARY_RE` match) では receiver-agnostic regex のため Layer B 単体で receiver identity を確証できず、Step 9 + Layer A の operator discipline に retreat する弱点を明記して契約する (Open Question 8)。
