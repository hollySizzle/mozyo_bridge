...
- 親 project coordinator と子 project の owning US / audit coordinator が別 lane の場合は、
  ...
  `Delegated callback targets` として `delegation_parent` と
  `owning_us_coordinator` / `audit_coordinator` を分けて記録し、required target 全件の
  callback outcome を journal に残す。単一 parent callback だけでは
  multi-coordinator adoption smoke の PASS にしない。
...
- 親 project coordinator と owning US / audit coordinator が別のとき、片方だけに callback
  して PASS としない。`delegation_parent` と `owning_us_coordinator` /
  `audit_coordinator` の required callback outcome を両方確認する。
