...
presentation:
  version: 1
  delegation_window_policy: shared   # shared (既定、#13085) | separate (opt-in)。window 分離方針 (#12467/#13015)
  project_groups:
...
: `shared` (既定、#13085) | `separate` (opt-in)。window 分離方針 (#12467)。表示語彙と
  display projection の正本は `delegated-coordinator-cockpit-display.md` `## window
  分離方針`。#13015 で launcher placement にも接続された: opt-in の `separate` の
  下では faithful `project_group_tmux_window` 実行時に sublane (非 default lane) を
  ...
  既定の `shared` では sublane は単一 project/common host window に留まる。
...
解決しない。path構成文字として合法な空白はtrimしない。canonical livenessは既知fieldを
型検査してfingerprint入力へ含めるため、
