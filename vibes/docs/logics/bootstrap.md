...
Stage 0 deliberately runs no backend probe. Which backend applies is a property
of the project's config, and the tools that read it correctly do not exist
until Stage 1 — a probe here could only guess, and a guess that checks the
wrong backend reports a failure the host does not have (Redmine #15517).
...
- TaskPilot startup menu for mozyo (Redmine #11406): keep the menu in a **User-level** `task-menu.yaml` referenced from VS Code User settings `taskPilot.configPath`, not a per-workspace `.vscode/task-menu.yaml`; otherwise `mozyo-bridge init` behavior updates do not propagate and the menu drifts. TaskPilot requires an **absolute** path in `configPath` (VS Code variables such as `${userHome}` are not expanded; a non-absolute value is joined with the workspace root and fails to find the User-level menu), so document a real absolute path with a `<user>` placeholder, e.g. `/Users/<user>/Library/Application Support/Code/User/task-menu.yaml` (Linux `/home/<user>/.config/Code/User/task-menu.yaml`, Windows `C:\Users\<user>\AppData\Roaming\Code\User\task-menu.yaml`). The startup snippet must not hardcode an operator repo path — resolve it as `repo="$(pwd -P)"` from the TaskPilot shellCommand cwd. See the README section `TaskPilot 起動メニューを User settings で共有する` for the full reference menu. Key mechanics:
  - detect the new tmux window by diffing `#{window_id}` snapshots taken before/after `tmux-integrated.newTerminal`, searching across **all** sessions (`list-windows -a`) and restricting candidates to the expected session or an underscore fallback session (`^_+$`) plus a shell pane (`zsh|bash|fish`);
  - before `mozyo-bridge init claude|codex <pane>`, send `cd -- <repo>` to the target pane and wait for `#{pane_current_path}` to equal `<repo>`, because smart `init` derives the workspace root and expected session from the pane cwd;
  - smart `init` then renames an underscore fallback session into the expected session, pins `tmux-integrated.sessionName`, and renames the window — so the menu need not call `session vscode-settings --write` separately;
  - a stale `taskPilot.globalMenu` mozyo entry can shadow/duplicate the `configPath` menu during rollout: remove it or give it the same label so the `configPath` menu overrides it.
