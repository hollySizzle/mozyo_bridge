...
- v1 は **`0.13.0` で deprecated**。以後 **最低 2 minor**（`0.13.x` / `0.14.x`）parseable を維持する。
- deprecation の **public surface = `mozyo-bridge config status`**（read-only）。v1 で migratable な
  provider-keyed 内容を持つ config は 0.13.0 deprecated / 0.15.0 earliest removal / `config migrate`
  next action を actionable に表出し、v1-unrelated-only / v2 / missing では silent。warning は
  secret-free（schema が credential-shaped field を禁止）。正本は `RepoLocalConfig.deprecation_warnings()`。
- 同 surface の per-key source 分類（#14223 / #14222 j#85125 F3）: top-level block と curated leaf
  path（`repo_local_config_status.CONFIG_LEAF_KEYS`）を `declared` / `default` / `compatibility` の
  3-source で分類する。`compatibility` = v1 legacy 宣言（`agent_launch` / `provider_binding`）と
  そこから派生する effective `agents` topology。各 row は machine-readable な `action` token
  （`config_migrate` / `schema_integration_pending`）を持ち、部分宣言 block の未宣言 leaf は
  block の `declared` に埋没しない。正本は `repo_local_config_status.py`。
