...
したがって `OOP-first architecture and static typing` roadmap group 以降の architecture work は
**OOP-first** を基本方針にする。Redmine Version 名は planning axis であり、package release
...
`OOP-first architecture and static typing` roadmap group 以降の実装 tranche では、まず巨大 orchestration surface を object boundary に分ける。
候補は次の順で扱う。
...
mozyo_bridge は `OOP-first architecture and static typing` roadmap group 以降、architecture work の基本姿勢を OOP-first とする。
