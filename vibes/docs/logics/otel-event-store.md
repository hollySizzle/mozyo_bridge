...
- key 受け渡し (#12306): launchd 配下の receiver は shell env を継がないため、Redmine API key/URL は **home-scoped credential file** `${MOZYO_BRIDGE_HOME:-~/.mozyo_bridge}/redmine-credentials.yaml` (`redmine: {url, api_key}`) で渡す。実装は `src/mozyo_bridge/redmine_credentials.py` (`resolve_redmine_credentials`)。解決順は **env が先 (interactive `otel serve`)、file が後 (launchd)** の per-field fallback。file は **home-scoped / user-owned で repo-local ではない** ため #56232 の credential boundary (送信先を repo-local file が選べない) を維持する (workspace-defaults は引き続き identifier + host-match のみ供給)。fail-closed: group/other 読み書き可・他 user 所有・regular file でない credential file は **読まずに拒否** し redacted warning (path / 8進 mode のみ、値は出さない) を launchd log へ出す。missing/malformed は従来どおり visible に `unconfigured` へ縮退、invalid key は fetch 経由で `unavailable`。key 値は repr / warning / payload / log / journal に出さない (test で pin)。keychain は将来 option として deferred (attack surface 最小化のため本 pass では未実装)。他 2 層 (OTel activity / tmux) は影響なし。
