...
- child が grandchild へ dispatch する時点では、grandchild が読む Redmine issue / journal
  ...
  `return_blocked(reason="redmine_anchor_missing")` を parent へ返す。
...
Operator -> Grandparent: ticketless_consultation(text, constraints)
activate Grandparent
Grandparent -> Grandparent: classify_ticketless_consultation(request,\nallowed_reads="routing_metadata_only")
...
alt classification_ambiguous
  Grandparent --> Operator: return_blocked(reason="classification_ambiguous")
else classified
  Grandparent -> Grandparent: resolve_project_gateway(repo_root,\nproject_scope, role="codex", target_kind="project_gateway")
  alt gateway_missing
    Grandparent -> Grandparent: start_project_gateway(repo_root,\nproject_scope, projection)
  else gateway_target_ambiguous
    Grandparent --> Operator: return_blocked(reason="gateway_target_ambiguous")
  else gateway_resolved
    Grandparent -> Parent: handoff_to_project_gateway(request,\ntarget=parent, kind="ticketless_consultation")
  end
end

activate Parent
Parent -> Parent: decide_implementation_need(request,\nallowed_reads="project_domain_metadata")
...
alt implementation_not_needed
  ...
  Parent -> Redmine: ensure_redmine_anchor(reason="implementation_required")
  alt redmine_anchor_missing
    Parent --> Grandparent: return_blocked(reason="redmine_anchor_missing")
  else anchor_ready
    Parent -> Parent: resolve_or_start_delegated_coordinator(anchor,\nrole="delegated_coordinator", target_kind="implementation_gateway")
    Parent -> Child: handoff_to_child_coordinator(anchor,\nkind="implementation_request")
  end
end

activate Child
Child -> Redmine: read_issue_and_required_docs(anchor)
Child -> Child: decide_grandchild_dispatch(anchor,\npurpose="preserve_coordinator_context")
alt no_grandchild_dispatch
  Child -> Redmine: record_no_grandchild_dispatch_reason(anchor)
  Child --> Parent: callback_to_project_gateway(anchor, state="blocked_or_no_dispatch")
else grandchild_dispatch
  Child -> Child: resolve_or_start_implementation_worker(anchor,\nrole="implementation_worker")
  Child -> Grandchild: dispatch_redmine_anchored_worker(anchor)
  activate Grandchild
  alt anchor_missing
    Grandchild --> Child: return_blocked(reason="redmine_anchor_missing")
    Child --> Parent: callback_to_project_gateway(anchor, state="blocked")
  else anchor_exists
    Grandchild -> Grandchild: execute_redmine_governed_work(anchor)
    Grandchild -> Redmine: record_implementation_done(anchor)
    Grandchild -> Redmine: record_review_request(anchor)
    Grandchild --> Child: callback_to_child_coordinator(anchor,\nstate="implementation_done_or_review_request")
    Child --> Parent: callback_to_project_gateway(anchor,\nstate="implementation_done_or_review_request")
  end
  deactivate Grandchild
end
deactivate Child

Parent --> Grandparent: callback_to_grandparent(anchor,\nstate="project_gateway_result")
deactivate Parent
Grandparent -> Redmine: record_transition_result(anchor)
...
classify_ticketless_consultation(...)
  ...
  help_contract: allowed action / forbidden probe / required durable anchor を error message で返す
...
resolve_project_gateway(...)
  ...
  help_contract: repo_root / project_scope / role / target_kind の不一致と複数候補を fail-closed reason として返す
...
start_project_gateway(...)
  ...
  help_contract: project_scope / project_path / project_label の stamp 結果と既存 unit focus 結果を返す
...
dispatch_redmine_anchored_worker(...)
  ...
  semantic_contract: source=redmine, kind=implementation_request, target_role=implementation_worker, callback target required
...
resolve_or_start_delegated_coordinator(...)
  ...
  semantic_contract: launch/adopt policy, target repo, parent route, parent/child issue, source journal を入力にする
...
handoff_to_child_coordinator(...)
  command_status: existing
  command_family: mozyo-bridge handoff send
  semantic_contract: source=redmine, kind=implementation_request, target_role=delegated_coordinator, parent/child project identity required
...
decide_grandchild_dispatch(...)
  ...
  semantic_contract: delegated coordinator enabled, grandchild dispatch enabled, max depth, owning route, source journal, target repo or no-dispatch reason
...
resolve_or_start_implementation_worker(...)
  ...
  semantic_contract: lane identity chain, grandchild unit, realization mode, parent/child issue relation
...
callback_to_child_coordinator(...)
  ...
  allowed_kinds: implementation_done, review_request, reply
  semantic_contract: source=redmine, issue/journal anchor, state pointer summary
...
callback_to_project_gateway(...)
  command_status: existing
  command_family: mozyo-bridge handoff send
  allowed_kinds: implementation_done, review_request, review_result, reply
  semantic_contract: target=parent project gateway, source=redmine, issue/journal anchor, state pointer summary
...
callback_to_grandparent(...)
  ...
  command_family: mozyo-bridge handoff send
  allowed_kinds: implementation_done, review_request, review_result, reply
  semantic_contract: target=grandparent coordinator, source=redmine, issue/journal anchor, state pointer summary
