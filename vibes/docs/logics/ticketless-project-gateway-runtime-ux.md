...
| 親 | `parent` | `project_gateway` | `project_gateway` | grandparent から受けた相談を受領し、project identity と child coordinator route を交通整理する。Redmine anchor 要否と domain/design 判断は child coordinator が所有する。 | parent 自身による design answer、project-domain docs research、official/web research、local probe、直接調査・実装、rclone / Drive 実操作、Redmine anchor なし domain probe、grandchild worker への direct implementation send。 | domain/design 判断、pilot、implementation、または worker dispatch が必要な相談でも、parent は anchor を作らず child coordinator へ ticketless no-anchor work-intake として渡す。Redmine issue / journal anchor の作成・選択は child coordinator が所有する。 |
| 子 | `child` | `delegated_coordinator` / `implementation_gateway` | `delegated_coordinator` | parent から ticketless no-anchor work-intake を受領し、作業化の要否を判断して必要なら Redmine anchor を作成/既存選択し、その anchor を読み、実装 scope を分解し、grandchild lane dispatch と callback aggregation を行う。 | owner close approval の代行、parent issue の close、release / publish / credential / destructive approval、parent への callback なしの完了扱い。 | child coordinator が Redmine anchor 要否を所有する。作業化が必要なら Redmine issue / journal anchor を作成または既存選択し、不要なら durable reason を記録する。framing / 権限 / project identity 不足で anchor を確保できない場合のみ `anchor_required` / `blocked` を parent 経由で返す。grandchild dispatch には anchor が必須。 |
...
禁止:
  ...
  - id: parent_implements_directly
    ...
    action: child_coordinatorへticketless work-intakeとして渡す（anchor作成・選択はchildが所有）
