...
#12737 の正本である。GK3500 fixed rerun #12709 で、route/adopt が green になり project
gateway が feasibility / anchor-required 境界を安全に判断したにもかかわらず、結果を自身の
pane に「Root coordinator callback」回答として書いただけで、product callback primitive を
自律的に呼ばなかったことが観測された。caller lane は structured な ticketless callback を
受け取れなかった。本節はこの欠陥を contract として固定する。
