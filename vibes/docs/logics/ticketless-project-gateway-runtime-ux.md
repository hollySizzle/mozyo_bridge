...
- parent は project-domain research、official/web research、local probe、rclone / Drive 操作、
  ...
  判断が必要な相談を受けた場合は、実行前に `ensure_redmine_anchor` を通し、
  child coordinator へ橋渡しする。anchor を作れない場合は `anchor_required` / `blocked`
  を grandparent へ返す。
...
禁止:
  ...
  - id: parent_absorbs_domain_design_decision
    ...
    action: Redmine anchor確保後にchild_coordinatorへdispatchする。anchorを作れない場合はanchor_requiredを返す
...
- Redmine anchor が既にあるか、または child coordinator へ渡すために anchor が必要かを判定する。
- child coordinator を semantic route で起動 / 採用 / handoff する。
- anchor が無い場合は `anchor_required` / `blocked` を grandparent へ返す。
...
- `anchor_required`: domain/design 判断、pilot、implementation、または worker dispatch に進むには
  Redmine issue / journal anchor が必要。
- `blocked`: route 不能、project identity mismatch、anchor 作成不能、target ambiguous。
- `consultation_result`: domain answer ではなく、上記 workflow state を grandparent に返すための
  transport classification としてのみ使う。
...
alt route_or_anchor_blocked
  Parent --> Grandparent: hands_off_consultation_result(blocked_or_anchor_required)
  note right of Parent
    fields: classification, redmine_anchor_required,
    next_action_owner
  end note
else child_coordination_required
...
| 親 -> 子 | delegated coordinator adoption + lane handoff | 必須。parent が domain/design 判断を要する相談を受けた時点で Redmine issue / journal anchor を作成または選択する。 | parent は直接調査・設計回答・実装判断をせず、Redmine anchor と project identity を保持したまま child coordinator へ橋渡しする。 | Redmine anchor missing、parent direct design answer / investigation / implementation、grandchild への direct send、child coordinator の route identity 不一致、Claude 誤送信。 |
...
| 親 -> 祖父 | project gateway result callback / ticketless hands-off | anchor_required / blocked では不要。child へ進んだ場合は child 結果 anchor が必須。 | parent が workflow state (`blocked` / `anchor_required` / child handoff result pointer) を grandparent へ返し、grandparent が transition result を記録する。 | pane 上の回答だけで停止する、parent が domain/design answer を直接返して相談を閉じる、parent が実装結果を保持したまま祖父へ返さない、callback transport missing を blocked として返さない、owner / grandparent へ work log を直接貼って durable anchor を欠く、親が close approval / review authority を代行。 |
...
- project gateway が traffic-control actor として consultation を受け取り、domain/design 判断を
  自身で吸収しない。
- domain/design 判断、pilot、implementation dispatch が必要な相談では、child coordinator へ
  渡す前に Redmine issue / journal anchor を作成または要求する。
- parent project gateway が直接調査・設計回答・実装判断をせず、Redmine anchor 付きで child coordinator へ橋渡しする。
