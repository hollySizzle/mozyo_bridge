...
- **lane role 分類は divergent model を作らない (設計原則 4)。** tmux state machine と同じ
  ...
  authority を持たない (default-lane の `coordinator_assistant` は worker ではなく、attested
  workflow role や現在の provider binding も routing authority ではない)。よって **非 default lane slot のみ** を lane-local class として分類し、default lane は
  fail-closed する:
