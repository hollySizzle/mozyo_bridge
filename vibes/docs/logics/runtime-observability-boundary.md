...
runtime_observation_snapshot:
  ...
  source: redmine | tmux | herdr | otel | sidecar | managed_event | cache | other
...
- `source: herdr` (#13355) は herdr terminal-runtime backend の live `agent list`
  inventory snapshot (`method: live_query`、strong_runtime_signal)。tmux liveness と同格の
  layer-1 runtime observation であり、workflow truth を含意しない。herdr は現状 snapshot
  read (`agent list` / `agent get`) のみを公開するため push 型 event feed は defer
  (upstream event-subscription API 待ち)。repo-local config が herdr backend を選択して
  いない場合、`observe reload --source all` は herdr snapshot を含めない (tmux 面
  byte-invariant)。
