...
## #12639 Application Facade Retirement

US #12639 (parent Feature #12533、Version #276) は、#12624–#12628 expansion が
`src/mozyo_bridge/application/` 配下に残した `sys.modules` compatibility facade 35 件を、
#12631 / #12632 の `domain/` / `infrastructure/` facade retirement と同一手順で除去した。
top-level technical-layer residual を最終形に残さない方針 (top-level `domain` /
`infrastructure` の消滅) の application への適用であり、behavior-preserving。

`fallback-retirement-ledger.md` Retirement Process に沿って実施した:

- **caller inventory**: active source/tests の facade import を棚卸し (両 import 形
  `from mozyo_bridge.application.<mod> import ...` / `from mozyo_bridge.application import <mod>`、
  および dotted patch-string 参照を含む)。docs / catalog / pyproject / generated は facade path を
  参照していないことを確認。
- **source-of-truth 明記**: 各 facade は同一 module object を numbered Feature path
  (`e_<order>_<epic>/f_<order>_<feature>/application/<mod>.py`) へ再束縛していたため、
...
物理 dir `src/mozyo_bridge/application/` は **存続する** (`commands.py` /
`cli.py` (`cli:main` entry) / `cli_core` / `cli_common` / `commands_common` / `doctor*` 等の
real composition-root module を引き続き host するため)。`application/__init__.py` も存続する。
本 US は上記 Fixed 表の `application/commands.py` / CLI composition root 行を move しない
(composition root は固定、retirement 対象は facade alias のみ)。`commands.py` 内部の OOP 分解は
別 US (本 US 範囲外)。
