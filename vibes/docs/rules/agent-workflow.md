...
- 送信側は 2 本の rail から正しいものを選ぶ。default を黙って弱化しない。
  ...
  - opt-in な relaxed rail (`mozyo-bridge handoff send --mode queue-enter`、Claude / Codex agent pane 限定、`--force` 不可): typing 前に deterministic preflight が走り、すべて pass したときだけ marker 未観測でも Enter を発行して `sent` / `queue_enter` という別の durable outcome を emit する。silent な strict 成功には倒さない。preflight は (a) explicit `--target` は receiver の tmux window 配下、(b) target pane は sender と同じ tmux session、(c) target pane は所属 window の active split、(d) foreground process が receiver の allowlist (`claude` literal は claude-strong、`codex` literal は codex-strong、`node` literal と `versioned-native-binary` basename は両 receiver で weak admit) を要求する。1 つでも false なら typing 前に `blocked` で die する (`Reason` は `invalid_args` / `target_not_agent` のいずれか)。codex TUI の wrap-shape 観測差 (Asana `1214749106025548` / `1214765093829972`) など receiver TUI が marker を wrap してしまう既知ケースのときだけ選ぶ。
