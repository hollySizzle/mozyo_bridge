...
- delivery rail dogfood、pre-smoke gate、release-adjacent runtime verification では runtime fingerprint を durable record に残す。`mozyo-bridge --version` だけを根拠に「最新」と判断しない。bare CLI / source CLI / installed CLI の区別、executable path、imported package path、version string、依存 feature probe (例: `standard_target_admission` present) を記録する。fingerprint が期待 behavior と一致しない場合は `blocked` / `environmental` とし、#12546 などの PASS evidence に混ぜない。local bare / pipx runtime alignment は source-runtime smoke green 後の別 Task とし、owner / operator approval なしに dogfood / smoke 中の pipx reinstall、local install、tag、publish、version bump で PASS を作らない。詳細は `vibes/docs/logics/tmux-send-safety-contract.md` の `### Runtime Fingerprint Gate` を参照する。
...
- Version は、関連 issue が複数 Feature / UserStory に分かれる場合の roadmap grouping surface でもある。同じ stabilization、UX 改善、dogfooding batch、acceptance batch に属する work package は、親子関係を無理に寄せず同じ Version に割り当てて束ねる。Version は親子関係の代替ではなく、進捗・残 scope・release readiness を横断して見るための planning axis である。
- 現行 Redmine-only 運用では、bucket は Redmine Version / issue `fixed_version` で管理する。ここでいう bucket は planning bucket、execution bucket、conflict bucket、acceptance window / run window を含む横断 grouping であり、UserStory を bucket 代わりの親 issue として追加しない。UserStory は価値・受け入れ・review / owner close の単位として守る。
- Redmine Version は、必要に応じて **execution bucket / conflict bucket / acceptance window** として切る。並列開発を速く進めるため、同じ file surface、runtime invariant、handoff route、cockpit projection、workflow rail を触る issue は同じ bucket に寄せ、衝突しにくい issue は別 bucket に分ける。機能分類として綺麗な tree になることより、同時実行時の conflict / merge order / smoke readiness が読めることを優先する。
- Version bucket を実行整理に使う場合でも、roadmap を押し出して不可視化しない。Version 名・description・関連 container issue / journal に、その bucket が planning / execution / conflict / acceptance のどれを主目的にするか、押し出した scope がどの Version に残るかを残す。Version 名だけで「後で扱う」と宣言しない。
- Smoke / acceptance / real-machine rerun は実装 blocker と同じ bucket に混ぜず、最後に実行する acceptance window / run window として分ける。実装 blocker が残る間は blocker issue を実装 bucket に置き、smoke issue は run window に残す。
- 親 UserStory が umbrella として複数 bucket にまたがる場合、親へ fixed_version を一括 propagation しない。子 issue の fixed_version を実行 bucket の正本として読み、親 issue には umbrella であることと子 bucket の判断を journal / description に記録する。
- Redmine Version 名に将来の package release 番号 (`v0.10.x` など) を先入れしない。Version は作業テーマ / planning bucket / execution bucket / conflict bucket / acceptance bundle の名前であり、package version の正本ではない。
...
- 既存の番号付き Version 名は歴史記録として残してよいが、新規 planning bucket 作成時は semver 風の番号を避ける。番号付き Version を改名する場合は、参照 issue / journal / roadmap への影響を Redmine に記録し、release / tag / publish / version bump とは別作業として扱う。
- 将来 `execution_bucket` / `lane_set` 相当の Redmine custom field や workflow DB が整備された場合、実行 bucket identity はその surface へ分離してよい。ただしその移行が Redmine issue / journal と規約に記録されるまでは、Version / fixed_version を bucket 正本として扱う。
