...
- 詳細・state machine 全体・例外条件・transient gap (CLI binary default flip と contract default の整合) の正本は `vibes/docs/logics/tmux-send-safety-contract.md` の `## Default Delivery Promise (v0.4)` / `## Queue-Enter Default Rail` 節を参照する (重複させない)。
