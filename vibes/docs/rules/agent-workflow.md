...
- delivery rail dogfood、pre-smoke gate、release-adjacent runtime verification では runtime fingerprint を durable record に残す。`mozyo-bridge --version` だけを根拠に「最新」と判断しない。bare CLI / source CLI / installed CLI の区別、executable path、imported package path、version string、依存 feature probe (例: `standard_target_admission` present) を記録する。fingerprint が期待 behavior と一致しない場合は `blocked` / `environmental` とし、#12546 などの PASS evidence に混ぜない。詳細は `vibes/docs/logics/tmux-send-safety-contract.md` の `### Runtime Fingerprint Gate` を参照する。
