...
Redmine #13148。distributed / repo-local 軸 (上記) と public / private 軸 (`rule-public-private-boundary`) に加えて、規則には **activation 軸** がある。同じ正本規則でも「いつ適用されるか」で到達性の要件が変わる。activation 軸は distributed / repo-local 軸と直交する (どの layer に正本を置くかは activation では決めない — 配置判断ルールに従う)。

| activation | 定義 | 到達性の要件 |
| --- | --- | --- |
| always | task 種別と無関係に毎 turn 適用される規則 | router (AGENTS.md / CLAUDE.md) の最小 reminder に **生成された 1 行 digest + 正本 pointer** として常時載る。skill を load しない session でも構造的に破られない |
| per-task | 特定の作業種別 (handoff / review / release / scaffold 等) に入ったときだけ適用される規則 | on-demand 層 (skill body / preset / cataloged doc) に正本を置き、その作業に入る agent が読む。router には載せない |
| exceptional | 例外・エスカレーション・稀な復旧手順 | 該当 doc / preset に正本を置き、発生時に参照する。router にも digest にも載せない |

### always 認定基準

規則が always かどうかは **1 点だけ** で判定する: 「task 種別と無関係に毎 turn 適用されるか」。yes なら always、no なら per-task か exceptional。この基準は narrow に保つ — 曖昧なものは always ではなく per-task に倒す (router digest の膨張を防ぐ)。
...
router digest は最小 reminder であり、膨張すると router の thin 性が失われる。digest entry は **≦10 行** を上限とする (目安)。上限に達した状態で新しい always 規則を足す場合は、既存 entry の入替 (より高優先の always 規則で置換) を検討する — 無条件に足さない。upper bound の guard は test で pin する。
