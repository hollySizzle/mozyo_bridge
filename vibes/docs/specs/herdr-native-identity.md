...
- **durable identity** = herdr **assigned name** の `mzb1_<workspace>_<role>_<lane>` scheme
  ...
  - `workspace` = mozyo workspace_id (registry / anchor が持つ workspace identity)。
    **例外 (Redmine #13331 / design j#73357): linked git worktree を sublane herdr
    workspace として使う lane では、`workspace` segment は registry workspace_id ではなく
    lane worktree の canonical path から決定的に導出する lane-scoped token
    (`derive_lane_workspace_token`, `wt_<hash>`) を使う。** linked worktree は main
    checkout の registry identity を継承し (#13152 `workspace_registry`)、独自の per-lane
    workspace_id を持たないため。standalone / main checkout は従来どおり registry
    workspace_id を使う。判別は git topology (`_is_linked_worktree`) で行い、anchor 欠落
    だけを判別子にしない (未登録 standalone repo も anchor を持たない)。registry schema は
    無変更。token は private absolute path を出さず hash のみを assigned name / journal /
    tests に載せる machine-local runtime identity であり、branch / lane_label は route
    identity drift を避けるため混ぜない。mint (§5) と全 resolve 側 (send / retire /
    projection) は単一 resolver `herdr_workspace_segment` を共有し、同一 canonical path で
    一致させる。
...
2. workspace segment を単一 resolver `herdr_workspace_segment(repo_root)` で得る (§1 の workspace
   ...
   (Redmine #13331 / j#73357)** は registry を触らず `derive_lane_workspace_token(canonical path)`
   を segment とする (linked worktree は main identity を継承し per-lane id を持たないため)。判別は
   git topology (`_is_linked_worktree`)。同 resolver を send / retire / projection の resolve 側でも
   使い、mint と resolve を一致させる。
