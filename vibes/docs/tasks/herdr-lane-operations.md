...
- **`unresolved`** = 試行したが durable な fate を確定できなかったもの（`redispatch_fate_unresolved`）。**effect ではない**: redispatch の `uncertain` は「reserve 前の zero-write」と「send 後の fate 不明」の両方を指すため、applied 側に置くと status が支えない書き込みを主張することになる。
...
**どの分岐でも固定値にしない**。全 return path が単一 composer を通り、`RecoverPairOutcome` / `RecoverPairDeliveryRetryOutcome` は `__post_init__` で「effects ⊆ 語彙」と `executed == bool(effects)` を**構造的に検証**する（矛盾した outcome を構築不能にする）。close failure / relaunch failure も例外ではない（first-close failure は適用 0、second-close failure は先行 close を報告する）。

redispatch の terminal 分類は**全域**とする: fresh delivery = effect / already-redispatched = no-op（unblocked）/ **`target_retiring` = reserve cancel の blocked zero-send**（「already delivered」と誤報しない）/ failed・uncertain = unresolved fate（blocked）。retry surface（`recover-pair-delivery`）も同じ契約に従う。`detail` は実 disposition と一致させ、JSON / text の双方が effects と unresolved を出す（blocked / partial / no-op でも operator が区別できるように）。
