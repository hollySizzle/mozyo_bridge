...
- **zero-slot は success ではない** (review j#80506 F1): expected slot が 0 件でも、それは「pair がここに無い」ことの証明であって「**この command が retire した**」ことの証明ではない。`--lane` の typo と never-launched は absence として区別不能なため `retire_evidence_absent` で non-success (exit 1) / zero-write。**idempotent success (2 回目の run) と append 失敗からの repair は durable proof store を要し、その store 選定は設計判断として保留中** (#13892 j#80511)。
