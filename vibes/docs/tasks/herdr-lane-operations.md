...
- **execute (destructive, owner approval 必須)**: 上記に加え `--journal <owner approval j>` `--action-id refresh-worker:<lane>:<role>:<provider>:<name>:<locator>:r<revision>` `--action-generation <n>` `--execute`。`turn_failed_no_durable_gate` + 全 fence green のときだけ、**exact worker generation のみ** close → same-slot fresh launch → action-bound attestation → **既存 anchor を governed handoff rail で exactly-once resume**。IR/RR は再生成しない。
  - ★**approval は「pointer の形」ではなく「durable な承認」を要求する** (#14661 review j#92443 F2)。`--journal` は **fresh durable read で実在が確認され**、かつその本文が **`refresh-worker:<action id>:g<generation>` という literal token を名指している**必要がある。この token だけが action id (= lane/role/provider/assigned name/locator/row revision の導出元) と承認 generation を**同時に**運ぶため、別 worker・別 generation・別 round の承認がこの close を許可することはない。reader 未配線 / snapshot reader / 読取不能 / journal 不在 / token 不一致は**すべて zero-close**。新しい marker 文法は導入していない (leaf Task が repo 全体の承認語彙を書き下ろさない)。
  - 既知の残存差分: 同型の approval 処理は `recover-gateway` (#14203) / `recover-stale` (#13806) にも pointer-shape のみの形で存在する。本 command は sibling より**厳しい**側へ倒してあるが、family 全体の approval 強度統一は本 Task の changed-path 境界外であり coordinator 判断に委ねる。
