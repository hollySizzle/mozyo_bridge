...
- **execute (destructive, owner approval 必須)**: 上記に加え `--journal <owner approval j>` `--action-id refresh-worker:<lane>:<role>:<provider>:<name>:<locator>:r<revision>` `--action-generation <n>` `--execute`。`turn_failed_no_durable_gate` + 全 fence green のときだけ、**exact worker generation のみ** close → same-slot fresh launch → action-bound attestation → **既存 anchor を governed handoff rail で exactly-once resume**。IR/RR は再生成しない。
  ...
  - ★**issuer authority (journal author) は未実装の残存項目**。`RedmineJournalEntry` は `issue_id` / `journal_id` / `notes` / `created_on` しか運ばず author を持たないため、現行 seam では authorship を証明できない。**検証していないものを検証したと書かない**方針で、marker/field/uniqueness/positive-decision までを実装し author authority は follow-up に残す。
...
- ★**resume は governed rail でも action-bound attestation を要求する** (#14661 review j#92487 F3)。従来 `target_action_id` を渡していたのは one-shot rail だけで、governed rail は logical slot を再解決して送り、confirmation も秒精度の `observed_at` だけを見ていた (#14203 j#87445 が「同一秒の 2 launch は timestamp を共有する」として既に否定した identity)。現在は **send 直前**と **confirmation** の双方で `LiveRecoveryAnchorDeliveryService.preflight` (v1 side-binding / v2 `replacement_action_id` を検証する既存 authority) を通し、actuator attestation 後に recycle された slot へは送らず・確認もしない。
