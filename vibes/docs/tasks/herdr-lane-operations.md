...
- read-only preflight は **store CAS の読み取り可能な signature を全部 projection する** (review j#88505 F2 / j#88512 / j#88513): disposition / issue / **`binding_kind == issue`**（project scope とは独立軸。malformed row は kind だけが不整合なことがある）/ project scope / `process_release == released` / replacement settled、さらに declared pins は非空だけでなく **store と同じ `validate_declared_slots` を通す**（duplicate / malformed pins が dry-run を green にしない）。各軸は typed blocker を返す。一部しか投影しないと「dry-run は green、`--execute` は `repair_cas_refused`」という **false green** が生じ、それが owner approval の根拠になり得る（preflight が自分の effect を予測できていない = 本 issue の主題そのものの再発）。
