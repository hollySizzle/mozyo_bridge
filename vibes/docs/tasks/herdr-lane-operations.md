...
- **preflight (既定, read-only)**: `sublane refresh-worker --issue <owning issue> --lane <label> --role claude --provider claude --assigned-name <mzb1_…> --locator <live locator> --worker-revision <row revision> --lane-revision <r> --lane-generation <g> --resume-anchor-journal <j> --resume-gate <gate> --json`。
  ...
  - ★**`--worker-revision` は空 pin では一致しない**。`recover-stale` の同名 field は空 pin を許容する (既に死んでいる slot を回復するため) が、live worker の破壊的 refresh は unpinned generation に乗ってはならない (#14203 j#87364 F5 と同じ規律)。
- **execute (destructive, owner approval 必須)**: 上記に加え `--journal <owner approval j>` `--action-id refresh-worker:<lane>:<role>:<provider>:<name>:<locator>:r<revision>` `--action-generation <n>` `--execute`。`turn_failed_no_durable_gate` + 全 fence green のときだけ、**exact worker generation のみ** close → same-slot fresh launch → action-bound attestation → **既存 anchor を governed handoff rail で exactly-once resume**。IR/RR は再生成しない。
  ...
  - ★**issuer authority は実装済み** (#14661 j#92494 scope resolution / review j#92533 F1)。marker が `approval_source=direct_owner` と*名乗る*ことは何の証明にもならない — note を書ける者は誰でもその field を書ける。したがって承認 journal の **author** を durable record から取り、**anchor issue 自身の author** (= caller が指定できない durable anchor) と一致することを要求する。author 不在 / issuer 解決不能 / 不一致は **zero-close**。
    - 共有 journal seam を**後方互換に拡張**した: `RedmineJournalEntry.author_id` (既定 `""`) と `MappingRedmineJournalSource.issue_author_id()` / `LiveRedmineJournalSource.read_issue_author_id()`。cursor projection (`_apply_since`) も issue author を保全する。
    - ★**運ぶのは provider の数値 id のみで、表示名は一切読まない** (j#92494 の「値非表示」+ 組織 baseline の個人情報禁止)。id は authority 照合に十分な opaque key であり、人物情報を DTO / journal / log へ持ち込まない。
  ...
  - 既知の残存差分: 同型の pointer-shape-only approval は `recover-gateway` (#14203) / `recover-stale` (#13806) にも存在する (#14663)。本 command は sibling より**厳しい**側へ倒してあるが、family 統一は本 Task の changed-path 境界外。
...
- ★**close 直前に durable progress を再読する** (#14661 review j#92533 F3)。run 冒頭の turn 分類を使い回すと、**worker が preflight と close の間に自分の gate を land させても close する** (実測: `observe_turn_calls=1` / `closed_count=1`)。これは本 surface が守るべきもの — dirty worktree を抱えた worker の作業 — を破壊する経路そのものである。first close の直前に progress authority を fresh 再読し、landed / ambiguous / unreadable のいずれでも zero-close。post-close replay では skip する (そこでは既に close 済で、replay は transaction を完了させるために存在する)。
