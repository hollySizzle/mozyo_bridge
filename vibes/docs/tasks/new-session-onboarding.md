...
fresh session が chat 履歴なしで受け取る最初の入力は handoff の **session boundary-prompt** である。この prompt は現在地の snapshot ではなく durable record への pointer 束であり、次の anchor を必ず含む。欠けていたら実装前に停止し、durable record を引き直す (pane の framing を source of truth の代わりにしない)。

- **durable anchor**: source ticket system + issue id + journal id (例 `source=redmine:issue=<id>:journal=<jid>`)。行動前に source-of-truth system から該当 journal を読む。判断の正本は Redmine issue / journal であり、prompt 本文ではない。
- **kind / receiver**: handoff kind (`implementation_request` / `review_request` / `custom` 等) と受信 role (`to=claude|codex`)。scope を framing する。
- **target execution root**: 実行対象の repo root (例 `.`)。pane cwd / workspace root とは別物であり、pane 位置ではなく durable anchor から確認する。
- **role profile pointer**: role token + `source` (spec path) + `version`。完全に解決された role contract は durable delivery record にあり、prompt 本文には compact な pointer clause のみ載る (`logic-role-profile-handoff-expansion`)。
