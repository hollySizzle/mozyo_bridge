...
### Installed black-box / container pilot を復元する場合

公開候補・公開済みpackageを使い捨て環境で検証するtransitionでは、[[spec-session-continuity-user-harness]] `### Installed environment evidence` を読み、次の順で再開する。

1. RedmineのTest / parent USからartifact source、exact version、origin-reachable source SHAを復元する。TestPyPI / PyPI acceptanceをlocal wheelやeditable installへ置き換えない。
2. blocking gateのpinned image digestと、non-blockingなfloating OS canaryを別verdictとして復元する。
3. runtime user、fresh home、source checkout mountの有無を確認する。non-root / source非混入が受入条件なら、root実行やsource mountありを成功へ読み替えない。
4. 実行済みsurfaceと未実施surfaceを分ける。rules / scaffold / docs / doctorのgreenは、real TUI、tmux delivery、外部ticket serviceのlive E2Eを保証しない。
5. failure時はexact image / artifact / command / expected / actualを同じTestへ記録し、再現後にだけ個別Bugへ切り出す。

このhandoffは環境検証の証拠を復元するものであり、production publish authorityを付与しない。publish判断・実行が必要ならrelease issueの明示gateを別に要求する。
