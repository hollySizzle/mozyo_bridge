...
- hibernate / resume 後の lane では lifecycle decision と新しい work anchor が異なる正当形を、current gateway の delivery receipt、startup attestation、lane / generation の一致で限定許可します。これにより resumed gateway から worker への初回配送を復旧しつつ、古い locator・別世代・読取不能な authority は引き続き zero-send になります。
...
本 release notes の確定にあわせて `pyproject.toml` / `src/mozyo_bridge/__init__.py` の version mirror を `0.7.1` → `0.8.0` に bump しました(release helper `release bump --to 0.8.0`、version bump は実装本体と分離した standalone commit)。tag / GitHub Release / TestPyPI / production PyPI publish は本メモでは行わず、親の release decision gate(#12043)と owner 明示承認のもとで別途実施します。

### v0.8.0 architecture / plugin-ready refactor(設計 + 最初の built-in seam 実装)

外部システム連携(ticket / catalog / cockpit layout / telemetry / release helper)を core に足し続けると Redmine 前提の全部入りに肥大化する、という問題意識(#11826、親 #11825 `Plugin / Adapter 境界設計`)に対し、まず **plugin-ready な built-in adapter / provider 境界**を設計 doc として固め、その上で最初の built-in seam を core に実装しました。設計・台帳の docs を正本としたまま、ticket adapter normalized record seam / internal-only provider registry skeleton / catalog backend cache-index boundary の 3 つ(#12034 / #12035 / #12036)が implemented・closed・`main` 統合済みです。external plugin API は core boundary が固まるまで引き続き公開しません(dynamic import / third-party entry point / public ABI なし)。
...
最初の built-in adapter / provider seam(実装・統合済み):

- ticket adapter の normalized record seam を core に追加しました。Redmine API 応答を `IssueRef` / `JournalRef` / `CommentRef` / `WorkflowGate` / `OwnerApproval` の pure record へ正規化する provider → core 境界を導入し、built-in Redmine provider(`infrastructure/redmine_ticket_provider.py`)が形状変換と Redmine 固有 URL 整形を所有します。close / handoff / review の判断(gate 語彙・owner approval)は core 側のみが持ち、provider は status update mechanics に限定します。gate name invariant は `WorkflowGate` の construction guard で code-level に enforce し、provider が gate vocabulary を広げられないようにしています。cockpit の Redmine read model も同 seam 経由に通し、既存 `latest_issue` payload は byte 互換のままです。(#12034)
- internal-only な built-in provider registry skeleton を追加しました。`ProviderCategory`(ticket / presentation / terminal_runtime / catalog / telemetry / release_helper)と pure description の `BuiltinProvider` / `BuiltinProviderRegistry` で built-in provider を分類・登録できるようにし、現存する built-in(redmine / tmux)のみを seed しています。registration は pure description のみ受理し(module path / callable / runtime import / third-party entry point / user script load は構造的に不可)、workflow authority / owner approval / close approval / routing authority を provider へ委譲できないことを construction guard と test で固定しました。public ABI / 互換保証は新設していません。(#12035)
- catalog backend provider 境界を cache / index 前提で docs に固定しました。正本順序を `committed Markdown → catalog.yaml → generated file conventions → rebuildable DB/cache index` と定義し、将来の DB / SQLite backend が committed rule docs / generator output を silently replace しないことを non-goal として明記しています。runtime DB backend 実装は別 issue へ分割します。(#12036)
...
- 同じ root cause で `agents targets` 側が停止する事象も修正しました。未登録 workspace defaults の materialization 待ちで discovery が無音停止しないよう、同じ軽量 inventory mode へ degrade します。本修正は実装・統合・close 済み(`c541077`)で、`main` に merge 済みです。(#12038)
...
本期間を `v0.8.0` の release 候補として整理し、version mirror を `0.8.0` に確定しました。実際の tag / publish は本メモでは行わず、親の release decision gate(#12043)と owner 明示承認のもとで実施します。

- **変更概要**: plugin-ready built-in adapter / provider 境界の設計確定と最初の seam 実装(#12000 / #12001 / #12002 / #12003 / #11808 / #12034 / #12035 / #12036)、cockpit attention / `/api/units` follow-up(#11950 / #11951 / #11952 / #11954 / #11957 / #11958 / #11999 / #12007)、cockpit discovery blocker / advisory defaults fix(#12032 / #12038)、coordinator / sublane routing guardrails(#12011 / #12015 / #12031 / #12039 / #11956)、cross-workspace consult / sublane retirement / owner-question guardrails(#11779 / #11959 / #11960 / #12009 / #11955)。いずれも additive で、既存 CLI の JSON / text 出力互換を壊していません。
- **version 候補の考え方**: 破壊的 CLI rename を含まず additive 中心(plugin-ready seam の core 追加・cockpit attention 投影・guardrail 強化)のため、直近 `v0.7.x` からの **minor 相当**として `0.8.0` を mirror に確定しました。最終的な production release 判断は owner / #12043 gate に委ねます。
- **release gate の実行手順**: 実際の検証・配布手順は `skills/mozyo-bridge-agent/references/release.md` の Standard Verification / Release Flow / Release Artifact Guardrails / Distribution Gates / Trusted Publishing を正本とします。version bump は本 issue で standalone commit として実施済みで、`main` push → GitHub Actions `Test` 成功確認 → TestPyPI → `pipx` install 検証 → owner 判断で production PyPI、の順で #12043 gate 側が進めます。本メモはこの gate の入口を明確化するもので、本 issue では tag / publish を実行しません。
