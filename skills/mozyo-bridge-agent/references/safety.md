...
- Two send rails exist; pick the right one rather than weakening either:
  ...
  - `--mode standard` (strict explicit fallback, preserved from v0.1; `mozyo-bridge message --submit` default behavior): strict marker-observed Enter. Marker miss is fail-closed — a `C-u` rollback is issued and Enter is not sent. Since Redmine #12450 the rollback is then re-captured to verify the composer: if the marker is gone the outcome is `blocked` / `marker_timeout` (rollback verified); if the typed prompt is still staged (the receiver TUI did not clear it on `C-u`) the outcome is the distinct `blocked` / `marker_timeout_pending`, which does NOT claim a clean rollback — recover by submitting the already-staged prompt (`mozyo-bridge keys <target> Enter`) or clearing it, and do not resend (that would duplicate). Select this rail explicitly when strict landing observation is required (regression check after a queue-enter rail change, brand-new pane where queue-pickup probability is unverified, observability test, audit requirement that demands strict landing evidence) or when the target falls outside the v0.4 default scope. Record the reason for selecting it in the durable record so an auditor can replay why the default was overridden.
