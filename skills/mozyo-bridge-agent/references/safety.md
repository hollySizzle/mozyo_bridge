...
- 標準の handoff/reply 経路は高レベル primitive である: `mozyo-bridge handoff send` / `mozyo-bridge handoff reply` / top-level alias `mozyo-bridge reply`。この primitive は receiver を解決し、backend 別の preflight、本文の injection、submit / turn-start の観測、structured outcome の生成を一体で所有する。`queue-enter` / `standard` は submit を試み、`pending` は input を意図的に残すが、成功は単に「本文を type して Enter を押した」ことではなく、下記の backend 契約と `injection_stage` で判定する。caller は通常の handoff/reply のために `mozyo-bridge read` + `mozyo-bridge message` の shell choreography を自前で組み立てない。
...
- `mozyo-bridge handoff send` / `handoff reply` が失敗した場合、status / reason や exit code だけで `un-notified` と断定しない。structured outcome の共有 `injection_stage` を読む。`not_sent` は本文も Enter も送っていないため bounded retry 可能、`uncertain_partial` は本文または Enter が到達した可能性があるため blind resend 禁止、`submitted_confirmed` は causal confirmation 済みである。`uncertain_partial` は durable anchor と receiver 状態を照合して reconcile し、receiver pane の mutation で自己修復しない。
- 送信 rail は 2 本存在する。どちらかを弱めるのではなく、正しい方を選ぶ:
  - `--mode queue-enter` は agent pane handoff の v0.4 規範 default である (Claude / Codex pane を target とする `mozyo-bridge handoff send` / `handoff reply` / `notify-*` 標準 variant。`--force` を拒否する)。typing より前の admission が失敗した場合は `injection_stage=not_sent` のまま fail-closed する。admission 後の保証は backend で異なる。
  - **tmux compatibility backend の queue-enter** は決定的な Layer B preflight を使う。次の check のいずれかが失敗した場合、CLI は `send-keys -l` を発行する前に `blocked` と対応する `Reason` を出して die する:
    ...
    すべての check が pass すると本文を 1 回 type し、landing marker が観測されたかどうかに関わらず Enter を発行する。outcome は、marker 観測ありなら `sent` / `ok`、未観測なら `sent` / `queue_enter` である。どちらも practical queued submission であって causal confirmation ではないため、receiver は durable な Asana task comment / Redmine journal を正本として読む。v0.4 の default 切り替えは preflight を弱めない。scope 外の target (`mozyo-bridge message`、非 agent pane) はこの rail に入らない。
  - **Herdr backend の queue-enter** は admission 後に本文を高々 1 回だけ type する。idle / turn-ended の受信では、first Enter は body injection 後の pinned generation 再確認、working-transition wait の arm、absolute deadline check がすべて成功した場合だけ発行し (zero-or-one)、同じ terminal と collision-free generation に結び付く causal turn-start を確認した場合だけ `sent` / `ok` / `submitted_confirmed` とする。busy baseline は例外 (ADR-0002 / Redmine #15537): 受信 agent が既に working の場合は working-transition wait を attribution に使えないため、pending observer 要求なしで Enter を発行し、injected body が current composer から消えた事実を提出証拠として `sent` / `queue_enter` (practical queued submission、causal claim なし、injection_stage は `uncertain_partial` のまま) を返す。実 effect 直前には generation・deadline・composer body・screen・runtime state を fresh に再検証し、blocked / unknown / drift は zero actuation とする。その後は既定 30 秒 / 2 秒の単一 budget 内で、同一 target / launch generation / composer body / screen / runtime state を fresh に確認した Enter-only fallback を行える。wait error は次の Enter を許可しない。上記いずれの提出証拠も無い場合は precise `blocked` + non-zero に閉じる。本文は再入力しない。
  - `--mode standard` は v0.1 から維持される strict な明示 fallback である。**tmux compatibility backend** (`mozyo-bridge message --submit` の default 挙動を含む) は marker 観測を Enter の必要条件とし、marker miss では `C-u` rollback を発行して Enter を送らず `blocked` / `marker_timeout` にする。ただし sender は composer clear を検証できないため `marker_timeout` は `uncertain_partial` であり、非配達の証明にも blind resend の根拠にもならない。**Herdr backend** は本文を高々 1 回だけ入力する event-driven turn-start rail で、causal confirmation が無ければ fail-closed する。strict landing / turn-start observation が必要なとき、または target が default scope 外のときに理由を durable record へ残して明示的に選ぶ。default 切り替えは tmux の historical behavior を変えない。
