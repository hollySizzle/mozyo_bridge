...
### `handoff delegate-coordinator` (project-router delegation primitive)

`mozyo-bridge handoff delegate-coordinator` packages the project-router delegation route into one command (Redmine #12438 / US #12437): a coordinator that owns a project with an external-submodule dependency (e.g. gk-3500-it-operations, whose `projects.yaml` classifies `giken-3800-mozyo-bridge` as `external-submodule` / `external_dependency_reference`) hands the work to that submodule's *canonical* project Codex as a `delegated_coordinator` instead of editing the submodule in place. Like the consult primitive it is a boundary-preserving wrapper over `handoff send` — it resolves routing and role-profile fields but hides or weakens no gate:

- **Routing comes from project-router metadata.** `--projects-config <projects.yaml>` plus `--target-project <id>` are read and classified: only a positively-classified external-submodule with a declared canonical repo root is delegable. A non-external project, an absent project, or a missing canonical root fails closed — delegation never guesses.
- **The target Codex gateway is resolved by canonical-repo-root match.** The unique Codex pane whose checkout is the canonical repo is selected from the same candidate projection `agents targets` uses. Absent → `target_unavailable` (the canonical Unit is not loaded; **operator action is required** — this route never auto-launches a Unit). Ambiguous (more than one, or only an ambiguous-role pane) → `blocked`. An explicit `--target %pane` overrides discovery.
- **Receiver is fixed to `codex`; there is no direct cross-project Claude send.** The canonical Codex reads the durable anchor and, if implementation follows, performs the local same-session Claude handoff inside its own project. Grandchild dispatch stays the delegated coordinator's own policy decision — the route creates no hidden worker.
- **The repo-identity gate is preserved.** The chosen pane's cwd must resolve to the canonical repo root (`target_repo_mismatch` otherwise); an explicit `--target-repo` is honored if supplied. The receiver binding holds in every mode (`require_receiver_binding`).
- **The `delegated_coordinator` role profile is auto-filled.** Its placeholder fields (`parent_project` / `child_project` / `parent_callback_target` / `parent_issue` / `redmine_project`) are derived from the router decision, with explicit `--parent-project` / `--parent-issue` / `--parent-callback-target` / `--profile-field` overrides winning. `--kind` defaults to `implementation_request`.

Operational route: (1) record the delegation request on the durable anchor first; (2) ensure the canonical project's Codex Unit is loaded (`agents targets` shows it); (3) run `mozyo-bridge handoff delegate-coordinator --source <redmine|asana> <anchor flags> --projects-config <projects.yaml> --target-project <id> [--parent-issue <id>] [--summary ...]`; (4) the canonical Codex reads the anchor and dispatches inside its own project. The exact `projects.yaml` schema is confirmed on hardware by the verification issue; the parser accepts documented key aliases and fails closed on anything it cannot positively classify.

### `handoff delegate-coordinator-lane` (launch/adopt harness)

`mozyo-bridge handoff delegate-coordinator-lane` is the launch/adopt front stage over `delegate-coordinator` (Redmine #12447 / US #12437). The plain route silently selects whatever unique Codex pane already lives in the canonical repo, so an existing-lane route can read as PASS even when no fresh child lane was launched and no adoption was recorded. This command makes the lane decision explicit and replayable, and reuses the same fail-closed routing core:

- **The decision is explicit; there is no auto mode.** `--lane {launch,adopt}` is required, so a pre-existing lane is never silently reused as completion. Omitting it fails closed (`lane_decision_required`).
- **`--lane adopt` adopts a *visible* existing lane.** An explicit `--adopt-target <%pane>` wins, otherwise the unique unambiguous canonical Codex lane is selected (fail-closed on absent → launch instead / ambiguous → name the pane). It then hands off to that lane through the Codex gateway with the `delegated_coordinator` role profile — the same boundary-preserving send as `delegate-coordinator`, so there is no direct cross-project Claude send.
- **`--lane launch` plans a fresh visible lane.** `--child-issue` plus `--branch` / `--worktree` form the replayable lane identity. The command emits the launch plan but never spawns the lane: mozyo-bridge core is not a git worktree manager, so the operator materializes the visible worktree / cockpit Unit (plain git) and the live run is verified in the dedicated verification issue. Launch fails closed when the canonical repo root is not present locally (`launch_canonical_root_absent`) or the identity is incomplete.
- **Every decision emits a replayable durable record** carrying the launch/adopt selection, target / parent issue, target project, canonical repo root, lane / worktree identity, the purpose-tagged required callback targets, the parent → child delegation breadcrumb (`delegation_root` / `delegation_parent` / `delegation_depth`, projection only), and an explicit `no_hidden_subagent` marker. Owner approval and parent close authority stay on the parent coordinator (the breadcrumb records the parent as this lane's retire owner).
- **Callback targets are multi-recipient, not a single route (Redmine #12449).** #12448 showed a single-`callback_route` model let a delegated lane record PASS after notifying only the parent project coordinator while a separate owning-US / audit coordinator never received a pointer (cockpit looked stalled for US-level audit). The record therefore models purpose-tagged required callback targets: `--parent-callback-target` (`delegation_parent`) plus `--owning-us-coordinator` / `--audit-coordinator` when a distinct US owns the child issue. Purposes that resolve to the same route collapse into one target carrying both purposes (recorded explicitly, never dropped). A delegated lane is not "done" until **every** required callback target has a recorded outcome (`sent`, a replayable `blocked`, or an explicit `not_applicable`); same-lane surfacing is not a callback outcome, and notifying only the parent coordinator does not pass the acceptance check.
