...
- review_result の journal には v2 marker と同一 journal で `review-finding-manifest` sidecar (#14971) を必ず併記する。承認でも count=0 の明示的空集合 sidecar が必須である。canonical producer `workflow callbacks --emit-gate --gate review_result` が write できない環境 (opt-in unset) では `--render-note-only` の出力 note を 1 journal としてそのまま貼り、sidecar や `set_digest` を自作・省略しない。manifest を欠く review_result は retire / integration fence が finding 母集合を証明できず、lane が恒久 blocked になる (#15699)。
