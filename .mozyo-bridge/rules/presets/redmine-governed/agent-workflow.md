...
lane process の hibernate (early hibernate を含む) も完了ではない。lane を early hibernate — same-lane Review Gate approved + coordinator staging integration + required CI green を根拠に、TestPyPI / installed dogfood と close を専用 release issue へ durable に委譲して process を畳む — しても、それは issue close でも owner close approval でも dogfood 成功でもない。hibernate した lane の issue は依然 open であり、上記 Completion 条件を満たすまで close しない。hibernate を close / owner approval / dogfood 成功へ読み替えない (process drain と ticket close の分離)。運用手順の正本は skill `references/workflow.md` `## Sublane hibernate (プロセス解放) と early hibernate`、集約 dogfood の release issue 運用は同 `references/release.md` を読む。
