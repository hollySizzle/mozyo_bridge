...
3. **Design Consultation Gate** — 後戻りしにくい判断の前に作る。例: 仕様解釈、責務境界、永続化形状、自動補正、UI flow、authorization、DB integrity、既存 URL / data compatibility。背景、正本参照、選択肢、pro/con、実装者の推奨、判断が必要な質問、残る不明点を分けて記録する。実装指示 (implementation request) や review finding の内容に実装者が異議を持つ場合の **上申 (dispute)** も、新しい gate や transport kind を作らずこの gate で扱う。dispute では `purpose: dispute` を明示し、異議対象の journal id、根拠 (確認した code / docs / 事実)、counterproposal、owner 判断の要否を分けて記録する。異議の表明は役割境界違反ではなく、`事実姿勢` が要求する義務である。
4. **Design Consultation Answer Gate** — 相談を受けた audit / design role が記録する。選択した案、理由、却下した案と却下理由、残る不明点、owner 判断が必要な事項を分ける。相談・dispute を受けた側は、放置せず Answer Gate で応答する責務を持つ。Answer 後も合意できない場合の終端は owner 判断への escalate であり、どちらの agent も自案を一方的に通さない (`判断の routing` を読む)。
...
- 順序は Review Gate approval → owner クローズ可否確認 journal → owner close approval 記録 → Close Gate であり、この収集〜記録は reviewer (project が定めるユーザー窓口 role) 側で完結させる。実装者 pane の chat で owner 承認を取りに行く運用は、この分離を崩す逸脱として扱い、発生したら correction journal を残す。
- project は通常開発 task の owner close approval を窓口 role へ **事前委任 (standing delegation)** できる。委任しても owner close approval journal の記録自体は省略せず、承認の出所 (`approval_source: standing_delegation | direct_owner`) を journal に明示する。委任の範囲・除外条件は project の preset / rules に明文化されている場合に限り有効で、暗黙の委任は無効である。

## Direct Request Triage

default の作業入口は project が定めるユーザー窓口 role である。ユーザー / owner が窓口以外の agent (例: 実装者 pane) に直接作業を依頼した場合も、依頼を chat に留めず次の triage 経路で durable record にする。

1. 依頼を受けた agent は、対応する Redmine issue を即起票する (**triage-pending** と明示する)。起票による durable record 化が最優先であり、窓口への転送待ちで record 化を遅らせない。
2. 起票 agent は triage role (audit / 窓口 role) へ精査を handoff する。精査 journal による re-parent / 分割 / 統合 / tracker 変更 / 優先度変更は手戻り扱いしない。チケット構造の権威は triage role に残る。
3. 着手条件は risk で分ける。低リスク scope は Start Gate 記録後に着手してよいが、triage 完了 journal が無いまま close してはならない。高リスク scope (設計分岐、互換性、外部影響、guardrail / 配布物 / credential 接触) は triage 完了まで着手しない。
4. 窓口 role が直接受けた依頼は従来どおり窓口 role が起票する。本 triage 経路は例外時の救済であり、入口の既定を置き換えない。
...
- 標準通知は高レベル handoff primitive を使う: `mozyo-bridge handoff send --to <claude|codex> --source redmine --issue <issue_id> --journal <journal_id> --kind <implementation_request|design_consultation|review_request|review_result|implementation_done|reply|custom>`。reply では `mozyo-bridge handoff reply ...` または上位 alias `mozyo-bridge reply ...` を使う。primitive は receiver 解決、backend 別 admission、本文の高々 1 回の injection、submit / turn-start observation、structured outcome を所有する。tmux compatibility queue-enter は従来の practical queued-submission mapping を維持する。Herdr queue-enter は本文を高々 1 回だけ type する。idle / turn-ended 受信では same-generation causal turn-start を確認した場合だけ `submitted_confirmed` / exit 0、busy 受信では ADR 系決定 (mozyo_bridge #15537) により Enter を 1 回発行して composer clear を提出証拠に `sent` / `queue_enter` / exit 0 (queued submission、causal claim なし) を返す。caller は通常 handoff / reply で `read` + `message` shell choreography を手で組み立てない。
...
- raw tmux pane mutation を agent の workflow delivery / recovery 手段として使わない。具体的には `tmux send-keys`、`tmux paste-buffer`、agent pane への直接 Enter / `C-u` / その他 raw key injection を禁止する。これらは handoff send/reply rail の代替にも、failed handoff の self-repair にもしない。
