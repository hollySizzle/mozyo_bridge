...
- **Raw tmux pane mutation is never a delivery or recovery step.** No checklist step authorizes an agent to drive an agent pane with `tmux send-keys`, `tmux paste-buffer`, a direct Enter / `C-u`, or any other raw key injection, nor with low-level `mozyo-bridge type` / `mozyo-bridge keys` used as a handoff substitute, nor with `mozyo-bridge message --no-submit` followed by an agent-issued raw Enter to submit. When the Standard path and its gated fallbacks all fail, the agent records `un-notified` (step 6) and does not self-repair the delivery by mutating the pane. Read-only inspection (`tmux capture-pane`, `mozyo-bridge read`) is allowed. Raw tmux mutation is operator/debug-only and requires an explicit operator instruction at the moment of use plus a durable record of that instruction; an agent never performs it on its own judgement.
