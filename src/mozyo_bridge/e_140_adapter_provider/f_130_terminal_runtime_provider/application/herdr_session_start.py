...
def _prepare_session_locked(
    *,
    ...
    # The ratio rail is handed the LANE's initial occupancy, not the container's: under
    # `shared_tab` (#14567) a lane's first slot splits beside another LANE, and that divider
    # is #14604's axis rather than this pair's. The two counters are equal on every
    # `per_lane_tab` / coordinator path, so this is byte-invariant there.
    finalize_container_geometry(
        ...
        initial_lane_occupancy=plan_of_container.lane_occupancy, dry_run=dry_run,
