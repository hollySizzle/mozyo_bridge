"""Lane-tab resolution for a session-start run (Redmine #13411 / #15702).
...
created so its born root pane IS the first launch slot's prepared pane.
    ...
    *,
    ...
    if binding is None or resolved is None:
        raise HerdrSessionStartError(
            "prepared lane tab creation has no preflighted native binding / provider "
            "for the first launch slot; refuse to mint a tab for an unresolved launch"
        )
    entries = build_pane_launch_env(
...
__all__ = ("LaneTabResolution", "resolve_lane_tab")
