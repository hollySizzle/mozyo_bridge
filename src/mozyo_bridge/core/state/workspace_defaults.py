...
A single workspace-local YAML (`<repo>/.mozyo-bridge/project-defaults.yaml`,
formerly `.mozyo-bridge/workspace-defaults.yaml`) is the source of truth for
repo-specific defaults that Codex / Claude / local docs all reference. The file
was renamed (Redmine #11920 / #11921) to say what it carries — project-level
governance defaults, not workspace runtime state; the legacy name stays
readable as a fallback and new writes only ever create the new name. Today's
slice covers the Redmine default project resolution contract:
...
from mozyo_bridge.shared.errors import die, warn
from mozyo_bridge.shared.name_compat import CompatResolution, resolve_compat_path


# New primary input name (Redmine #11920 / #11921). The legacy name stays a
...
PROJECT_DEFAULTS_INPUT_RELATIVE = Path(".mozyo-bridge/project-defaults.yaml")
WORKSPACE_DEFAULTS_LEGACY_RELATIVE = Path(".mozyo-bridge/workspace-defaults.yaml")
# Deprecated alias kept for import compatibility; resolves to the legacy name.
WORKSPACE_DEFAULTS_INPUT_RELATIVE = WORKSPACE_DEFAULTS_LEGACY_RELATIVE
...
def render_redmine_defaults_markdown(defaults: WorkspaceDefaults) -> str:
    ...
    Source-file references in the rendered body track the actual input name
    (Redmine #11920 / #11921): a workspace on the new
    ``project-defaults.yaml`` renders that name, one still on the legacy
    ``workspace-defaults.yaml`` renders the legacy name, so the "edit this
    file" instructions always point at the file that exists.
    """
    project = defaults.default_project
    verification = defaults.verification
    source_rel = f".mozyo-bridge/{defaults.source_path.name}"
    ...
    if verification.is_complete:
        ...
        lines.append(
            ...
            f"`{source_rel}` is incomplete (see the "
...
def defaults_resolution(repo_root: Path) -> CompatResolution:
    """Resolve which defaults name(s) exist for ``repo_root`` (new vs legacy)."""
    return resolve_compat_path(
        repo_root,
        PROJECT_DEFAULTS_INPUT_RELATIVE,
        WORKSPACE_DEFAULTS_LEGACY_RELATIVE,
    )


def resolve_input_path(repo_root: Path) -> Path:
    """The defaults file a reader/loader should consume.

    Name compatibility (Redmine #11920 / #11921): the new
    ``project-defaults.yaml`` wins, the legacy ``workspace-defaults.yaml`` is
    the fallback. When neither exists, the new name is returned so the
    not-found error points operators at the current name. This helper does not
    enforce the both-exist contract — use :func:`load_repo_defaults` for the
    command-level fail-closed behavior.
    """
    resolution = defaults_resolution(repo_root)
    return (resolution.read_path or resolution.new_path).resolve()


def load_repo_defaults(repo_root: Path) -> WorkspaceDefaults:
    """Load a workspace's defaults with the rename compatibility contract.

    Mutating / explicit commands go through here: the both-exist state fails
    closed (no silent merge — the operator must remove the superseded legacy
    file), the legacy-only state loads with a deprecation warning, and the new
    name is preferred otherwise.
    """
    resolution = defaults_resolution(repo_root)
    if resolution.both_exist:
        die(
            f"both {PROJECT_DEFAULTS_INPUT_RELATIVE.as_posix()} and "
            f"{WORKSPACE_DEFAULTS_LEGACY_RELATIVE.as_posix()} exist in "
            f"{repo_root}. The new name is authoritative; remove the legacy "
            f"{WORKSPACE_DEFAULTS_LEGACY_RELATIVE.as_posix()} after confirming "
            f"the new file is correct, then re-run."
        )
    if resolution.using_legacy:
        warn(
            f"{WORKSPACE_DEFAULTS_LEGACY_RELATIVE.as_posix()} uses the legacy "
            f"name; rename it to {PROJECT_DEFAULTS_INPUT_RELATIVE.as_posix()} "
            "(the legacy name still reads during the compatibility window)."
        )
    source = resolution.read_path or resolution.new_path
...
def collect_render_results(repo_root: Path) -> list[RenderResult]:
    defaults = load_repo_defaults(repo_root)
