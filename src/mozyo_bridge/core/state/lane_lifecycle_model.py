...
# ``norm`` (the component's single field normalizer) lives in the lowest leaf next to
# the codec that needs it, and is re-exported below with the rest of that leaf's names.
...
# the model module keeps working — the public import surface is unchanged.
from mozyo_bridge.core.state.lane_declared_slots import (  # noqa: E402
...
class LaneLifecycleRecord:
    ...
    kind (``coordinator`` | ``delegated_coordinator`` | ``implementation``) this lane was
    created with, stored generation-bound so a heal resolves the SAME lane-role pane
    placement OFFLINE (never re-read from Redmine, never from the display cache). It is the
    launch path's *heal authority* for lane-role placement; the fresh-launch authority stays
    the caller-supplied ``LaneLaunchContext``. Immutable within a generation — a governance
    change re-binds it on a NEW generation, never an in-place overwrite. Empty on a pre-v7
