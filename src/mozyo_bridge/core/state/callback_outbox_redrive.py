...
deliverable callback. This module is the sanctioned OUT-OF-BAND repair: an explicit,
fingerprint-gated compare-and-swap that returns one observed dead-letter row to ``pending``.

It lives beside :mod:`mozyo_bridge.core.state.callback_outbox` (same bounded context, same
``workflow-runtime.sqlite``) as its redrive companion: the store module owns the schema and the
automatic state machine, this module owns the operator-driven exception to it. It deliberately
reuses the store's package-internal connection helpers rather than growing a second
...
#: Typed results of :func:`requeue_dead_letter`. A closed vocabulary so the operator surface
#: can map each disposition to a distinct exit code; every value except
...
def dead_letter_fingerprints(
    outbox: CallbackOutbox, *, workspace_id: Optional[str] = None
) -> tuple["tuple[CallbackOutboxRow, str]", ...]:
    """Read the dead-letter backlog with each row's redrive fingerprint (read-only).

    The dry-run half of the explicit redrive: the operator reads this, decides which ONE row
    to redrive, and quotes its fingerprint back to :func:`requeue_dead_letter`.
    ``workspace_id`` filters to that partition (``None`` = all — the CLI gates this behind its
    own workspace attestation). The fingerprint is computed from the same persisted fields the
    apply re-reads, so a row that mutates in between can never be redriven.
    """
    outbox._ensure_migrated_if_exists()
    conn = outbox._connect_ro()
    if conn is None:
        return ()
    try:
        if not outbox._table_present(conn):
            return ()
        sql = (
...
def requeue_dead_letter(
    ...
    *,
    ...
    is what makes the fingerprint ABA-proof — a redriven row that dead-letters again can never
    re-match an old observation) and ``max_attempts`` becomes
    ...
    re-runs the whole fenced pipeline (claim -> generation fences -> admission -> one send), so
    a stale row is still zero-send terminal — a redrive re-admits, it never bypasses. Every
    other outcome (:data:`REDRIVE_ABSENT` / :data:`REDRIVE_STATE_MISMATCH` /
    :data:`REDRIVE_FINGERPRINT_MISMATCH`) is a typed zero-write.
    """
    stamp = now or _utc_now()
    conn = outbox._connect_immediate()
    try:
