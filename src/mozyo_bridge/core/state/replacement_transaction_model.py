...
#: The manifest envelope this build WRITES (Redmine #14741 j#97038). v2 adds the update
#: evidence triplet; v1 is still READ, as a legacy all-empty triplet.
PARTICIPANTS_VERSION = 2
#: Envelope versions this build can read. A v1 manifest predates identity receipts, so its
#: participants simply carry no evidence — that is a fact, not a gap.
PARTICIPANTS_READABLE_VERSIONS: tuple = (1, 2)
...
class ParticipantPin:
    ...
    lane_revision: str = ""
    lane_generation: str = ""
    #: The exact generation whose update evidence this transaction owns, plus the typed
    #: cause (Redmine #14741 j#97038). Pinned HERE rather than re-derived at consume time:
    #: reconstruction needs the launch-generation authority to still exist, so a transaction
    #: could not replay its own evidence after that authority was lost — and owning the
    #: evidence across crash and replay is exactly what C15 asks of the transaction.
    #:
    #: ``lane_id`` / ``provider`` / ``assigned_name`` above already carry the rest of the
    #: key, so only the workspace, the startup action and the cause are added.
    #:
    #: All three empty (legacy / generic) or all three non-empty (receipt-capable). A
    #: partial triplet is refused: it would name a generation nobody can address.
    ...
    def __post_init__(self) -> None:
        ...
        if any(triplet) and not all(triplet):
            raise ParticipantPinError(
                ...
                "generic) or wholly present (receipt-capable); a partial triplet names a "
                "generation nobody can address"
            )
...
def decode_participants(raw: str) -> tuple[ParticipantPin, ...]:
    ...
    if (
        ...
        or version not in PARTICIPANTS_READABLE_VERSIONS
    ):
        raise ParticipantPinError(
            f"participant manifest version {version!r} is not one this build reads "
            f"{list(PARTICIPANTS_READABLE_VERSIONS)} (unknown / newer / malformed); "
            "fail closed"
        )
