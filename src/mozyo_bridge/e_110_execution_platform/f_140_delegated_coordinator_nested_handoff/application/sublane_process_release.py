...
def declared_generation_exactly_live(
    ...
    *,
    ...
    generation before it releases anything. A live managed slot is the declared generation
    only when it :meth:`ProcessGenerationPin.binds_same_generation` its declared pin — the
    ``(role, provider, assigned_name, locator)`` identity, the type 正本's required identity
    (``lane_lifecycle_model.py`` / ``managed-state-model.md`` ``### ...world state``). Any
    other live slot — a **recycled** locator (the declared process died and relaunched), a
    **renamed** slot, a **provider-rebound** slot (Redmine #13811 R1 F1: ``pin.provider`` is
    a real match axis, never collapsed to ``role``), an **undeclared** role, or an
    **ambiguous** inventory (one role live at two locators) — is a newer / foreign
    generation the declared authority does not name, and the action fails closed rather than
    close it (§2 "newer generation / stale approval -> zero-actuation"). A declared slot that
    is simply **gone** (no live row) needs no close and does not block (the 0/1/2-slot /
    dead-process case).
    ...
    """
    declared_by_role: dict[str, ProcessGenerationPin] = {}
    for pin in declared_pins:
        role = _norm(pin.role)
        prior = declared_by_role.get(role)
        if prior is not None and not prior.binds_same_generation(pin):
            # Two declared pins for one canonical role that are not the same generation:
            # an internally ambiguous declaration. Fail closed rather than pick one.
            return False
        declared_by_role[role] = pin

    want_ws = _norm(workspace_id)
    want_lane = _norm_lane(lane_id)
    live_by_role: dict[str, set[tuple[str, str, str, str]]] = {}
    for row in rows:
        if not isinstance(row, Mapping):
            continue
    ...
    for role, live in live_by_role.items():
        if len(live) != 1:
            # This role is live at more than one distinct observation — an ambiguous
        ...
        declared = declared_by_role.get(role)
        if declared is None:
        ...
        try:
            ...
            # A live row that cannot form an identity pin is anomalous — fail closed.
            return False
