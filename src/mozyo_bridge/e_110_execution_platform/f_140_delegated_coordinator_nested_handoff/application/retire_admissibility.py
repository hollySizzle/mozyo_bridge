...
- :func:`_resolve_superseded_failure_admissible` — the SUPERSEDED FAILURE terminal (#14755): a
...
def _read_live_issue_journals(issue: str) -> "list[tuple[str, str]]":
    ...
    Shared by the routes whose premise cannot be carried by a caller-supplied file. #14695 needs it
    because its premise is NEGATIVE (nothing anywhere in the record declares change) and a subset
    satisfies a negative claim by omission alone. #14755 needs it because its premise is a
    CORRELATION across two issues — "this round is the newest, its findings were accepted, and
    that other issue supersedes it" — and every one of those is falsifiable by dropping a journal.
...
def _resolve_superseded_failure_admissible(
    args: argparse.Namespace,
    *,
    target: Optional[RetireEvidenceTarget] = None,
    repo_root: Optional[Path] = None,
    ...
    - the target issue's full history, read LIVE (never a caller-supplied file — see
      :func:`_read_live_issue_journals`), folded with the same glance grammar every other consumer
    ...
    try:
        ...
        outcome = evaluate_superseded_failure_admissible(
            ...
            verdicts=fold_finding_verdicts(
                journals, review_journal=declaration.review_journal
            ),
