    ...
    *,
    ...
    sublane_retire.add_argument(
        ...
        help=(
            ...
            "**THIS IS A BOUNDED MIGRATION, NOT A GENERAL TERMINAL.** The lanes it may terminalize "
            "are ENUMERATED IN CODE (`SANCTIONED_MIGRATIONS`) by exact source issue, audit "
            "journal, successor issue, successor review journal, lane, generation, head and "
            "integration branch; anything else is refused with "
            "`not_an_enumerated_audit_failure_migration` whatever its records say. Two rounds "
            "tried to derive that binding from records and each was refuted by measurement: mutual "
            "acknowledgement (review j#101880 finding 1 — one unauthenticatable writer can place "
            "both halves) and then head coverage (review j#101909 finding 1 — on a zero-change "
            "lane the lane head IS the integration head, so every unrelated approved issue on that "
            "base shares it, and a successor review about entirely different work still admitted). "
            "The binding is a coordinator judgement and this workspace has no authority that "
            "carries one, so it is enumerated where a journal author cannot reach it; extending "
            "the route to another lane is a reviewed code change. "
            "On top of that pin it admits only when ALL of: one canonical "
            ...
            "that approval's reviewed head literal-equals the pinned head; and the live "
            ...
            "#14219 j#86718); the pin removes the need to cross that gap by refusing to generalize. "
