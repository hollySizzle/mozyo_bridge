    ...
    *,
    ...
    sublane_retire.add_argument(
        ...
        help=(
            ...
            "**What this route rests on is head coverage, NOT the two markers agreeing** (review "
            "j#101880 finding 1): a source declaration and a successor acknowledgement can be "
            "placed by one unauthenticatable actor across two issues, which is the shape #14695 "
            "refuses forever, so the admitting fact is instead that the successor's APPROVED "
            "Review Gate examined THIS LANE'S EXACT HEAD — read from its own review_result marker, "
            "already correlated by the shared grammar against the head its review_request pinned. "
            "Admits only when ALL of: one canonical "
            ...
            "that approval's reviewed head literal-equals the declaration's head; and the live "
            ...
            "live-zero conjuncts BOUND the route: a real approved review covers the state, and "
            "there is no other state, so admitting drains a process without integrating anything "
            "or minting any approval. It does NOT establish that the audit concluded a failure "
            "(that is prose, not a governed surface — the audit field is a POINTER and carries no "
            "admission weight), it does NOT assert that THIS issue passed a review (head coverage "
            "is about which commit state was examined, never about whose acceptance was met), and "
