    ...
    *,
    ...
    sublane_retire.add_argument(
        ...
        help=(
            ...
            "**THIS ROUTE CURRENTLY ADMITS NOTHING** and always refuses with "
            "`coordinator_receipt_authority_unresolvable` once a record is otherwise complete. "
            "Nothing available here can establish that the writer of a coordinator decision REALLY "
            "WAS the coordinator runtime: five review rounds each refuted a different answer, most "
            "recently an isolated-temp-repo reproduction showing that the writer attestation reads "
            "only material a caller can itself create (a workspace anchor and provider config under "
            "any --repo it names, plus its own MOZYO_* env). The coordinator's ruling on design "
            "consultation j#102184 holds the route as a typed refusal until #15195 supplies a "
            "Herdr-runtime-issued, action-bound receipt the writer can verify (#15195 blocks "
            "#15166). Every check below is live and reports its own reason FIRST, so the flag is "
            "still useful for diagnosing a record; it just cannot unlock a retire. The intended "
            "contract, for when it can: "
