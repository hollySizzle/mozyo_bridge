    ...
    *,
    ...
    parser_.add_argument(
        ...
        help=(
            ...
            "in before delivery. Omitting the flag skips the repo gate — "
            "but note a relative `--workdir` then resolves against the "
            "SENDER's cwd, so do NOT drop it to get past an `auto` "
            "failure (Redmine #14249). `auto` resolves the root per "
            "backend: under tmux (Redmine #11778) from an explicit "
            "`%%pane` target's own cwd, which it therefore requires; "
            "under herdr (Redmine #14249) there is no pane cwd, so it "
            "resolves the TARGET's own frame — this checkout when the "
            "target is the sender's own lane, otherwise that lane's "
            "canonical worktree via its lifecycle worktree binding. "
            "Both stay fail-closed (`target_repo_mismatch` / "
            "`auto_target_repo_unresolved`) and never fall back to the "
            "sender's root; pass an explicit `--target-repo <root>` "
            "instead."
        ),
    )
    parser_.add_argument(
    ...
    parser_.add_argument(
        ...
        help=(
            ...
            "Herdr, an idle/turn-ended receiver gets a causal wait armed "
            "before every Enter and succeeds as sent/ok only when the same "
            "launch generation confirms the turn start; a BUSY receiver "
            "(ADR-0002, #15537) gets the Enter through a wait-free full "
            "effect fence (fresh generation/composer/screen/state checks) "
            "and succeeds as sent/queue_enter when the composer releases "
            "the body (queued submission, no causal claim). Failures stay "
            "fail-closed (`turn_start_absent` for an absent target, "
            "`receiver_blocked` for a confirmed blocked runtime, otherwise "
            "`turn_start_unconfirmed`; transport failures use "
            "`transport_error`); the staged body makes blind retry unsafe. "
            "Under tmux it keeps the legacy marker-based "
...
def register_message(
    ...
    message.add_argument(
        ...
        help=(
            ...
            "compatibility outcome while Herdr reports causal success on an "
            "idle/turn-ended baseline and the noncausal busy queued submission "
            "as sent/queue_enter (ADR-0002, #15537). "
