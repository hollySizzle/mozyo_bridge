...
# Redmine #12750: the ticketless no-anchor anchor models and the
# ``SOURCE_TICKETLESS`` token are factored into their own module so the
# Redmine-anchored vs ticketless-no-anchor boundary is physically explicit. They
...
from mozyo_bridge.e_110_execution_platform.f_130_handoff_routing.domain.handoff_send_semantics import MODE_PENDING, MODE_QUEUE_ENTER, SEND_SEMANTIC_CUSTOM_SUMMARY, default_body_for_kind, send_semantic_gap  # noqa: E501
...
# Redmine #12750: the ticketless consultation / callback / work-intake durable
# record rendering is factored into its own module. ``build_delivery_record`` and
# ``_anchor_pointer_or_dash`` delegate to these pure renderers so the anchored
# delivery-record renderer no longer grows for every ticketless field.
from mozyo_bridge.e_110_execution_platform.f_130_handoff_routing.domain.ticketless_record_rendering import (
...
# ``SOURCE_TICKETLESS`` (the no-anchor rail source token) is imported from
# ``domain.ticketless_anchors`` and re-exported (Redmine #12750). It is
# deliberately NOT a member of ``SOURCES`` above, so ``normalize_anchor`` / the
# anchored send/reply rails can never select it to bypass the anchor requirement.

MODE_STANDARD = "standard"
# MODE_PENDING / MODE_QUEUE_ENTER are re-exported from handoff_send_semantics (rule owner).
...
def build_notification_body(
    ...
    if anchor.source == SOURCE_TICKETLESS:
        ...
        # forward consultation leg (#12740), the forward work-intake leg (#12748),
        # or the return callback leg (#12703).
        if isinstance(anchor, TicketlessConsultationAnchor):
            noun = "consultation"
        elif isinstance(anchor, TicketlessWorkIntakeAnchor):
            noun = "work-intake"
        else:
            noun = "callback"
        body = (
    ...
    if ticketless_work_intake is not None:
        body = f"{body} {ticketless_work_intake.pointer_clause()}."
    return body
...
class DeliveryOutcome:
    ...
    # Redmine #12748: structured FORWARD ticketless no-anchor parent -> child
    # work-intake fields (`work_shape` / `callback_to_role` / `callback_methods` /
    # `read_contract` / `anchor_decision_owner` and the fixed-true invariants
    # `worker_dispatch_requires_anchor` / `parent_must_not_answer_domain` /
    # `child_owns_anchor_decision`). This is the workflow *request + return
    # contract* of the project-gateway -> child-coordinator forward work-intake,
    # recorded distinctly from this transport outcome, from the grandparent->parent
    # `ticketless_consultation` above, and from the return-leg `ticketless_callback`.
    # Free-text-free (fixed tokens) and durable-record safe in full; no Redmine
    # anchor is fabricated (the child owns the anchor decision) and the
    # worker-dispatch anchor gate is not relaxed. `None` for every other rail (the
...
def _anchor_pointer_or_dash(anchor_payload: Optional[dict[str, Any]]) -> str:
    ...
    if source == SOURCE_TICKETLESS:
        # Redmine #12703 / #12740 / #12748: there is no ticket anchor; the
        # rendering (which names the source plainly and points at the matching
        # structured block below) lives in `ticketless_record_rendering` so the
        # record never implies a fabricated issue/journal (Redmine #12750 split).
        return ticketless_anchor_pointer(anchor_payload)
    return "—"
    ...
    *,
    ...
    return DeliveryOutcome(
        ...
        ticketless_work_intake=(
            ticketless_work_intake.to_structured_dict()
