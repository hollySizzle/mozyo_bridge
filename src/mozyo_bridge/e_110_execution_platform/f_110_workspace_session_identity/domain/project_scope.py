...
This module is **pure**: it parses already-loaded ``project.env`` mappings,
...
- **discovered candidates** — every ``project.env`` carrying the
...
# A `project.env` is recognized only when PROJECT_SCHEMA carries this marker.
# File existence alone never makes a directory routable, and adoption still
# requires PROJECT_RUNTIME_IDENTITY_ENABLED=true.
...
DRIFT_FINGERPRINT = (
    "fingerprint_mismatch"  # cache entry exists but the source project.env changed
)
DRIFT_MISSING_SOURCE = (
    "missing_source"  # cache entry points at a project that no longer discovers
)
...
def project_fingerprint(raw_text: str) -> str:
    """Stable content fingerprint of a ``project.env`` source (``sha256:<hex>``).
...
class ProjectCandidate:
    """One discovered ``project.env`` (schema-marked), adopted or not.
...
def parse_project_environment(
    ...
    *,
    ...
    """Parse one loaded ``project.env`` into a :class:`ProjectCandidate`.

    Returns ``None`` when the environment is not a mozyo project descriptor or
    lacks the mandatory schema/project markers. ``path`` / ``source`` are
    ...
    Adoption is explicit: ``PROJECT_RUNTIME_IDENTITY_ENABLED`` must be truthy.
    The flat names deliberately match Docker/Compose env-file syntax so one
    tracked file is both the runtime input and project-discovery authority.
    """
    if not isinstance(environment, Mapping):
        return None

    schema = str(environment.get("PROJECT_SCHEMA") or "").strip()
    if not schema.startswith(PROJECT_SCHEMA_PREFIX):
        return None

    scope = str(environment.get("PROJECT_REDMINE_PROJECT") or "").strip()
    if not scope:
        return None

    enabled = _as_bool(environment.get("PROJECT_RUNTIME_IDENTITY_ENABLED", "false"))
    kind = (
        str(environment.get("PROJECT_RUNTIME_IDENTITY_KIND") or "project_scope").strip()
        or "project_scope"
    )
    parent_workspace = (
        str(environment.get("PROJECT_PARENT_WORKSPACE") or "").strip() or None
    )
    workdir = str(environment.get("PROJECT_WORKDIR") or ".").strip() or "."

    label = str(environment.get("PROJECT_DISPLAY_LABEL") or "").strip() or scope
...
def detect_cache_drift(
    ...
    local ``project.env``: when the cache and the live source disagree the
        ...
        if candidate is None:
            drifts.append(
                CacheDrift(
                    DRIFT_MISSING_SOURCE,
                    key,
                    "cached project no longer discovers from a source project.env",
                )
            )
            continue
...
def path_under_project(
    candidate_path: str, *, repo_root: str, scope: ProjectScope
) -> bool:
