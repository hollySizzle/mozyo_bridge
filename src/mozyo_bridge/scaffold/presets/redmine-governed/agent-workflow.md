...
- **callback outcome は token ではなく記録で確認する。** `callback_result` が `blocked` / `not-attempted` のときは、同一 durable record に理由を、`blocked` にはさらに replay 可能な retry command を伴うこと (`### Callback 手順`)。token だけを通すと「誰にも渡されないまま park された」状態を token 経由で再導入することになる。`sent` に追加要求は無い。
