...
- N 本並列開発の operator policy (上限・命名規約など)。repo-local dogfood の soft profile は [[logic-sublane-bandwidth-policy]] に置くが、core CLI default にはしない。
...
- 社内固有の削除条件・並列上限・命名規約は private operating policy 側に置く。mozyo_bridge dogfood の local soft profile は [[logic-sublane-bandwidth-policy]] に記録してよいが、portable core default ではないと明示する。
