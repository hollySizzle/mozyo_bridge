...
- **Standard path (required default)** — notify the receiver with the high-level handoff primitive: `mozyo-bridge handoff send --to <claude|codex> --source asana --task-id <gid> --comment-id <story_gid> --kind <implementation_request|design_consultation|review_request|review_result|implementation_done|reply|custom>` (or `mozyo-bridge handoff reply ...` / `mozyo-bridge reply ...`). The primitive owns target resolution, backend-specific admission, at-most-once body injection, submit/turn-start observation, and the structured outcome; callers do not assemble `read` + `message` shell choreography. The `notify-*` wrappers are compatibility entrypoints for Redmine-shaped notifications; `notify-*-legacy-task` is retired-queue cleanup only. `read` / `message` / `type` / `keys` remain operator/debug primitives. On the tmux compatibility backend, default `queue-enter` keeps the historical practical queued-submission mapping (`sent` / `ok` when the marker is observed, `sent` / `queue_enter` when it is not). On Herdr, admission is followed by at most one body injection and the first Enter is zero-or-one. For an idle/turn-ended receiver every issued Enter is preceded by an armed causal wait and a same-generation causal turn-start returns `submitted_confirmed` / exit 0. On a BUSY receiver (ADR-class decision, mozyo_bridge #15537) the working-transition wait cannot attribute anything, so the Enter is issued once through a wait-free full effect fence (fresh generation / composer / screen / state), and the composer releasing the body returns the noncausal `sent` / `queue_enter` / exit 0 (queued submission; `injection_stage=uncertain_partial`, blind retry stays prohibited). Anything else fails closed without retyping the body. Record the literal command, structured outcome, and `injection_stage` in the Asana comment. The durable Asana comment remains the source of truth.
