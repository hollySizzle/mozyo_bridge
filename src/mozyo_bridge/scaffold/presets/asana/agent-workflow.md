...
1. **Standard path** — `mozyo-bridge handoff send` / `handoff reply` / `notify-*` (default `--mode queue-enter` for agent pane handoff; `--mode standard` when strict landing is required).
