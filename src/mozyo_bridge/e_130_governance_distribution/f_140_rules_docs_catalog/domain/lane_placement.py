...
LANE_PLACEMENT_KEYS: frozenset[str] = frozenset(
    {"version"} | set(LANE_PLACEMENT_LANE_CLASSES)
)
...
class LanePlacementConfig:
    ...
    def from_record(
        ...
        class_record = {key: value for key, value in record.items() if key != "version"}
        placements = parse_lane_placement_record(
            class_record or None, source="lane placement config"
        )
        return cls(version=version, placements=placements)
