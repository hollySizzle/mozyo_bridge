...
:mod:`mozyo_bridge.features.operations_cockpit.application.cockpit_payload`.
...
Epic #12502). The served cockpit read-only API payload assembly moved out of the technical-layer ``application/`` package into
...
``mozyo_bridge.application.cockpit_payload`` and the new
``mozyo_bridge.features.operations_cockpit.application.cockpit_payload`` refer to the exact same
