...
:mod:`mozyo_bridge.features.adapter_provider.infrastructure.redmine_credentials`.

US #12595 (parent US #12590) moved Redmine credential resolution out of the loose
top-level ``mozyo_bridge`` namespace into the ``adapter_provider`` Epic-slug
package ``features/adapter_provider/infrastructure/`` (layer-leaf shape, #12591
j#65435), grouping it with the rest of the Redmine adapter seam. No external write
/ network behavior is added by the move. The legacy import path
``mozyo_bridge.redmine_credentials`` is preserved per
