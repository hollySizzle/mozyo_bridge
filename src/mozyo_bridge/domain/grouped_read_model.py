...
:mod:`mozyo_bridge.features.operations_cockpit.domain.grouped_read_model`.
...
Epic #12502). The cockpit grouped read-model projection moved out of the technical-layer ``domain/`` package into
...
``mozyo_bridge.domain.grouped_read_model`` and the new
``mozyo_bridge.features.operations_cockpit.domain.grouped_read_model`` refer to the exact same
