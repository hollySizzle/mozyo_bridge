...
:mod:`mozyo_bridge.features.quality_architecture.domain.module_health`.

The US #12596 quality_architecture source-layout slice (parent US #12590, Feature
#12533 `140_ソース配置管理`) moved the module-health metrics + oversized-module gate
core out of the technical-layer ``domain/`` package and into the Redmine
...
here via ``sys.modules`` so that ``mozyo_bridge.domain.module_health`` and the new
``mozyo_bridge.features.quality_architecture.domain.module_health`` refer to the
