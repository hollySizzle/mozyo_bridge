...
:mod:`mozyo_bridge.features.operations_cockpit.domain.grouped_reload_view`.
...
Epic #12502). The cockpit grouped reload-view projection moved out of the technical-layer ``domain/`` package into
...
``mozyo_bridge.domain.grouped_reload_view`` and the new
``mozyo_bridge.features.operations_cockpit.domain.grouped_reload_view`` refer to the exact same
