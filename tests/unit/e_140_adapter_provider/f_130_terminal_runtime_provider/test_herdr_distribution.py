...
# --- supported-Python compatibility (Redmine #14754) --------------------------

# Runs inside a child interpreter under the Python 3.10 import surface: stdlib
# `tomllib` unimportable, `tomli` present instead.
#
# On a real <3.11 interpreter that surface is already the truth, so the probe
# uses it as-is and asserts against the genuinely installed `tomli`. On 3.11+ it
# has to be simulated — `tomllib` is blocked and a *distinct* module object named
# `tomli` is put in its place, so a run that ends up bound to `tomllib` reads as a
# leak, not a pass. The shim exists because `tomli` is not installed on 3.11+
# (its dependency marker is `python_version < '3.11'`): the pin is on the import
# shape, not on the wheel being present on an interpreter that does not need it.
...
class PinPostureSupportedPythonTest(unittest.TestCase):
    """Pin Redmine #14754: the ops module must import on every supported Python.

    `pyproject` declares `requires-python = ">=3.10"`, but `tomllib` is stdlib
    only on 3.11+. An unconditional `import tomllib` here made the whole herdr
    distribution surface — this module, `cli_herdr_distribution`,
    `herdr_integration_install_ops`, and therefore `mozyo-bridge herdr
    pin-posture` — unimportable on 3.10. The fallback to `tomli` follows the
    precedent in `mozyo_bridge.application.instruction_doctor` and adds no new
    dependency (`tomli>=2.0; python_version < '3.11'` is already declared).
    """
