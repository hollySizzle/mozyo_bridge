...
class FakeOps:
    ...
    def execute_phase(self, *, phase, **kwargs):
        name = phase["phase"]
...
class OfflineRolloutActionTests(unittest.TestCase):
    ...
    def test_approval_note_is_exact_and_enumerates_high_blast_facts(self) -> None:
        ...
        note = render_approval_note(manifest)
        self.assertTrue(approval_matches(note, self.plan, self.digest))
        self.assertEqual(parse_approval_note(note), manifest)
        ...
        for changed in (note + "\n", "prefix\n" + note, note.replace('"ws"', '"x"', 1)):
            self.assertFalse(approval_matches(changed, self.plan, self.digest))
    ...
    def _delegate(self, ops: FakeOps, action_id: str = "offline_" + "a" * 32):
        result = delegate_offline_rollout(
            plan=self.plan,
            plan_digest=self.digest,
    ...
    def test_blocked_phase_resumes_forward_without_repeating_completed_prefix(self) -> None:
        ...
        action_id = self._delegate(ops, "offline_" + "b" * 32)
    ...
    def test_active_phase_is_persisted_before_effect_and_tamper_is_refused(self) -> None:
        ops = FakeOps(fail_once="migrate_attestation")
        action_id = self._delegate(ops, "offline_" + "d" * 32)
    ...
    def test_sealed_store_refuses_tamper_and_uses_private_modes(self) -> None:
        action_id = self._delegate(FakeOps(), "offline_" + "c" * 32)
