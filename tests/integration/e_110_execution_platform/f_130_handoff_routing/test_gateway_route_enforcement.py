...
class GatewayRouteEnforcementCliTest(unittest.TestCase):
    def setUp(self) -> None:
        # The gate resolves the workflow role binding from MOZYO_REPO, falling back
        # to the cwd git root — the live checkout's committed operational config.
        # This fixture hardcodes the DEFAULT binding (codex gateway / claude worker),
        # so pin MOZYO_REPO to a config-less temp dir: the test must not depend on
        # the live checkout's .mozyo-bridge/config.yaml.
